...
def estimate_flops(model, seq_len=512):
    ...
    if hasattr(model, 'shared_neurons'):
        ...
        per_layer = (
            ...
            2 * d_model * d_ff * seq_len           # FFN (⚠️ ISSUE)
        )
```
...
| FFN/Knowledge | ✅ `4 * top_k_know * d_model * knowledge_rank` | ⚠️ `2 * d_model * d_ff` | **수정 필요** |
| Router overhead | ❌ 미포함 | N/A | 확인 필요 |

**Vanilla FFN FLOPs 계산 오류**:
```python
# 현재 코드 (evaluate.py line 87-91)
2 * d_model * d_ff * seq_len  # ← 한 방향만 계산

# 실제 필요한 계산:
# FFN: x @ W1 (d→d_ff) + h @ W2 (d_ff→d)
# = 2*seq*d*d_ff + 2*seq*d_ff*d = 4*d*d_ff*seq
4 * d_model * d_ff * seq_len  # ← 올바른 계산
```

**영향**: Vanilla FLOPs가 실제보다 낮게 계산됨 → DAWN이 상대적으로 불리하게 비교됨
...
| **FLOPs Calculation** | `estimate_flops()` | ⚠️ Check | Vanilla FFN 계산 확인 필요 |
...
| FFN/Knowledge | `4 * top_k_know * d_model * knowledge_rank` | `2 * d_model * d_ff` | ⚠️ **Vanilla 2x 누락** |
...
per_layer = (
    ...
    2 * d_model * d_ff * seq_len           # FFN ← ⚠️ 한 방향만
)

# FFN = up-proj (d→d_ff) + down-proj (d_ff→d) = 2개 matmul
# 올바른 계산: 4 * d_model * d_ff * seq_len
```

**결론**: Vanilla FLOPs가 실제보다 낮게 계산됨 → DAWN이 상대적으로 불리하게 비교되나, FFN이 총 FLOPs의 일부이므로 영향 제한적
...
2. **FLOPs 계산 수정** (선택적): `scripts/evaluation/evaluate.py` line 90의 Vanilla FFN 계산
   - 현재: `2 * d_model * d_ff * seq_len`
   - 수정: `4 * d_model * d_ff * seq_len` (up + down projection 모두 포함)
   - 영향: Vanilla FLOPs가 ~30% 증가 → DAWN 대비 비율 개선됨
