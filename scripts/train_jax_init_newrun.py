"""
DAWN/Transformer Init-NewRun Training Script (TPU Multi-Device)
...
- Init-newrun: loads params from --init-from and writes a fresh run folder

Usage:
    # Just provide config - auto-resumes if checkpoint exists, otherwise starts fresh
    python scripts/train_jax_init_newrun.py --config configs/train_config_tpu.yaml --init-from gs://.../run_v...

    # Force start from scratch (ignores existing checkpoints)
    python scripts/train_jax_init_newrun.py --config configs/train_config_tpu.yaml --init-from gs://.../run_v... --from-scratch
"""
...
def main():
    ...
    parser.add_argument('--resume-from', type=str, default=None,
                        help='[legacy] Resume from specific run folder path. Avoid this in init-newrun mode.')
    parser.add_argument('--init-from', type=str, required=True,
                        help='Source checkpoint file or run folder to initialize params from. A NEW run folder is always created; source checkpoint is never written to.')
    parser.add_argument('--load-opt-state', action='store_true',
                        help='Also load optimizer state from the source checkpoint. Default: load params only and create a fresh optimizer.')
    parser.add_argument('--preserve-step', action='store_true',
                        help='When used with --load-opt-state, keep epoch/step from checkpoint. Default resets epoch/step to 0 for a new run.')
    parser.add_argument('--inherit-model-config', action=argparse.BooleanOptionalAction, default=True,
                        help='Load model shape config from config.json next to --init-from, then let YAML keys override it. Default: true.')
    parser.add_argument('--inherit-training-config', action=argparse.BooleanOptionalAction, default=False,
                        help='Also inherit training hyperparameters from config.json next to --init-from. Default: false.')
    ...
    # ----------------------------------------------------------
    # Init checkpoint resolution + optional config inheritance
    # ----------------------------------------------------------
    def _parent_dir(path_str):
        path_str = str(path_str).rstrip('/')
        if _is_gcs(path_str):
            return path_str.rsplit('/', 1)[0]
        return str(Path(path_str).parent)

    def _resolve_init_checkpoint(src):
        src = str(src).rstrip('/')
        if src.endswith('.flax'):
        ...
        raise FileNotFoundError(f"No .flax checkpoint found under --init-from: {src}")

    init_checkpoint_path, init_checkpoint_dir = _resolve_init_checkpoint(cli_args.init_from)

    if cli_args.inherit_model_config or cli_args.inherit_training_config:
        source_config_json = (init_checkpoint_dir.rstrip('/') + '/config.json'
                              if _is_gcs(init_checkpoint_dir)
                              else str(Path(init_checkpoint_dir) / 'config.json'))
        if _file_exists(source_config_json):
            try:
                with _open_file(source_config_json, 'r') as f:
                    source_cfg = json.loads(f.read())
                if cli_args.inherit_model_config and source_cfg.get('model'):
                    merged_model = dict(source_cfg.get('model', {}))
                    merged_model.update(cfg.get('model', {}))
                    cfg['model'] = merged_model
                if cli_args.inherit_training_config and source_cfg.get('training'):
                    merged_training = dict(source_cfg.get('training', {}))
                    merged_training.update(cfg.get('training', {}))
                    cfg['training'] = merged_training
                if jax.process_index() == 0:
                    print(f"  Inherited config from source checkpoint: {source_config_json}")
                    print(f"  Init checkpoint: {init_checkpoint_path}")
            except Exception as e:
                if jax.process_index() == 0:
                    print(f"  Warning: failed to inherit source config: {e}")
        elif jax.process_index() == 0:
            print(f"  Warning: source config.json not found: {source_config_json}")
            print("  YAML config must fully specify model/data/training shapes.")
    ...
    if cli_args.init_from:
        # Init-newrun mode: source checkpoint is read-only and checkpoint_dir is
        # intentionally left None so a fresh run folder is created below.
        # Broadcast host-0's resolved checkpoint path so all hosts load exactly
        # the same source file even if GCS listings race or cache differently.
        resume_path = _broadcast_str_from_host0(init_checkpoint_path)
        checkpoint_dir = None
        if jax.process_index() == 0:
            print(f"  Init-newrun mode: loading params from {resume_path}")
            print("  Source checkpoint folder will NOT be written to.")
    ...
    # ----------------------------------------------------------
    if resume_path and (not cli_args.init_from) and _file_exists(resume_path):
    ...
    if resume_path and _file_exists(resume_path):
        ...
        if cli_args.init_from:
            # New-run initialization: do not mutate source folder, do not append
            # logs, and by default do not continue optimizer/scheduler state.
            if cli_args.load_opt_state:
                opt_state = ckpt['opt_state']
            if cli_args.load_opt_state and cli_args.preserve_step:
                ...
                best_val_loss = float('inf')
            if is_host0:
                opt_msg = "loaded" if cli_args.load_opt_state else "fresh"
                print(f"  Init-newrun: params loaded, optimizer={opt_msg}, step={global_step}")
                print(f"  New checkpoints/logs will be written only to: {checkpoint_dir}")
        else:
            opt_state = ckpt['opt_state']
            start_epoch = ckpt.get('epoch', 0)
            global_step = ckpt.get('step', 0)
    ...
    if _spec is not None and (mesh_model > 1 or _force_sharded):
        ...
        # v3.9.4 legacy expects sharded_fns as a tuple:
        #   (fused_single, fused_paired)
        # Current v4.1+/SRW expects a dict with separate attn_v/rst functions.
        if model_version == 'spatial-r1-v3.9.4':
            _legacy_single_chunk = min(attn_v_max_chunk, rst_max_chunk)
            _legacy_single = make_sharded_srw(
                max_chunk_size=_legacy_single_chunk, **_srw_base_kwargs)
            if hasattr(_v3, 'make_sharded_srw_paired'):
                _legacy_paired = _v3.make_sharded_srw_paired(
                    max_chunk_size=attn_qk_max_chunk, **_srw_base_kwargs)
                _sharded_fns = (_legacy_single, _legacy_paired)
            else:
                _sharded_fns = _legacy_single
        else:
    ...
    # ----------------------------------------------------------
    # Multi-host preflight / OOM check
    # ----------------------------------------------------------
    # Disabled by default for TPU pods. A short-lived dummy train_step
    # can touch device_put/sharding before every worker has fully joined
    # the PJRT slice, which may abort with:
    #   GetSliceInfo can only be invoked after a slice is built...
    # The real training loop below compiles the same train_step on all
    # workers together and is the safer multi-host path.
    run_oom_check = bool(tcfg.get('run_oom_check', False))
    if is_host0:
        if run_oom_check:
            print("\n=== OOM/preflight check requested by config: run_oom_check=true ===", flush=True)
        else:
            print("\n=== OOM/preflight check skipped for multi-host TPU pod ===", flush=True)
            print("  train_step will compile on the first real batch after all workers join.", flush=True)

    if run_oom_check:
        if is_host0:
            print("  WARNING: run_oom_check=true may crash on multi-host TPU pods; use only for debugging.", flush=True)
        # Intentionally no dummy device_put here. If you need a true OOM
        # probe, run a short real training job with max_steps/total_steps
        # reduced in the config.
    ...
    if is_host0:
        ...
        _is_log_resume = (resume_path is not None) and (not cli_args.init_from) and bool(_existing_logs)
        if _is_log_resume:
