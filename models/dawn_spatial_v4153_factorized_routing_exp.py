"""
DAWN-SRW v4.1.5.3: Two-stage gate with joint/factorized routing
...
### Neurons (rank-1 atomic units)
  - Embedding/tag: free norm in routing
  - Read / Write: unit norm (forward)

  Read/write represent direction. Learned tag norm is allowed to express
  routing strength.
...
### Denominator
    route_i = concat(tag_i, normalize(unit_read_i @ Rr),
                     normalize(unit_write_i @ Rw))
    a_i = activation_i * intensity_i * <x, unit_read_i>
    den = max(sum(activation_i * intensity_i), 1.0)

  v4.1.5.2 routes on a learned free-norm tag plus a fixed functional signature
...
DEFAULT_ROUTING_TYPE = "joint"      # "joint" | "factorized"
DEFAULT_ROUTE_TAG_WEIGHT = 1.0
DEFAULT_ROUTE_READ_WEIGHT = 1.0
DEFAULT_ROUTE_WRITE_WEIGHT = 1.0
...
def fixed_sig_proj_init():
    ...
    Zero-width signature dims are allowed for tag-only/factorized routing.
    """
    def init(key, shape, dtype=jnp.float32):
        if shape[-1] == 0:
...
def _plain_layer_norm_no_params(x, eps=1e-6):
    mean = jnp.mean(x, axis=-1, keepdims=True)
    var = jnp.mean(jnp.square(x - mean), axis=-1, keepdims=True)
    return (x - mean) / jnp.sqrt(var + eps)


def _score_against_bank(query, bank):
    """query [..., D], bank [N, D] -> scores [..., N]."""
    if query.ndim == 3:
        return query @ bank.T
    if query.ndim == 4:
        return jnp.einsum('bsrd,nd->bsrn', query, bank)
    raise ValueError(f"Unsupported query rank for routing scores: {query.ndim}")


def score_route_chunk(x, h, tag, read_unit, write_unit,
                      read_sig_proj, write_sig_proj,
                      routing_type=DEFAULT_ROUTING_TYPE,
                      route_tag_weight=DEFAULT_ROUTE_TAG_WEIGHT,
                      route_read_weight=DEFAULT_ROUTE_READ_WEIGHT,
                      route_write_weight=DEFAULT_ROUTE_WRITE_WEIGHT):
    """Compute routing scores for one local neuron chunk.

    joint:
        route = concat(tag, read_sig, write_sig)
        score = h @ route.T

    factorized:
        score = w_tag * tag_score + w_read * read_score + w_write * write_score

    If tag_dim == d_model, tag scoring is direct LN(x) @ tag.T.
    Otherwise, the tag query is taken from the first tag_dim channels of h.
    Read/write queries are taken from subsequent h slices.
    """
    if routing_type == "joint":
        route = build_route_signature(tag, read_unit, write_unit,
                                      read_sig_proj, write_sig_proj)
        return _score_against_bank(h, route)

    if routing_type != "factorized":
        raise ValueError(f"Unknown routing_type={routing_type!r}; expected 'joint' or 'factorized'")

    tag_dim = tag.shape[-1]
    d_model = x.shape[-1]
    read_sig_dim = read_sig_proj.shape[-1]
    write_sig_dim = write_sig_proj.shape[-1]

    x_norm = _plain_layer_norm_no_params(x)
    if h.ndim == 4:
        x_norm_for_tag = jnp.broadcast_to(x_norm[:, :, None, :], h.shape[:-1] + (d_model,))
    else:
        x_norm_for_tag = x_norm

    if tag_dim == d_model:
        h_tag = x_norm_for_tag
    else:
        h_tag = h[..., :tag_dim]

    score = jnp.float32(route_tag_weight) * _score_against_bank(
        h_tag.astype(jnp.float32), tag.astype(jnp.float32))
...
def make_sharded_srw(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    def fused_gate_srw(x, h, tag_local, tau_offset, scan_bias,
        ...
        def route_chunk(start):
            ...
            return tc, rc_f.astype(jnp.bfloat16), wc_f.astype(jnp.bfloat16)
        ...
        if analysis:
            ...
            def stats_step(carry, i):
                ...
                s = i * cs
                tc, rc, wc = route_chunk(s)
                scores_f = score_route_chunk(
                    x_bf, h_bf, tc, rc.astype(jnp.float32), wc.astype(jnp.float32),
                    read_sig_proj, write_sig_proj, routing_type,
                    route_tag_weight, route_read_weight, route_write_weight)
                scores = scores_f.astype(jnp.bfloat16)
...
class NeuronPool(nn.Module):
    ...
    def setup(self):
        ...
        # Learned route tags keep norm as a routing-strength DoF. Read/write
        # operator signatures still come from unit read/write directions.
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    _attn_tag_direct_only = (
        routing_type == "factorized"
        and qk_emb.shape[-1] == D and v_emb.shape[-1] == D
        and (qk_read_sig_proj.shape[-1] == 0 or route_read_weight == 0.0)
        and (qk_write_sig_proj.shape[-1] == 0 or route_write_weight == 0.0)
        and (v_read_sig_proj.shape[-1] == 0 or route_read_weight == 0.0)
        and (v_write_sig_proj.shape[-1] == 0 or route_write_weight == 0.0))
    if _attn_tag_direct_only:
        h_Q = jnp.zeros_like(x)
        h_K = jnp.zeros_like(x)
        h_V = jnp.zeros_like(x)
    else:
...
def _know_forward(x, pool_params, router_params, rng,
    ...
    rng, rng_drop = jax.random.split(rng)
    _know_tag_direct_only = (
        routing_type == "factorized"
        and know_emb.shape[-1] == x.shape[-1]
        and (know_read_sig_proj.shape[-1] == 0 or route_read_weight == 0.0)
        and (know_write_sig_proj.shape[-1] == 0 or route_write_weight == 0.0))
    if _know_tag_direct_only:
        h = jnp.zeros_like(x)
    else:
...
class DAWN(nn.Module):
    ...
    __version__ = "spatial-r1-v4.1.5.3"
    ...
    read_sig_dim: int = DEFAULT_READ_SIG_DIM
    write_sig_dim: int = DEFAULT_WRITE_SIG_DIM
    routing_type: str = DEFAULT_ROUTING_TYPE
    route_tag_weight: float = DEFAULT_ROUTE_TAG_WEIGHT
    route_read_weight: float = DEFAULT_ROUTE_READ_WEIGHT
    route_write_weight: float = DEFAULT_ROUTE_WRITE_WEIGHT
    n_qk: int = 1580
    n_v: int = 2600
    n_know: int = 25200
    router_dropout: float = 0.1
    ...
    def setup(self):
        ...
        if self.routing_type not in ("joint", "factorized"):
            raise ValueError(f"routing_type must be 'joint' or 'factorized', got {self.routing_type!r}")
        if self.tag_dim + self.read_sig_dim + self.write_sig_dim != self.d_route:
            raise ValueError(
                f"d_route must equal tag_dim + read_sig_dim + write_sig_dim, got "
                f"d_route={self.d_route}, tag_dim={self.tag_dim}, "
        ...
        if self.is_initializing():
            ...
            def scan_body(carry, xs):
                ...
                attn_ret = _attn_forward(
                    ...
                    route_read_weight=self.route_read_weight,
                    route_write_weight=self.route_write_weight)
    ...
    def get_config(self):
        return {
            ...
            'write_sig_dim': self.write_sig_dim,
            'routing_type': self.routing_type,
            'route_tag_weight': self.route_tag_weight,
            'route_read_weight': self.route_read_weight,
            'route_write_weight': self.route_write_weight,
        }
