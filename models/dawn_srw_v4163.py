"""
DAWN-SRW v4.1.6.3 Generalized Gaussian Boundary DirectTau

Clean v4163 experimental model path.

Implemented concepts:
- cosine-space tau reference with bounded sigmoid min/max mapping
- one-sided generalized Gaussian boundary DirectTau gate
...
# v4163 exposure diagnostics are soft-gate based, not hard score>tau based.
...
def _gate_eps_suffix(value):
    ...
    for known_value, suffix in GATE_EPS_VALUE_TO_SUFFIX.items():
        if math.isclose(value_f, known_value, rel_tol=0.0, abs_tol=known_value * 1.0e-6):
            return suffix
    raise ValueError(
        f"Unsupported v4163 sparsity eps={value!r}; supported values are "
...
def _tau_from_param(raw_tau):
    """v4163 cosine-space bounded tau using a sigmoid min/max map."""
...
def _boundary_gate_from_margin(margin, intensity, boundary_scale,
                               boundary_power):
    soft_weight = _boundary_soft_weight_from_margin(
        margin, boundary_scale, boundary_power)
    return soft_weight * intensity


def _compute_soft_gate(score, tau, intensity, boundary_scale,
                       boundary_power=2.0,
                       effective_active_eps=1.0e-6,
                       execution_prune_eps=0.0):
    """Clean v4163 one-sided generalized Gaussian boundary gate."""
    execution_prune_eps = jnp.asarray(execution_prune_eps, dtype=jnp.float32)
    margin = score - tau
    soft_weight = _boundary_soft_weight_from_margin(
        margin, boundary_scale, boundary_power)
    gate_unpruned = soft_weight * intensity
    gate = jnp.where(
        execution_prune_eps > 0.0,
...
def make_sharded_srw(mesh, max_chunk_size=2048,
    ...
    One-sided generalized Gaussian boundary DirectTau gate:
        rho              = cosine(q_select, signature)
        margin           = rho - tau
        gate             = exp(-((max(tau - rho, 0) / B) ** p)) * intensity
        den              = max(sum(gate), 1.0)
    ...
    def fused_gate_srw(x, h, emb_local, raw_tau,
        ...
        def route_rw_chunk(start):
            ...
            # v4.1.6.3: stored read/write params stay raw, but SRW
        ...
        # v4.1.6.3 so regular training does not require a rho-statistics pass.
        ...
        def angular_gate_parts(rho, intensity):
            (selection_margin, positive_margin, base_gate,
             active_mask) = _compute_soft_gate(
                rho, tau, intensity, soft_gate_temperature,
        ...
        def gate_sparsity_parts(selection_margin, intensity, gate):
            ...
            projected_gate = _boundary_gate_from_margin(
                margin_sg, intensity_sg, soft_gate_t_final,
                soft_gate_boundary_power_final)
        ...
        def soft_gate_exposure_parts(gate_unpruned):
            # v4163 soft-gate exposure diagnostic only.  Do not use the hard
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                # base_gate may be execution-pruned during eval sweeps.
                # Keep current_cost on the unpruned soft/hard gate so retained
                # mass and unpruned-den diagnostics remain meaningful.
        ...
        # max(sum(soft_weight * intensity), 1.0).
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048,
    ...
    Boundary DirectTau gate:
        exp(-((max(tau - rho, 0) / B) ** p)) * intensity.
...
class Router(nn.Module):
    ...
    def setup(self):
        ...
        if missing_tau:
            raise ValueError(
                "v4163 requires explicit cosine-space tau_init_attn_qk/v/rst; "
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    def _focus_srw_rows_for_pool(pool_id, x_in, h_in, emb, raw_tau, read, write, pool_scale):
        ...
        margin, pos_margin, gate, active_mask = _compute_soft_gate(
            rho, tau, intensity, pool_soft_gate_T,
            boundary_power=soft_gate_boundary_power,
            execution_prune_eps=execution_prune_eps)
...
def _rst_forward(x, pool_params, router_params, rng,
    ...
    def _focus_srw_rows_for_pool(pool_id, x_in, h_in, emb, raw_tau, read, write, pool_scale):
        ...
        tau = _tau_from_param(raw_tau[_focus_b, _focus_pos, 0].astype(jnp.float32))[:, None]
        margin, pos_margin, gate, active_mask = _compute_soft_gate(
            rho, tau, intensity, soft_gate_T_rst,
            boundary_power=soft_gate_boundary_power,
            execution_prune_eps=execution_prune_eps)
        gate_sum = gate.sum(axis=-1)
        den = jnp.maximum(gate_sum, 1.0)
...
class DAWN(nn.Module):
    """DAWN-SRW v4.1.6.3 with generalized Gaussian boundary gates."""
    __version__ = "spatial-r1-v4.1.6.3"
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        result = {
            ...
            # Denominator diagnostic: sum(soft_weight * intensity).
...
def _angular_gate_kwargs_from_model_cfg(model_cfg):
    """Extract angular Select/Intensity and boundary-gate settings for inference."""
    ...
    if d_select_cfg is None:
        ...
        if intensity_route_dim <= 0:
            raise ValueError(
                "v4.1.6.3 angular inference requires model.d_select "
...
def _tau_init_calibration_scores(params, input_ids, d_select,
    ...
    states and the shared v4163 router/pool parameters. Selection rho follows
...
def _angular_gate(h, emb, raw_tau, raw_scan_offset=None, d_select=None,
                  intensity_beta=0.5, soft_gate_temperature=0.07,
                  soft_gate_boundary_power=4.0,
                  execution_prune_eps=0.0,
                  soft_gate_effective_active_eps=1.0e-6):
    """Canonical v4163 boundary gate for non-sharded inference helpers."""
    rho, intensity = _angular_relation_and_intensity(
        h, emb, d_select, intensity_beta)
    tau = _tau_from_param(raw_tau)
    _, _, gate, _ = _compute_soft_gate(
        rho, tau, intensity, soft_gate_temperature,
        boundary_power=soft_gate_boundary_power,
...
def _srw_inference(x, h, emb, raw_tau, raw_scan_offset, w_read, w_write,
    ...
    # v4.1.6.3: inference uses read/write directions; params stay raw.
...
def _srw_inference_with_gates(x, h, emb, raw_tau, raw_scan_offset, w_read,
    ...
    # v4.1.6.3: analysis inference uses read/write directions; params stay raw.
    r_n = _forward_unit_direction(w_read.astype(jnp.float32))
    w_n = _forward_unit_direction(w_write.astype(jnp.float32))
    gate = _angular_gate(
        h, emb, raw_tau, raw_scan_offset, **angular_gate_kwargs)
...
def build_suppressed_forward(params, model_cfg, suppress_masks):
    ...
    def _srw_sup(x, h, emb, tau_off, raw_scan_offset, w_read, w_write, mult):
        ...
        # v4.1.6.3: suppressed forward uses read/write directions.
        r_n = _forward_unit_direction(w_read.astype(jnp.float32))
        w_n = _forward_unit_direction(w_write.astype(jnp.float32))
        gate = _angular_gate(
