...
Changelog:
  spatial-r1-v4.0.4 (2026-04-20):
    - Drop emb forward normalisation (emb_unit = emb / ||emb||).
      init still uses unit_norm_init so all emb rows start on the
      unit sphere; during training WD + task loss are the only forces
      on ||emb||. Per-neuron "strength" degree of freedom: specialist
      neurons can grow large emb norms, generalists can shrink.
      read / write stay unit-normalised in the forward (size-dominance
      guard), and the v4.0.3 Σz^+ denominator is unchanged.
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    # v4.0.4: no forward emb normalisation. init is unit; WD + task loss
    ...
    if sharded_fns is not None:
        ...
        (QK_out, qk_active, qk_raw_gmax, qk_lb, qk_sstd, qk_es, qk_anm,
         ...
         qk_skew, qk_apt_std, qk_entropy, qk_z_sum) = fused_paired(
        ...
        (V, v_active, v_raw_gmax, v_lb, v_sstd, v_es, v_anm,
         ...
         v_skew, v_apt_std, v_entropy, v_z_sum) = fused_single(
        ...
        (Q, q_active, q_raw_gmax, q_lb, q_sstd, q_es, q_anm,
         ...
         q_skew, q_apt_std, q_entropy, q_z_sum) = _srw_chunked(
        ...
        (K, k_active, k_raw_gmax, k_lb, k_sstd, k_es, k_anm,
         ...
         k_skew, k_apt_std, k_entropy, k_z_sum) = _srw_chunked(
            x, h_K, qk_emb_unit, tau_all[:, :, 1:2], qk_read, qk_write, n_chunks_qk)
        (V, v_active, v_raw_gmax, v_lb, v_sstd, v_es, v_anm,
         v_strong, v_phi_bin, v_z_act,
         v_tau_abs, v_z075, v_z030,
         v_skew, v_apt_std, v_entropy, v_z_sum) = _srw_chunked(
...
def _attn_forward_cached(x, pool_params, router_params, expand_O_kernel,
    ...
    # v4.0.4: no forward emb normalisation.
