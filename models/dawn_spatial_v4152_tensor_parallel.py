...
def _tp_residual(x):
    """Tensor-parallel residual-stream constraint: batch x sequence x D/model."""
    return jax.lax.with_sharding_constraint(x, P('data', None, 'model'))


def _tp_route_signature(tag, read_unit, write_unit, read_sig_proj, write_sig_proj):
    """Route signature for D-sharded read/write vectors inside shard_map."""
    tag_f = tag.astype(jnp.float32)
    r = read_unit.astype(jnp.float32)
    w = write_unit.astype(jnp.float32)
    r_norm = jnp.sqrt(jax.lax.psum(jnp.sum(r * r, axis=-1, keepdims=True), 'model') + 1e-8)
    w_norm = jnp.sqrt(jax.lax.psum(jnp.sum(w * w, axis=-1, keepdims=True), 'model') + 1e-8)
    r = r / r_norm
    w = w / w_norm
    ...
    read_sig = jax.lax.psum(r @ rproj, 'model')
    write_sig = jax.lax.psum(w @ wproj, 'model')
...
def make_sharded_srw(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    _slim_out_specs = (
        P('data', None, 'model'),  # out [B,S,D/model]
    ...
    def fused_gate_srw(x, h, tag_local, tau_offset, scan_bias,
        ...
        def route_chunk(start):
            ...
            route = _tp_route_signature(tc, rc_f, wc_f, read_sig_proj, write_sig_proj)
            rc_f = rc_f / (jnp.sqrt(jax.lax.psum(jnp.sum(rc_f * rc_f, axis=-1, keepdims=True), 'model')) + 1e-8)
            wc_f = wc_f / (jnp.sqrt(jax.lax.psum(jnp.sum(wc_f * wc_f, axis=-1, keepdims=True), 'model')) + 1e-8)
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                xr = jax.lax.psum(x_bf @ rc.T, 'model')
        ...
        strong_frac = total_strong / N_total
        z_mean_active = total_weighted_cost / (global_active + 1e-8)
        ...
        activation_cost_out = total_activation_cost.mean()
        current_cost_out = total_current_cost.mean()
        int_cap_frac_out = jax.lax.stop_gradient(
            total_int_cap_count / jnp.float32(B * S * N_total))
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    _slim_out_specs = (
        P('data', None, None, 'model'),  # out [B,S,2,D/model]
        P('data', None, None),        # active [B,S,1]
        P('data', None, None),        # gate_max [B,S,1]
        P(),                          # lb_loss scalar
        P(),                          # score_std scalar
    ...
    def fused_gate_srw_paired(x, h, tag_local, tau_offset, scan_bias,
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                xr = jax.lax.psum(x_bf @ rc.T, 'model')  # [B,S,N]
                xr_f = xr.astype(jnp.float32)
                a = gate * xr_f[:, :, None, :]
        ...
        current_cost_out = total_current_cost.mean()
        int_cap_frac_out = jax.lax.stop_gradient(
            total_int_cap_count / jnp.float32(B * S * 2 * N_total))
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    h_all = jax.lax.with_sharding_constraint(h_all, P('data', None, None))
    ...
    tau_all = jax.lax.with_sharding_constraint(tau_all, P('data', None, None))
    scan_bias_all = x @ router_params['scan_bias_attn']['kernel'] + router_params['scan_bias_attn']['bias']
    scan_bias_all = jax.lax.with_sharding_constraint(scan_bias_all, P('data', None, None))
    if analysis:
    ...
    K = jax.lax.with_sharding_constraint(K, P('data', 'model', None, None))
    V = jax.lax.with_sharding_constraint(V, P('data', 'model', None, None))
    ...
    out = jax.lax.with_sharding_constraint(out, P('data', 'model', None, None))
    if analysis:
        o_input_norm = jnp.linalg.norm(out, axis=-1).mean()
    out = out.transpose(0, 2, 1, 3).reshape(B, S, D)
    out = _tp_residual(out)
    out = _tp_residual(out @ expand_O_kernel)
...
def _know_forward(x, pool_params, router_params, rng,
    ...
    tau = jax.lax.with_sharding_constraint(tau, P('data', None, None))
    scan_bias = x @ router_params['scan_bias_know']['kernel'] + router_params['scan_bias_know']['bias']
    scan_bias = jax.lax.with_sharding_constraint(scan_bias, P('data', None, None))
    if analysis:
...
class DAWN(nn.Module):
    ...
    __version__ = "spatial-r1-v4.1.5.2"
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        if self.is_initializing():
            ...
            def scan_body(carry, xs):
                ...
                normed = _tp_residual(_layer_norm(
