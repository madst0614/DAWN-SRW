"""
DAWN-Spatial v3.9.5: Sense-Read-Write (JAX/Flax)
...
def threshold_gate(scores, tau_offset):
    ...
    # init path: default sqrt_active
    norm_factor = jnp.maximum(jnp.sqrt(active_n + 1.0), 1.0)
...
def make_sharded_srw(mesh, max_chunk_size=2048, gate_norm_mode="sqrt_active"):
    ...
    def fused_gate_srw(x, h, emb_local, tau_offset, read_local, write_local):
        ...
        global_active_n = jax.lax.psum(total_gate_sum, 'model')
        global_gate_max = jax.lax.pmax(jax.lax.stop_gradient(total_gate_max), 'model')
        if gate_norm_mode == "sqrt_active":
            norm_factor = jnp.maximum(jnp.sqrt(global_active_n + 1.0), 1.0)
        else:  # "active_n"
            norm_factor = jnp.maximum(global_active_n, 1.0)
        ...
        es_out = global_active_n.mean()
        active_n_mean = global_active_n.mean()
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048, gate_norm_mode="sqrt_active"):
...
def _srw_chunked(x, h, emb_unit, tau_offset, w_read, w_write, n_chunks, gate_norm_mode="sqrt_active"):
    ...
    if gate_norm_mode == "sqrt_active":
        norm_factor = jnp.maximum(jnp.sqrt(total_gate_sum + 1.0), 1.0)
    else:  # "active_n"
        norm_factor = jnp.maximum(total_gate_sum, 1.0)
    out = (raw_out / norm_factor).astype(jnp.bfloat16)

    score_std_out = s_std.mean()
    es_out = total_gate_sum.mean()
    active_n_mean = total_gate_sum.mean()
    score_mean_out = s_mean.mean()
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
                  ...
                  sharded_fns=None, gate_norm_mode="sqrt_active"):
    B, S, D = x.shape
    ...
    if sharded_fns is not None:
        ...
        Q, q_active, q_raw_gmax, q_lb, q_sstd, q_es, q_anm, q_smean = _srw_chunked(
            x, h_Q, qk_emb_unit, tau_all[:, :, 0:1], qk_read, qk_write, n_chunks_qk, gate_norm_mode)
        K, k_active, k_raw_gmax, k_lb, k_sstd, k_es, k_anm, k_smean = _srw_chunked(
            x, h_K, qk_emb_unit, tau_all[:, :, 1:2], qk_read, qk_write, n_chunks_qk, gate_norm_mode)
        V, v_active, v_raw_gmax, v_lb, v_sstd, v_es, v_anm, v_smean = _srw_chunked(
            x, h_V, v_emb_unit, tau_all[:, :, 2:3], v_read, v_write, n_chunks_v, gate_norm_mode)
...
def _know_forward(x, pool_params, router_params, rng,
                  router_dropout, dropout_rate, deterministic,
                  n_chunks_know=1, sharded_fns=None, gate_norm_mode="sqrt_active"):
    ...
    if sharded_fns is not None:
        ...
        out, active_frac, raw_gate_max, lb_loss, score_std, gate_sum, active_n_mean, score_mean = _srw_chunked(
            x, h, know_emb_unit, tau, know_read, know_write, n_chunks_know, gate_norm_mode)
...
class DAWN(nn.Module):
    ...
    gate_norm_mode: str = "sqrt_active"  # "sqrt_active" or "active_n"
            ...
            def scan_body(carry, xs):
                ...
                (attn_out, attn_aux, a_qk_active, a_v_active, a_raw_gmax,
                 ...
                 a_q_norm, a_k_norm, a_v_norm_dbg, a_logit_max, a_o_input_norm
                    ...
                    sharded_fns=_sharded, gate_norm_mode=self.gate_norm_mode)
                x = x + attn_out
                ...
                (know_out, know_aux, k_active, k_raw_gmax, k_sstd, k_gsum, k_active_n_mean,
                 ...
                 k_tau_mean, k_raw_out_norm
                    ...
                    self.router_dropout, self.dropout_rate, deterministic,
                    self.n_chunks_know, sharded_fns=_sharded, gate_norm_mode=self.gate_norm_mode)
                x = x + know_out
...
def _srw_inference(x, h, emb_norm, tau_offset, w_read, w_write, gate_norm_mode="sqrt_active"):
    """Non-chunked SRW for inference (S=1 typically).
    No checkpoint, no LB loss, no bf16 casting.
    STE binary gate (pure binary at inference).
    """
    scores = h @ emb_norm.T
    scores_f32 = scores.astype(jnp.float32)
    s_mean = scores_f32.mean(axis=-1, keepdims=True)
    ...
    xr = x @ r_n.T
    raw_out = (gate * xr) @ w_n
    out = raw_out.astype(jnp.float32) / norm_factor
    return out.astype(jnp.float32)


def _srw_inference_with_gates(x, h, emb_norm, tau_offset, w_read, w_write, gate_norm_mode="sqrt_active"):
...
def build_suppressed_forward(params, model_cfg, suppress_masks):
    ...
    def _srw_sup(x, h, emb_n, tau_off, w_read, w_write, mult):
        """SRW with optional gate suppression. Binary gate (inference)."""
        scores = h @ emb_n.T
        ...
        active_n = gate.sum(axis=-1, keepdims=True).astype(jnp.float32)
        norm_factor = jnp.maximum(jnp.sqrt(active_n + 1.0), 1.0)  # default sqrt_active
        r_n = w_read / (jnp.linalg.norm(w_read, axis=-1, keepdims=True) + 1e-8)
        w_n = w_write / (jnp.linalg.norm(w_write, axis=-1, keepdims=True) + 1e-8)
        xr = x @ r_n.T
        out = (gate * xr) @ w_n
        return (out.astype(jnp.float32) / norm_factor).astype(jnp.float32)
