...
_ACTIVE_EDGE_BACKWARD_PROFILES = {
    # The shared capacity is per dense operator chunk.  It is deliberately
    # larger than the calibrated active mass, while remaining far below the
    # full token x operator rectangle.  Capacity overflow is exact: complete
    # operators are replayed through compact operator-block tasks, and no edge
    # is dropped.
    "qk": {
        "capacity_fraction": 0.40,
        "operator_edge_block_size": 1024,
        "token_edge_block_size": 256,
        "operator_task_group_size": 64,
        "token_task_group_size": 128,
    },
    "v": {
        "capacity_fraction": 0.15,
        "operator_edge_block_size": 512,
        "token_edge_block_size": 128,
        "operator_task_group_size": 128,
        "token_task_group_size": 256,
    },
    "rst": {
        "capacity_fraction": 0.075,
        "operator_edge_block_size": 256,
        "token_edge_block_size": 64,
        "operator_task_group_size": 256,
        "token_task_group_size": 512,
    },
}
...
def _active_edge_backward_layout(
        profile_name: str,
        n_routes: int,
        n_spaces: int,
        token_capacity: int,
        chunk_size: int) -> dict[str, int]:
    """Resolve one static shared-capacity operator-major backward layout."""
    if profile_name not in _ACTIVE_EDGE_BACKWARD_PROFILES:
        raise ValueError(
            f"unsupported active-edge backward profile {profile_name!r}")
    profile = _ACTIVE_EDGE_BACKWARD_PROFILES[profile_name]
    dense_edge_count = (
        int(n_routes) * int(n_spaces)
        * int(token_capacity) * int(chunk_size))
    operator_edge_block_size = min(
        int(profile["operator_edge_block_size"]), int(token_capacity))
    operator_edge_block_size = max(1, operator_edge_block_size)
    token_edge_block_size = min(
        int(profile["token_edge_block_size"]),
        int(n_routes) * int(chunk_size))
    token_edge_block_size = max(1, token_edge_block_size)
    requested_capacity = max(
        1,
        math.ceil(
            dense_edge_count * float(profile["capacity_fraction"])))
    edge_capacity = min(
        dense_edge_count,
        math.ceil(requested_capacity / operator_edge_block_size)
        * operator_edge_block_size)
    operator_count = int(n_routes) * int(n_spaces) * int(chunk_size)
    token_owner_count = int(n_spaces) * int(token_capacity)
    operator_task_group_size = min(
        int(profile["operator_task_group_size"]), operator_count)
    token_task_group_size = min(
        int(profile["token_task_group_size"]), token_owner_count)
    raw_operator_task_capacity = (
        math.ceil(edge_capacity / operator_edge_block_size)
        + operator_count)
    raw_token_task_capacity = (
        math.ceil(edge_capacity / token_edge_block_size)
        + token_owner_count)
    operator_task_capacity = (
        math.ceil(
            raw_operator_task_capacity / operator_task_group_size)
        * operator_task_group_size)
    token_task_capacity = (
        math.ceil(raw_token_task_capacity / token_task_group_size)
        * token_task_group_size)
    return {
        "edge_capacity": edge_capacity,
        "operator_edge_block_size": operator_edge_block_size,
        "token_edge_block_size": token_edge_block_size,
        "operator_count": operator_count,
        "token_owner_count": token_owner_count,
        "operator_task_group_size": operator_task_group_size,
        "token_task_group_size": token_task_group_size,
        "operator_task_capacity": operator_task_capacity,
        "token_task_capacity": token_task_capacity,
    }


def _pack_active_edges_operator_major(
        selection_margin: jax.Array,
        valid: jax.Array,
        edge_capacity: int) -> tuple[jax.Array, jax.Array, jax.Array]:
    """Prefix-pack derivative-support edges with exact operator overflow."""
    valid = jnp.broadcast_to(valid, selection_margin.shape)
    n_routes, n_spaces, token_capacity, chunk_size = map(
        int, selection_margin.shape)
    capacity = int(edge_capacity)
    if capacity <= 0:
        raise ValueError("active-edge capacity must be positive")

    # JAX's clip uses a nonzero boundary subgradient at exactly zero.  Include
    # the equality boundary even though its forward admission is zero.
    support = (selection_margin >= jnp.float32(0.0)) & valid
    operator_major = jnp.transpose(support, (0, 1, 3, 2))
    operator_count = n_routes * n_spaces * chunk_size
    flat_support = operator_major.reshape((operator_count, token_capacity))
    counts = flat_support.sum(axis=1, dtype=jnp.int32)
    offsets = jnp.concatenate((
        jnp.zeros((1,), dtype=jnp.int32),
        jnp.cumsum(counts[:-1], dtype=jnp.int32),
    ))
    primary_operator = (
        offsets + counts <= jnp.int32(capacity))
    primary_counts = jnp.where(
        primary_operator, counts, jnp.int32(0))
    local_rank = (
        jnp.cumsum(flat_support, axis=1, dtype=jnp.int32)
        - flat_support.astype(jnp.int32))
    candidate_valid = (
        flat_support & primary_operator[:, None])
    destination = offsets[:, None] + local_rank
    destination = jnp.where(
        candidate_valid, destination, jnp.int32(capacity))
    candidate_token = jnp.broadcast_to(
        jnp.arange(token_capacity, dtype=jnp.int32)[None, :],
        flat_support.shape)
    packed_token = jnp.zeros(
        (capacity,), dtype=jnp.int32
    ).at[destination].set(candidate_token, mode="drop")
    overflow_operator = (counts > 0) & ~primary_operator
    return packed_token, primary_counts, overflow_operator


def _pack_active_edges_token_major(
        selection_margin: jax.Array,
        valid: jax.Array,
        edge_capacity: int) -> tuple[jax.Array, jax.Array, jax.Array]:
    """Prefix-pack the same derivative support with token-owned offsets."""
    valid = jnp.broadcast_to(valid, selection_margin.shape)
    n_routes, n_spaces, token_capacity, chunk_size = map(
        int, selection_margin.shape)
    capacity = int(edge_capacity)
    ...
    support = (selection_margin >= jnp.float32(0.0)) & valid
    token_major = jnp.transpose(support, (1, 2, 0, 3))
    token_owner_count = n_spaces * token_capacity
    edges_per_owner = n_routes * chunk_size
    flat_support = token_major.reshape(
        (token_owner_count, edges_per_owner))
    counts = flat_support.sum(axis=1, dtype=jnp.int32)
    offsets = jnp.concatenate((
        jnp.zeros((1,), dtype=jnp.int32),
        jnp.cumsum(counts[:-1], dtype=jnp.int32),
    ))
    primary_owner = offsets + counts <= jnp.int32(capacity)
    primary_counts = jnp.where(
        primary_owner, counts, jnp.int32(0))
    local_rank = (
        jnp.cumsum(flat_support, axis=1, dtype=jnp.int32)
        - flat_support.astype(jnp.int32))
    candidate_valid = flat_support & primary_owner[:, None]
    destination = offsets[:, None] + local_rank
    destination = jnp.where(
        candidate_valid, destination, jnp.int32(capacity))
    candidate_edge = jnp.broadcast_to(
        jnp.arange(edges_per_owner, dtype=jnp.int32)[None, :],
        flat_support.shape)
    packed_edge = jnp.zeros(
        (capacity,), dtype=jnp.int32
    ).at[destination].set(candidate_edge, mode="drop")
    overflow_owner = (counts > 0) & ~primary_owner
    return packed_edge, primary_counts, overflow_owner


def _build_primary_active_edge_tasks(
        primary_counts: jax.Array, *,
        edge_block_size: int,
        task_capacity: int,
        max_edges_per_owner: int) -> tuple[
            jax.Array, jax.Array, jax.Array, jax.Array]:
    """Build compact owner/block tasks for the shared primary edge buffer."""
    owner_count = int(primary_counts.shape[0])
    block_size = int(edge_block_size)
    capacity = int(task_capacity)
    edge_offsets = jnp.concatenate((
        jnp.zeros((1,), dtype=jnp.int32),
        jnp.cumsum(primary_counts[:-1], dtype=jnp.int32),
    ))
    task_counts = (
        primary_counts + jnp.int32(block_size - 1)
    ) // jnp.int32(block_size)
    task_offsets = jnp.concatenate((
        jnp.zeros((1,), dtype=jnp.int32),
        jnp.cumsum(task_counts[:-1], dtype=jnp.int32),
    ))
    candidate_block_count = max(
        1, math.ceil(int(max_edges_per_owner) / block_size))
    candidate_block = jnp.arange(
        candidate_block_count, dtype=jnp.int32)
    candidate_valid = candidate_block[None, :] < task_counts[:, None]
    destination = task_offsets[:, None] + candidate_block[None, :]
    destination = jnp.where(
        candidate_valid, destination, jnp.int32(capacity))
    candidate_owner = jnp.broadcast_to(
        jnp.arange(owner_count, dtype=jnp.int32)[:, None],
        destination.shape)
    task_owner = jnp.full(
        (capacity,), jnp.int32(owner_count), dtype=jnp.int32
    ).at[destination].set(candidate_owner, mode="drop")
    task_block = jnp.zeros(
        (capacity,), dtype=jnp.int32
    ).at[destination].set(
        jnp.broadcast_to(candidate_block[None, :], destination.shape),
        mode="drop")
    return (
        task_owner,
        task_block,
        task_counts.sum(dtype=jnp.int32),
        edge_offsets,
    )


def _dense_rw_output_impl_sharded(
        local_f32: jax.Array,
        ...
        active_edge_backward_profile: Optional[str] = None):
    """Compute dense forward output and optionally compact VJP metadata."""
    ...
    active_layout = None
    if active_edge_backward_profile is not None:
        active_layout = _active_edge_backward_layout(
            active_edge_backward_profile,
            n_routes, n_spaces, token_capacity, chunk_size)
    ...
    def production_step(carry_value, chunk_index):
        ...
        rho_compute = jnp.where(valid, rho, tau)
        (selection_margin, gate, _,
         execution_weight, _) = _shared_compute_admission_drive(
            rho_compute, tau, soft_gate_temperature,
            boundary_power=soft_gate_boundary_power,
        ...
        if active_layout is None:
            ...
            metadata = (
                _pack_active_edges_operator_major(
                    selection_margin,
                    valid,
                    active_layout["edge_capacity"]),
                _pack_active_edges_token_major(
                    selection_margin,
                    valid,
                    active_layout["edge_capacity"]),
            )
        return next_carry, metadata

    scan_step = jax.checkpoint(production_step, prevent_cse=False)
    (raw_out, gate_mass), metadata = jax.lax.scan(
        scan_step, carry, jnp.arange(n_chunks))
    if active_layout is not None:
        return raw_out, gate_mass, metadata
    return raw_out, gate_mass


def _active_edge_sparse_rw_gradients_sharded_loop_reference(
        local_f32: jax.Array,
        local_norm: jax.Array,
        token_valid: jax.Array,
        read_vectors: jax.Array,
        ...
        execution_prune_eps: jax.Array,
        packed_metadata,
        raw_out_cotangent: jax.Array,
        gate_mass_cotangent: jax.Array, *,
        max_chunk_size: int,
        srw_composition_mode: str,
        heat_kernel_beta: float,
        effective_active_eps: float,
        throughput_bf16: bool,
        active_edge_backward_profile: str):
    """Differentiate active edges after operator-local dense reductions."""
    n_routes, n_spaces, token_capacity, d_route = map(
        int, (read_vectors.shape[0], local_f32.shape[0],
              local_f32.shape[1], local_f32.shape[2]))
    n_local = int(read_vectors.shape[2])
    ...
    def normalized_padded(value):
        return forward_unit_direction(jnp.pad(value, pad_spec))

    reads, read_normalization_pullback = jax.vjp(
        normalized_padded, read_vectors)
    writes, write_normalization_pullback = jax.vjp(
        normalized_padded, write_vectors)
    valid_rows = jnp.arange(n_padded) < n_local
    beta = jnp.float32(heat_kernel_beta)
    active_eps = jnp.float32(effective_active_eps)
    rw_dot = (
        _throughput_einsum_bf16_f32
        if throughput_bf16 else _control_einsum_f32)
    layout = _active_edge_backward_layout(
        active_edge_backward_profile,
        n_routes, n_spaces, token_capacity, chunk_size)
    edge_capacity = int(layout["edge_capacity"])
    edge_block_size = int(layout["edge_block_size"])
    operator_count = int(layout["operator_count"])
    task_group_size = int(layout["task_group_size"])
    task_capacity = int(layout["task_capacity"])
    max_tasks_per_operator = math.ceil(
        token_capacity / edge_block_size)
    overflow_task_capacity = (
        math.ceil(
            operator_count * max_tasks_per_operator
            / task_group_size)
        * task_group_size)
    combined_task_capacity = (
        task_capacity + overflow_task_capacity)
    edge_slots = jnp.arange(edge_block_size, dtype=jnp.int32)

    packed_tokens_by_chunk = packed_metadata[0]
    primary_counts_by_chunk = packed_metadata[1]
    overflow_by_chunk = packed_metadata[2]
    gradient_carry = (
        jnp.zeros(local_f32.shape, dtype=jnp.float32),
        jnp.zeros(local_norm.shape, dtype=jnp.float32),
        jnp.zeros(
            (n_routes, n_spaces, n_padded, d_route),
            dtype=jnp.float32),
        jnp.zeros(
            (n_routes, n_spaces, n_padded, d_route),
            dtype=jnp.float32),
        jnp.zeros(raw_tau.shape, dtype=jnp.float32),
        jnp.zeros(
            soft_gate_temperature.shape, dtype=jnp.float32),
        jnp.zeros(
            soft_gate_boundary_power.shape, dtype=jnp.float32),
        jnp.zeros(
            execution_prune_eps.shape, dtype=jnp.float32),
    )

    def chunk_gradients(carry_value, chunk_index):
        packed_tokens = packed_tokens_by_chunk[chunk_index]
        primary_counts = primary_counts_by_chunk[
            chunk_index].reshape((operator_count,))
        overflow_operator = overflow_by_chunk[
            chunk_index].reshape((operator_count,))
        edge_offsets = jnp.concatenate((
            jnp.zeros((1,), dtype=jnp.int32),
            jnp.cumsum(primary_counts[:-1], dtype=jnp.int32),
        ...
        task_counts = (
            primary_counts + jnp.int32(edge_block_size - 1)
        ) // jnp.int32(edge_block_size)
        task_offsets = jnp.concatenate((
            jnp.zeros((1,), dtype=jnp.int32),
            jnp.cumsum(task_counts[:-1], dtype=jnp.int32),
        ))
        total_tasks = task_counts.sum(dtype=jnp.int32)
        candidate_task_chunk = jnp.arange(
            max_tasks_per_operator, dtype=jnp.int32)
        primary_candidate_valid = (
            candidate_task_chunk[None, :]
            < task_counts[:, None])
        primary_destination = (
            task_offsets[:, None]
            + candidate_task_chunk[None, :])
        primary_destination = jnp.where(
            primary_candidate_valid,
            primary_destination,
            jnp.int32(combined_task_capacity))
        candidate_task_operator = jnp.broadcast_to(
            jnp.arange(operator_count, dtype=jnp.int32)[:, None],
            primary_destination.shape)
        overflow_rank = (
            jnp.cumsum(
                overflow_operator.astype(jnp.int32),
                dtype=jnp.int32)
            - overflow_operator.astype(jnp.int32))
        overflow_task_count = (
            overflow_operator.sum(dtype=jnp.int32)
            * jnp.int32(max_tasks_per_operator))
        overflow_destination = (
            total_tasks
            + overflow_rank[:, None]
            * jnp.int32(max_tasks_per_operator)
            + candidate_task_chunk[None, :])
        overflow_candidate_valid = jnp.broadcast_to(
            overflow_operator[:, None],
            overflow_destination.shape)
        overflow_destination = jnp.where(
            overflow_candidate_valid,
            overflow_destination,
            jnp.int32(combined_task_capacity))
        task_operator = jnp.full(
            (combined_task_capacity,),
            jnp.int32(operator_count),
            dtype=jnp.int32
        ).at[primary_destination].set(
            candidate_task_operator, mode="drop"
        ).at[overflow_destination].set(
            candidate_task_operator, mode="drop")
        task_chunk = jnp.zeros(
            (combined_task_capacity,), dtype=jnp.int32
        ).at[primary_destination].set(
            jnp.broadcast_to(
                candidate_task_chunk[None, :],
                primary_destination.shape),
            ...
            jnp.broadcast_to(
                ...
                overflow_destination.shape),
            mode="drop")
        task_is_overflow = jnp.zeros(
            (combined_task_capacity,), dtype=jnp.bool_
        ).at[overflow_destination].set(
            overflow_candidate_valid, mode="drop")
        combined_task_count = total_tasks + overflow_task_count
        combined_task_groups = (
            combined_task_count
            + jnp.int32(task_group_size - 1)
        ) // jnp.int32(task_group_size)

        def accumulate_operator_edges(
                gradients, group_operator, group_tokens, group_valid):
            local_operator = (
                group_operator % jnp.int32(chunk_size))
            operator_group = (
                group_operator // jnp.int32(chunk_size))
            space_ids = operator_group % jnp.int32(n_spaces)
            route_ids = operator_group // jnp.int32(n_spaces)
            global_operator = (
                chunk_index * jnp.int32(chunk_size)
                + local_operator)
            row_valid = global_operator < jnp.int32(n_local)
            safe_global_operator = jnp.minimum(
                global_operator, jnp.int32(n_padded - 1))
            safe_token = jnp.minimum(
                group_tokens, jnp.int32(token_capacity - 1))
            valid_edges = (
                group_valid
                & row_valid[:, None]
                & token_valid[space_ids[:, None], safe_token])

            edge_local = local_f32[
                space_ids[:, None], safe_token, :]
            edge_norm = local_norm[
                space_ids[:, None], safe_token, 0]
            edge_read = reads[
                route_ids, space_ids, safe_global_operator, :]
            edge_write = writes[
                route_ids, space_ids, safe_global_operator, :]
            edge_raw_tau = raw_tau[
                route_ids[:, None], space_ids[:, None],
                safe_token, 0]
            edge_raw_cotangent = raw_out_cotangent[
                route_ids[:, None], space_ids[:, None],
                safe_token, :]
            edge_gate_cotangent = gate_mass_cotangent[
                route_ids[:, None], space_ids[:, None],
                safe_token, 0]
            ...
            def edge_forward(
                    ...
                    exact_raw_tau,
                    exact_temperature,
                    exact_boundary_power,
                    exact_prune_eps):
                read_value = rw_dot(
                    "gcr,gr->gc",
                    exact_local, exact_read).astype(jnp.float32)
                exact_tau = _shared_tau_from_param(exact_raw_tau)
                rho = jnp.clip(
                    read_value / exact_norm, -1.0, 1.0)
                (_, gate, _, execution_weight,
                 _) = _shared_compute_admission_drive(
                     rho, exact_tau, exact_temperature,
                     boundary_power=exact_boundary_power,
                     effective_active_eps=active_eps,
                     execution_prune_eps=exact_prune_eps,
                     srw_composition_mode=srw_composition_mode,
                     heat_kernel_beta=beta)
                gate = jnp.where(valid_edges, gate, 0.0)
                execution_weight = jnp.where(
                    valid_edges, execution_weight, 0.0)
                edge_output = rw_dot(
                    "gc,gr->gcr",
                    execution_weight * read_value,
                    exact_write).astype(jnp.float32)
                return edge_output, gate

            _, edge_pullback = jax.vjp(
                edge_forward,
                edge_local,
                edge_norm,
                edge_read,
            ...
            (gradient_local,
             ...
             gradient_boundary_power,
             gradient_prune_eps) = gradients
            gradient_local = gradient_local.at[
                space_ids[:, None], safe_token, :
            ].add(edge_gradients[0], mode="drop")
            gradient_norm = gradient_norm.at[
                space_ids[:, None], safe_token, 0
            ].add(edge_gradients[1], mode="drop")
            gradient_read = gradient_read.at[
                route_ids, space_ids, safe_global_operator, :
            ].add(edge_gradients[2], mode="drop")
            gradient_write = gradient_write.at[
                route_ids, space_ids, safe_global_operator, :
            ].add(edge_gradients[3], mode="drop")
            gradient_raw_tau = gradient_raw_tau.at[
                route_ids[:, None], space_ids[:, None],
                safe_token, 0
            ].add(edge_gradients[4], mode="drop")
            return (
                gradient_local,
                gradient_norm,
                gradient_read,
                gradient_write,
                gradient_raw_tau,
                gradient_temperature + edge_gradients[5],
                gradient_boundary_power + edge_gradients[6],
                gradient_prune_eps + edge_gradients[7],
            )

        def task_loop_cond(loop_state):
            _, task_group_index = loop_state
            return task_group_index < combined_task_groups

        def task_loop_body(loop_state):
            gradients, task_group_index = loop_state
            start = task_group_index * task_group_size
            group_operator = jax.lax.dynamic_slice_in_dim(
                task_operator, start, task_group_size, axis=0)
            group_task_chunk = jax.lax.dynamic_slice_in_dim(
                task_chunk, start, task_group_size, axis=0)
            group_is_overflow = jax.lax.dynamic_slice_in_dim(
                task_is_overflow, start, task_group_size, axis=0)
            group_task_ids = (
                start
                + jnp.arange(task_group_size, dtype=jnp.int32))
            group_task_valid = (
                group_task_ids < combined_task_count)
            safe_group_operator = jnp.minimum(
                group_operator, jnp.int32(operator_count - 1))

            group_count = primary_counts[safe_group_operator]
            group_task_start = (
                edge_offsets[safe_group_operator]
                + group_task_chunk * jnp.int32(edge_block_size))
            positions = (
                group_task_start[:, None] + edge_slots[None, :])
            safe_positions = jnp.minimum(
                positions, jnp.int32(edge_capacity - 1))
            primary_tokens = packed_tokens[safe_positions]
            primary_valid = (
                group_task_valid[:, None]
                & ~group_is_overflow[:, None]
                & (group_task_chunk[:, None]
                   * jnp.int32(edge_block_size)
                   + edge_slots[None, :]
                   < group_count[:, None]))
            ...
            overflow_valid = (
                group_task_valid[:, None]
                & group_is_overflow[:, None]
                & (overflow_tokens < jnp.int32(token_capacity)))
            group_tokens = jnp.where(
                group_is_overflow[:, None],
                overflow_tokens,
                primary_tokens)
            group_valid = primary_valid | overflow_valid
            gradients = accumulate_operator_edges(
                gradients,
                safe_group_operator,
                group_tokens,
                group_valid)
            return (
                gradients,
                task_group_index + jnp.int32(1))

        gradients, _ = jax.lax.while_loop(
            task_loop_cond,
            task_loop_body,
            (carry_value, jnp.int32(0)))
        return gradients, None
    gradients, _ = jax.lax.scan(
        chunk_gradients,
        gradient_carry,
        jnp.arange(n_chunks))
    gradient_read = read_normalization_pullback(gradients[2])[0]
    gradient_write = write_normalization_pullback(gradients[3])[0]
    return (
        gradients[0].astype(local_f32.dtype),
        gradients[1].astype(local_norm.dtype),
        gradient_read,
        gradient_write,
        gradients[4].astype(raw_tau.dtype),
        gradients[5].astype(soft_gate_temperature.dtype),
        gradients[6].astype(soft_gate_boundary_power.dtype),
        gradients[7].astype(execution_prune_eps.dtype),
    )


def _active_edge_drive_pullback(
        read_value: jax.Array,
        exact_norm: jax.Array,
        exact_raw_tau: jax.Array,
        exact_temperature: jax.Array,
        exact_boundary_power: jax.Array,
        exact_prune_eps: jax.Array,
        mixed_cotangent: jax.Array,
        gate_cotangent: jax.Array,
        edge_valid: jax.Array, *,
        srw_composition_mode: str,
        heat_kernel_beta: float,
        effective_active_eps: float):
    """Differentiate only the scalar admission drive for one edge tile."""
    beta = jnp.float32(heat_kernel_beta)
    active_eps = jnp.float32(effective_active_eps)
    ...
    def drive_forward(
            ...
            raw_tau_value,
            temperature_value,
            boundary_power_value,
            prune_eps_value):
        exact_tau = _shared_tau_from_param(raw_tau_value)
        rho = jnp.clip(
            exact_read_value / local_norm_value, -1.0, 1.0)
        (selection_margin, gate, _,
         execution_weight, _) = _shared_compute_admission_drive(
             rho,
             exact_tau,
             temperature_value,
             boundary_power=boundary_power_value,
             effective_active_eps=active_eps,
             execution_prune_eps=prune_eps_value,
             srw_composition_mode=srw_composition_mode,
             heat_kernel_beta=beta)
        derivative_support = (
            edge_valid & (selection_margin >= jnp.float32(0.0)))
        mixed = jnp.where(
            derivative_support,
            execution_weight * exact_read_value,
            jnp.float32(0.0))
        exact_gate = jnp.where(
            derivative_support, gate, jnp.float32(0.0))
        return mixed, exact_gate

    (mixed, _), pullback = jax.vjp(
        drive_forward,
        read_value,
        exact_norm,
        exact_raw_tau,
        ...
        exact_boundary_power,
        exact_prune_eps)
    gradients = pullback((mixed_cotangent, gate_cotangent))
...
def _pallas_row_dot(
    ...
    """Row-wise dot with the production BF16/FP32 arithmetic contract."""
    if throughput_bf16:
        return jnp.sum(
...
def _pallas_reduce_rows(
        weights: jax.Array,
        rows: jax.Array,
        throughput_bf16: bool) -> jax.Array:
    """Reduce edge rows before any owner scatter."""
    if (
            throughput_bf16
            and int(rows.shape[0]) >= 128
            and int(rows.shape[0]) % 128 == 0):
        return pl.dot(
            weights.astype(jnp.bfloat16)[None, :],
            rows.astype(jnp.bfloat16)
        ).astype(jnp.float32).reshape((rows.shape[-1],))
    return jnp.sum(
...
def _active_edge_sparse_rw_gradients_sharded(
        local_f32: jax.Array,
        local_norm: jax.Array,
        token_valid: jax.Array,
        read_vectors: jax.Array,
        ...
        throughput_bf16: bool,
        active_edge_backward_profile: str):
    """Run two collision-free owner paths with task-parallel Pallas tiles."""
    n_routes, n_spaces, token_capacity, d_route = map(
        int, (read_vectors.shape[0], local_f32.shape[0],
              local_f32.shape[1], local_f32.shape[2]))
    n_local = int(read_vectors.shape[2])
    ...
    edge_capacity = int(layout["edge_capacity"])
    operator_count = int(layout["operator_count"])
    token_owner_count = int(layout["token_owner_count"])
    operator_edge_block_size = int(
        layout["operator_edge_block_size"])
    token_edge_block_size = int(layout["token_edge_block_size"])
    operator_task_capacity = int(layout["operator_task_capacity"])
    token_task_capacity = int(layout["token_task_capacity"])
    pallas_interpret = jax.default_backend() != "tpu"

    operator_metadata_by_chunk, token_metadata_by_chunk = packed_metadata
    operator_packed_tokens_by_chunk = operator_metadata_by_chunk[0]
    operator_primary_counts_by_chunk = operator_metadata_by_chunk[1]
    operator_overflow_by_chunk = operator_metadata_by_chunk[2]
    token_packed_edges_by_chunk = token_metadata_by_chunk[0]
    token_primary_counts_by_chunk = token_metadata_by_chunk[1]
    token_overflow_by_chunk = token_metadata_by_chunk[2]

    def operator_owner_gradients(chunk_index):
        packed_tokens = operator_packed_tokens_by_chunk[chunk_index]
        primary_counts = operator_primary_counts_by_chunk[
            chunk_index].reshape((operator_count,))
        overflow_owner = operator_overflow_by_chunk[
            chunk_index].reshape((operator_count,))
        (task_owner, task_block, total_tasks,
         edge_offsets) = _build_primary_active_edge_tasks(
             primary_counts,
             edge_block_size=operator_edge_block_size,
             task_capacity=operator_task_capacity,
             max_edges_per_owner=token_capacity)
        def primary_kernel(
                local_ref, norm_ref, token_valid_ref,
                reads_ref, writes_ref, raw_tau_ref,
                temperature_ref, boundary_power_ref, prune_eps_ref,
                raw_cotangent_ref, gate_cotangent_ref,
                packed_tokens_ref, counts_ref, offsets_ref,
                task_owner_ref, task_block_ref, total_tasks_ref,
                chunk_index_ref,
                task_read_ref, task_write_ref,
                task_temperature_ref, task_boundary_ref,
                task_prune_ref):
            task_id = pl.program_id(0)
            d_offsets = jnp.arange(d_route, dtype=jnp.int32)
            edge_slots = jnp.arange(
                operator_edge_block_size, dtype=jnp.int32)
            owner = pl.load(task_owner_ref, (task_id,))
            task_is_live = (
                task_id < pl.load(total_tasks_ref, ())
            ) & (owner < jnp.int32(operator_count))
            safe_owner = jnp.minimum(
                owner, jnp.int32(operator_count - 1))
            block_index = pl.load(task_block_ref, (task_id,))
            count = pl.load(counts_ref, (safe_owner,))
            offset = pl.load(offsets_ref, (safe_owner,))
            positions = (
                offset + block_index * operator_edge_block_size
                + edge_slots)
            safe_positions = jnp.minimum(
                positions, jnp.int32(edge_capacity - 1))
            tokens = pl.load(
                packed_tokens_ref, (safe_positions,),
                mask=task_is_live, other=0)
            local_operator = (
                safe_owner % jnp.int32(chunk_size))
            owner_group = safe_owner // jnp.int32(chunk_size)
            space_id = owner_group % jnp.int32(n_spaces)
            route_id = owner_group // jnp.int32(n_spaces)
            global_operator = (
                pl.load(chunk_index_ref, ()) * jnp.int32(chunk_size)
                + local_operator)
            row_valid = global_operator < jnp.int32(n_local)
            edge_valid = (
                task_is_live
                & row_valid
                & (block_index * jnp.int32(operator_edge_block_size)
                   + edge_slots < count))
            safe_tokens = jnp.minimum(
                tokens, jnp.int32(token_capacity - 1))
            edge_valid = edge_valid & pl.load(
                token_valid_ref, (space_id, safe_tokens),
                mask=edge_valid, other=False)
            local_rows = pl.load(
                local_ref,
                (space_id, safe_tokens[:, None], d_offsets[None, :]),
                mask=edge_valid[:, None], other=0.0)
            exact_norm = pl.load(
                norm_ref, (space_id, safe_tokens, jnp.int32(0)),
                mask=edge_valid, other=1.0)
            read_row = pl.load(
                reads_ref,
                (route_id, space_id, global_operator, d_offsets),
                mask=row_valid, other=0.0)
            write_row = pl.load(
                writes_ref,
                (route_id, space_id, global_operator, d_offsets),
                mask=row_valid, other=0.0)
            exact_raw_tau = pl.load(
                raw_tau_ref,
                (route_id, space_id, safe_tokens, jnp.int32(0)),
                mask=edge_valid, other=0.0)
            raw_cotangent = pl.load(
                raw_cotangent_ref,
                (route_id, space_id, safe_tokens[:, None],
                 d_offsets[None, :]),
                mask=edge_valid[:, None], other=0.0)
            gate_cotangent = pl.load(
                gate_cotangent_ref,
                (route_id, space_id, safe_tokens, jnp.int32(0)),
                mask=edge_valid, other=0.0)
            exact_temperature = pl.load(temperature_ref, ())
            exact_boundary = pl.load(boundary_power_ref, ())
            exact_prune = pl.load(prune_eps_ref, ())
            read_value = _pallas_row_dot(
                local_rows,
                jnp.broadcast_to(read_row[None, :], local_rows.shape),
                throughput_bf16)
            mixed_cotangent = _pallas_row_dot(
                raw_cotangent,
                jnp.broadcast_to(write_row[None, :],
                                 raw_cotangent.shape),
                throughput_bf16)
            mixed, drive_gradients = _active_edge_drive_pullback(
                read_value,
                exact_norm,
                exact_raw_tau,
                exact_temperature,
                ...
                heat_kernel_beta=heat_kernel_beta,
                effective_active_eps=effective_active_eps)
            task_read = _pallas_reduce_rows(
                drive_gradients[0], local_rows, throughput_bf16)
            task_write = _pallas_reduce_rows(
                mixed, raw_cotangent, throughput_bf16)
            pl.store(
                task_read_ref, (task_id, d_offsets), task_read)
            pl.store(
                task_write_ref, (task_id, d_offsets), task_write)
            pl.store(
                task_temperature_ref, (task_id,),
                drive_gradients[3])
            pl.store(
                task_boundary_ref, (task_id,), drive_gradients[4])
            pl.store(
                task_prune_ref, (task_id,), drive_gradients[5])

        primary_out_shape = (
            jax.ShapeDtypeStruct(
                (operator_task_capacity, d_route), jnp.float32),
            jax.ShapeDtypeStruct(
                (operator_task_capacity, d_route), jnp.float32),
            jax.ShapeDtypeStruct(
                (operator_task_capacity,), jnp.float32),
            jax.ShapeDtypeStruct(
                (operator_task_capacity,), jnp.float32),
            jax.ShapeDtypeStruct(
                (operator_task_capacity,), jnp.float32),
        )
        primary_read, primary_write, primary_t, primary_b, primary_p = (
            pl.pallas_call(
                primary_kernel,
                out_shape=primary_out_shape,
                grid=(operator_task_capacity,),
                compiler_params=pltpu.CompilerParams(
                    dimension_semantics=("arbitrary",)),
                interpret=pallas_interpret,
                name="v4174_active_edge_operator_primary")(
                    local_f32, local_norm, token_valid,
                    reads, writes, raw_tau,
                    soft_gate_temperature,
                    soft_gate_boundary_power,
                    execution_prune_eps,
                    raw_out_cotangent, gate_mass_cotangent,
                    packed_tokens, primary_counts, edge_offsets,
                    task_owner, task_block, total_tasks,
                    jnp.asarray(chunk_index, dtype=jnp.int32)))
        primary_read = jax.ops.segment_sum(
            primary_read,
            task_owner,
            num_segments=operator_count,
            mode="drop")
        primary_write = jax.ops.segment_sum(
            primary_write,
            task_owner,
            num_segments=operator_count,
            mode="drop")

        overflow_blocks = math.ceil(
            token_capacity / operator_edge_block_size)
        ...
        def overflow_kernel(
                ...
                temperature_ref, boundary_power_ref, prune_eps_ref,
                raw_cotangent_ref, gate_cotangent_ref,
                overflow_ref, chunk_index_ref,
                owner_read_ref, owner_write_ref,
                owner_temperature_ref, owner_boundary_ref,
                owner_prune_ref):
            owner = pl.program_id(0)
            d_offsets = jnp.arange(d_route, dtype=jnp.int32)
            edge_slots = jnp.arange(
                operator_edge_block_size, dtype=jnp.int32)
            local_operator = owner % jnp.int32(chunk_size)
            owner_group = owner // jnp.int32(chunk_size)
            space_id = owner_group % jnp.int32(n_spaces)
            route_id = owner_group // jnp.int32(n_spaces)
            global_operator = (
                pl.load(chunk_index_ref, ()) * jnp.int32(chunk_size)
                + local_operator)
            owner_is_live = (
                pl.load(overflow_ref, (owner,))
                & (global_operator < jnp.int32(n_local)))
            read_row = pl.load(
                reads_ref,
                (route_id, space_id, global_operator, d_offsets),
                mask=owner_is_live, other=0.0)
            write_row = pl.load(
                writes_ref,
                (route_id, space_id, global_operator, d_offsets),
                mask=owner_is_live, other=0.0)
            ...
            exact_prune = pl.load(prune_eps_ref, ())
            init = (
                jnp.zeros((d_route,), dtype=jnp.float32),
                jnp.zeros((d_route,), dtype=jnp.float32),
                jnp.float32(0.0),
                jnp.float32(0.0),
                jnp.float32(0.0),
            ...
            def block_body(block_index, carry):
                tokens = (
                    block_index * operator_edge_block_size
                    + edge_slots)
                edge_valid = (
                    owner_is_live
                    & (tokens < jnp.int32(token_capacity)))
                safe_tokens = jnp.minimum(
                    tokens, jnp.int32(token_capacity - 1))
                edge_valid = edge_valid & pl.load(
                    token_valid_ref, (space_id, safe_tokens),
                ...
                local_rows = pl.load(
                    local_ref,
                    (space_id, safe_tokens[:, None],
                     d_offsets[None, :]),
                    mask=edge_valid[:, None], other=0.0)
                exact_norm = pl.load(
                    norm_ref,
                    (space_id, safe_tokens, jnp.int32(0)),
                    mask=edge_valid, other=1.0)
                exact_raw_tau = pl.load(
                    raw_tau_ref,
                    (route_id, space_id, safe_tokens, jnp.int32(0)),
                ...
                read_value = _pallas_row_dot(
                    local_rows,
                    jnp.broadcast_to(
                        read_row[None, :], local_rows.shape),
                    throughput_bf16)
                mixed_cotangent = _pallas_row_dot(
                    raw_cotangent,
                    jnp.broadcast_to(
                        write_row[None, :], raw_cotangent.shape),
                    throughput_bf16)
                mixed, drive_gradients = _active_edge_drive_pullback(
                    read_value,
                    exact_norm,
                    ...
                    effective_active_eps=effective_active_eps)
                return (
                    carry[0] + _pallas_reduce_rows(
                        drive_gradients[0], local_rows,
                        throughput_bf16),
                    carry[1] + _pallas_reduce_rows(
                        mixed, raw_cotangent, throughput_bf16),
                    carry[2] + drive_gradients[3],
                    carry[3] + drive_gradients[4],
                    carry[4] + drive_gradients[5],
                )

            result = jax.lax.fori_loop(
                0, overflow_blocks, block_body, init)
            pl.store(owner_read_ref, (owner, d_offsets), result[0])
            pl.store(owner_write_ref, (owner, d_offsets), result[1])
            pl.store(owner_temperature_ref, (owner,), result[2])
            pl.store(owner_boundary_ref, (owner,), result[3])
            pl.store(owner_prune_ref, (owner,), result[4])

        overflow_out_shape = (
            jax.ShapeDtypeStruct(
                (operator_count, d_route), jnp.float32),
            jax.ShapeDtypeStruct(
                (operator_count, d_route), jnp.float32),
            jax.ShapeDtypeStruct((operator_count,), jnp.float32),
            jax.ShapeDtypeStruct((operator_count,), jnp.float32),
            jax.ShapeDtypeStruct((operator_count,), jnp.float32),
        )
        overflow_read, overflow_write, overflow_t, overflow_b, overflow_p = (
            pl.pallas_call(
                overflow_kernel,
                out_shape=overflow_out_shape,
                grid=(operator_count,),
                compiler_params=pltpu.CompilerParams(
                    dimension_semantics=("arbitrary",)),
                interpret=pallas_interpret,
                name="v4174_active_edge_operator_overflow")(
                    local_f32, local_norm, token_valid,
                    reads, writes, raw_tau,
                    soft_gate_temperature,
                    soft_gate_boundary_power,
                    ...
                    jnp.asarray(chunk_index, dtype=jnp.int32)))
        return (
            (primary_read + overflow_read).reshape(
                (n_routes, n_spaces, chunk_size, d_route)),
            (primary_write + overflow_write).reshape(
                (n_routes, n_spaces, chunk_size, d_route)),
            primary_t.sum(dtype=jnp.float32)
            + overflow_t.sum(dtype=jnp.float32),
            primary_b.sum(dtype=jnp.float32)
            + overflow_b.sum(dtype=jnp.float32),
            primary_p.sum(dtype=jnp.float32)
            + overflow_p.sum(dtype=jnp.float32),
        )

    def token_owner_gradients(chunk_index):
        packed_edges = token_packed_edges_by_chunk[chunk_index]
        primary_counts = token_primary_counts_by_chunk[
            chunk_index].reshape((token_owner_count,))
        overflow_owner = token_overflow_by_chunk[
            chunk_index].reshape((token_owner_count,))
        (task_owner, task_block, total_tasks,
         edge_offsets) = _build_primary_active_edge_tasks(
             primary_counts,
             edge_block_size=token_edge_block_size,
             task_capacity=token_task_capacity,
             max_edges_per_owner=n_routes * chunk_size)
        def primary_kernel(
                local_ref, norm_ref, token_valid_ref,
                reads_ref, writes_ref, raw_tau_ref,
                temperature_ref, boundary_power_ref, prune_eps_ref,
                raw_cotangent_ref, gate_cotangent_ref,
                packed_edges_ref, counts_ref, offsets_ref,
                task_owner_ref, task_block_ref, total_tasks_ref,
                chunk_index_ref,
                task_local_ref, task_norm_ref, task_tau_ref):
            task_id = pl.program_id(0)
            d_offsets = jnp.arange(d_route, dtype=jnp.int32)
            edge_slots = jnp.arange(
                token_edge_block_size, dtype=jnp.int32)
            owner = pl.load(task_owner_ref, (task_id,))
            task_is_live = (
                task_id < pl.load(total_tasks_ref, ())
            ) & (owner < jnp.int32(token_owner_count))
            safe_owner = jnp.minimum(
                owner, jnp.int32(token_owner_count - 1))
            block_index = pl.load(task_block_ref, (task_id,))
            count = pl.load(counts_ref, (safe_owner,))
            offset = pl.load(offsets_ref, (safe_owner,))
            positions = (
                offset + block_index * token_edge_block_size
                + edge_slots)
            safe_positions = jnp.minimum(
                positions, jnp.int32(edge_capacity - 1))
            packed_edge = pl.load(
                packed_edges_ref, (safe_positions,),
                mask=task_is_live, other=0)
            space_id = safe_owner // jnp.int32(token_capacity)
            token_id = safe_owner % jnp.int32(token_capacity)
            route_id = (
                packed_edge // jnp.int32(chunk_size))
            local_operator = (
                packed_edge % jnp.int32(chunk_size))
            global_operator = (
                pl.load(chunk_index_ref, ()) * jnp.int32(chunk_size)
            ...
            edge_valid = (
                task_is_live
                & (block_index * jnp.int32(token_edge_block_size)
                   + edge_slots < count)
                & (global_operator < jnp.int32(n_local)))
            edge_valid = edge_valid & pl.load(
                token_valid_ref, (space_id, token_id),
                mask=task_is_live, other=False)
            local_row = pl.load(
                local_ref, (space_id, token_id, d_offsets),
                mask=task_is_live, other=0.0)
            exact_norm_scalar = pl.load(
                norm_ref, (space_id, token_id, jnp.int32(0)),
                mask=task_is_live, other=1.0)
            read_rows = pl.load(
                reads_ref,
                (route_id[:, None], space_id,
                 global_operator[:, None], d_offsets[None, :]),
                mask=edge_valid[:, None], other=0.0)
            write_rows = pl.load(
                writes_ref,
                (route_id[:, None], space_id,
            ...
            exact_raw_tau = pl.load(
                raw_tau_ref,
                (route_id, space_id, token_id, jnp.int32(0)),
                mask=edge_valid, other=0.0)
            raw_cotangent = pl.load(
                raw_cotangent_ref,
                (route_id[:, None], space_id, token_id,
                 d_offsets[None, :]),
                mask=edge_valid[:, None], other=0.0)
            gate_cotangent = pl.load(
                gate_cotangent_ref,
                (route_id, space_id, token_id, jnp.int32(0)),
                mask=edge_valid, other=0.0)
            exact_norm = jnp.broadcast_to(
                exact_norm_scalar, (token_edge_block_size,))
            exact_temperature = pl.load(temperature_ref, ())
            exact_boundary = pl.load(boundary_power_ref, ())
            exact_prune = pl.load(prune_eps_ref, ())
            read_value = _pallas_row_dot(
                jnp.broadcast_to(
                    local_row[None, :], read_rows.shape),
                read_rows,
                throughput_bf16)
            mixed_cotangent = _pallas_row_dot(
                raw_cotangent, write_rows, throughput_bf16)
            _, drive_gradients = _active_edge_drive_pullback(
                read_value,
                exact_norm,
                exact_raw_tau,
                exact_temperature,
                ...
                heat_kernel_beta=heat_kernel_beta,
                effective_active_eps=effective_active_eps)
            task_local = _pallas_reduce_rows(
                drive_gradients[0], read_rows, throughput_bf16)
            task_norm = drive_gradients[1].sum(dtype=jnp.float32)
            task_tau = jnp.stack(tuple(
                jnp.where(
                    route_id == jnp.int32(route_index),
                    drive_gradients[2],
                    jnp.float32(0.0)
                ).sum(dtype=jnp.float32)
                for route_index in range(n_routes)))
            pl.store(
                task_local_ref, (task_id, d_offsets), task_local)
            pl.store(task_norm_ref, (task_id,), task_norm)
            pl.store(
                task_tau_ref,
                (task_id, jnp.arange(n_routes, dtype=jnp.int32)),
                task_tau)

        primary_out_shape = (
            jax.ShapeDtypeStruct(
                (token_task_capacity, d_route), jnp.float32),
            jax.ShapeDtypeStruct(
                (token_task_capacity,), jnp.float32),
            jax.ShapeDtypeStruct(
                (token_task_capacity, n_routes), jnp.float32),
        )
        primary_local, primary_norm, primary_tau = pl.pallas_call(
            primary_kernel,
            out_shape=primary_out_shape,
            grid=(token_task_capacity,),
            compiler_params=pltpu.CompilerParams(
                dimension_semantics=("arbitrary",)),
            interpret=pallas_interpret,
            name="v4174_active_edge_token_primary")(
                local_f32, local_norm, token_valid,
                reads, writes, raw_tau,
                soft_gate_temperature,
                soft_gate_boundary_power,
                execution_prune_eps,
                raw_out_cotangent, gate_mass_cotangent,
                packed_edges, primary_counts, edge_offsets,
                task_owner, task_block, total_tasks,
                jnp.asarray(chunk_index, dtype=jnp.int32))
        primary_local = jax.ops.segment_sum(
            primary_local,
            task_owner,
            num_segments=token_owner_count,
            mode="drop")
        primary_norm = jax.ops.segment_sum(
            primary_norm,
            task_owner,
            num_segments=token_owner_count,
            mode="drop")
        primary_tau = jax.ops.segment_sum(
            primary_tau,
            task_owner,
            num_segments=token_owner_count,
            mode="drop")
        ...
        edges_per_owner = n_routes * chunk_size
        overflow_blocks = math.ceil(
            edges_per_owner / token_edge_block_size)
        ...
        def overflow_kernel(
                ...
                raw_cotangent_ref, gate_cotangent_ref,
                overflow_ref, chunk_index_ref,
                owner_local_ref, owner_norm_ref, owner_tau_ref):
            owner = pl.program_id(0)
            d_offsets = jnp.arange(d_route, dtype=jnp.int32)
            edge_slots = jnp.arange(
                token_edge_block_size, dtype=jnp.int32)
            space_id = owner // jnp.int32(token_capacity)
            token_id = owner % jnp.int32(token_capacity)
            owner_is_live = (
                pl.load(overflow_ref, (owner,))
                & pl.load(
                    token_valid_ref, (space_id, token_id),
            ...
            init = (
                jnp.zeros((d_route,), dtype=jnp.float32),
                jnp.float32(0.0),
                jnp.zeros((n_routes,), dtype=jnp.float32),
            )

            def block_body(block_index, carry):
                packed_edge = (
                    block_index * token_edge_block_size
                    + edge_slots)
                route_id = packed_edge // jnp.int32(chunk_size)
                local_operator = packed_edge % jnp.int32(chunk_size)
                global_operator = (
                    pl.load(chunk_index_ref, ())
                    * jnp.int32(chunk_size)
                    + local_operator)
                edge_valid = (
                    owner_is_live
                    & (packed_edge < jnp.int32(edges_per_owner))
                    & (global_operator < jnp.int32(n_local)))
                read_rows = pl.load(
                    reads_ref,
                    (route_id[:, None], space_id,
                ...
                return (
                    carry[0] + _pallas_reduce_rows(
                        drive_gradients[0], read_rows,
                        throughput_bf16),
                    carry[1] + drive_gradients[1].sum(
                        dtype=jnp.float32),
                    carry[2] + block_tau,
                )
            ...
            result = jax.lax.fori_loop(
                0, overflow_blocks, block_body, init)
            pl.store(owner_local_ref, (owner, d_offsets), result[0])
            pl.store(owner_norm_ref, (owner,), result[1])
            pl.store(
                owner_tau_ref,
                (owner, jnp.arange(n_routes, dtype=jnp.int32)),
                result[2])

        overflow_out_shape = (
            jax.ShapeDtypeStruct(
                (token_owner_count, d_route), jnp.float32),
            jax.ShapeDtypeStruct(
                (token_owner_count,), jnp.float32),
            jax.ShapeDtypeStruct(
                (token_owner_count, n_routes), jnp.float32),
        )
        overflow_local, overflow_norm, overflow_tau = pl.pallas_call(
            overflow_kernel,
            out_shape=overflow_out_shape,
            grid=(token_owner_count,),
            compiler_params=pltpu.CompilerParams(
                dimension_semantics=("arbitrary",)),
            interpret=pallas_interpret,
            name="v4174_active_edge_token_overflow")(
                local_f32, local_norm, token_valid,
                reads, writes, raw_tau,
                soft_gate_temperature,
                soft_gate_boundary_power,
                ...
                overflow_owner,
                jnp.asarray(chunk_index, dtype=jnp.int32))
        token_local = (primary_local + overflow_local).reshape(
            (n_spaces, token_capacity, d_route))
        token_norm = (primary_norm + overflow_norm).reshape(
            (n_spaces, token_capacity, 1))
        token_tau = (primary_tau + overflow_tau).reshape(
            (n_spaces, token_capacity, n_routes))
        token_tau = jnp.transpose(
            token_tau, (2, 0, 1))[..., None]
        return token_local, token_norm, token_tau
    ...
    def chunk_gradients(carry, chunk_index):
        local_gradient, norm_gradient, tau_gradient, grad_t, grad_b, grad_p = (
            carry)
        read_gradient, write_gradient, chunk_t, chunk_b, chunk_p = (
            operator_owner_gradients(chunk_index))
        chunk_local, chunk_norm, chunk_tau = token_owner_gradients(
            chunk_index)
        return (
            local_gradient + chunk_local,
            norm_gradient + chunk_norm,
            tau_gradient + chunk_tau,
            grad_t + chunk_t,
            grad_b + chunk_b,
            grad_p + chunk_p,
        ), (read_gradient, write_gradient)

    gradients, operator_gradients = jax.lax.scan(
        chunk_gradients,
        local_carry,
        jnp.arange(n_chunks, dtype=jnp.int32))
    read_gradient = jnp.transpose(
        operator_gradients[0], (1, 2, 0, 3, 4)).reshape(
            (n_routes, n_spaces, n_padded, d_route))
    write_gradient = jnp.transpose(
        operator_gradients[1], (1, 2, 0, 3, 4)).reshape(
            (n_routes, n_spaces, n_padded, d_route))
    read_gradient = read_normalization_pullback(read_gradient)[0]
    write_gradient = write_normalization_pullback(write_gradient)[0]
    return (
        gradients[0].astype(local_f32.dtype),
        gradients[1].astype(local_norm.dtype),
        read_gradient,
        write_gradient,
        gradients[2].astype(raw_tau.dtype),
        gradients[3].astype(soft_gate_temperature.dtype),
        gradients[4].astype(soft_gate_boundary_power.dtype),
        gradients[5].astype(execution_prune_eps.dtype),
    )


def _xla_owner_row_dot(
        lhs: jax.Array,
        rhs: jax.Array,
        throughput_bf16: bool) -> jax.Array:
    if throughput_bf16:
...
def _xla_owner_reduce_rows(
        weights: jax.Array,
        rows: jax.Array,
        throughput_bf16: bool) -> jax.Array:
    if throughput_bf16:
        return _throughput_einsum_bf16_f32(
            "gb,gbr->gr", weights, rows)
    return _control_einsum_f32("gb,gbr->gr", weights, rows)


def _active_edge_owner_batched_rw_gradients_sharded(
        local_f32: jax.Array,
        local_norm: jax.Array,
        token_valid: jax.Array,
        read_vectors: jax.Array,
        ...
        throughput_bf16: bool,
        active_edge_backward_profile: str):
    """Run compact owner tasks without random full-gradient loop carries."""
    n_routes, n_spaces, token_capacity, d_route = map(
        int, (read_vectors.shape[0], local_f32.shape[0],
              local_f32.shape[1], local_f32.shape[2]))
    n_local = int(read_vectors.shape[2])
    ...
    operator_edge_block_size = int(
        layout["operator_edge_block_size"])
    token_edge_block_size = int(layout["token_edge_block_size"])
    operator_task_group_size = int(
        layout["operator_task_group_size"])
    token_task_group_size = int(layout["token_task_group_size"])
    operator_task_capacity = int(layout["operator_task_capacity"])
    token_task_capacity = int(layout["token_task_capacity"])
    ...
    def compact_owner_ids(overflow_owner):
        owner_count = int(overflow_owner.shape[0])
        owner_rank = (
            jnp.cumsum(
                overflow_owner.astype(jnp.int32), dtype=jnp.int32)
            - overflow_owner.astype(jnp.int32))
        destination = jnp.where(
            overflow_owner, owner_rank, jnp.int32(owner_count))
        owner_ids = jnp.full(
            (owner_count,), jnp.int32(owner_count), dtype=jnp.int32
        ).at[destination].set(
            jnp.arange(owner_count, dtype=jnp.int32), mode="drop")
        return owner_ids, overflow_owner.sum(dtype=jnp.int32)

    def operator_edge_group(
            group_owner, group_tokens, group_valid, chunk_index):
        safe_owner = jnp.minimum(
            group_owner, jnp.int32(operator_count - 1))
        local_operator = safe_owner % jnp.int32(chunk_size)
        owner_group = safe_owner // jnp.int32(chunk_size)
        space_id = owner_group % jnp.int32(n_spaces)
        route_id = owner_group // jnp.int32(n_spaces)
        global_operator = (
            chunk_index * jnp.int32(chunk_size) + local_operator)
        safe_tokens = jnp.minimum(
            group_tokens, jnp.int32(token_capacity - 1))
        edge_valid = (
            group_valid
            & (global_operator[:, None] < jnp.int32(n_local))
            & token_valid[space_id[:, None], safe_tokens])
        edge_local = local_f32[
            space_id[:, None], safe_tokens, :]
        edge_norm = local_norm[
            space_id[:, None], safe_tokens, 0]
        edge_read = reads[
            route_id, space_id, global_operator, :]
        edge_write = writes[
            route_id, space_id, global_operator, :]
        edge_raw_tau = raw_tau[
            route_id[:, None], space_id[:, None], safe_tokens, 0]
        edge_raw_cotangent = raw_out_cotangent[
            route_id[:, None], space_id[:, None], safe_tokens, :]
        edge_gate_cotangent = gate_mass_cotangent[
            route_id[:, None], space_id[:, None], safe_tokens, 0]
        read_value = _xla_owner_row_dot(
            edge_local,
            jnp.broadcast_to(edge_read[:, None, :], edge_local.shape),
            throughput_bf16)
        mixed_cotangent = _xla_owner_row_dot(
            edge_raw_cotangent,
            jnp.broadcast_to(
                edge_write[:, None, :], edge_raw_cotangent.shape),
            throughput_bf16)
        mixed, drive_gradients = _active_edge_drive_pullback(
            read_value,
            edge_norm,
            ...
            effective_active_eps=effective_active_eps)
        return (
            _xla_owner_reduce_rows(
                drive_gradients[0], edge_local, throughput_bf16),
            _xla_owner_reduce_rows(
                mixed, edge_raw_cotangent, throughput_bf16),
            drive_gradients[3],
            drive_gradients[4],
            drive_gradients[5],
        )
    ...
    def operator_owner_gradients(chunk_index):
        ...
        edge_slots = jnp.arange(
            operator_edge_block_size, dtype=jnp.int32)
        task_read_init = jnp.zeros(
            (operator_task_capacity, d_route), dtype=jnp.float32)
        task_write_init = jnp.zeros_like(task_read_init)

        def primary_cond(state):
            _, _, _, _, _, start = state
            return start < total_tasks

        def primary_body(state):
            task_read, task_write, grad_t, grad_b, grad_p, start = state
            group_owner = jax.lax.dynamic_slice_in_dim(
                task_owner, start, operator_task_group_size)
            group_block = jax.lax.dynamic_slice_in_dim(
                task_block, start, operator_task_group_size)
            group_ids = (
                start + jnp.arange(
                    operator_task_group_size, dtype=jnp.int32))
            safe_owner = jnp.minimum(
                group_owner, jnp.int32(operator_count - 1))
            count = primary_counts[safe_owner]
            positions = (
                edge_offsets[safe_owner, None]
                + group_block[:, None] * operator_edge_block_size
                + edge_slots[None, :])
            safe_positions = jnp.minimum(
                positions, jnp.int32(edge_capacity - 1))
            group_tokens = packed_tokens[safe_positions]
            group_valid = (
                (group_ids[:, None] < total_tasks)
                & (group_block[:, None] * operator_edge_block_size
                   + edge_slots[None, :] < count[:, None]))
            group_gradients = operator_edge_group(
                safe_owner, group_tokens, group_valid, chunk_index)
            task_read = jax.lax.dynamic_update_slice_in_dim(
                task_read, group_gradients[0], start, axis=0)
            task_write = jax.lax.dynamic_update_slice_in_dim(
                task_write, group_gradients[1], start, axis=0)
            return (
                task_read,
                task_write,
                grad_t + group_gradients[2],
                grad_b + group_gradients[3],
                grad_p + group_gradients[4],
                start + jnp.int32(operator_task_group_size),
            )

        task_read, task_write, grad_t, grad_b, grad_p, _ = (
            jax.lax.while_loop(
                primary_cond,
                primary_body,
                (task_read_init, task_write_init,
                 jnp.float32(0.0), jnp.float32(0.0),
                 jnp.float32(0.0), jnp.int32(0))))
        primary_read = jax.ops.segment_sum(
            task_read, task_owner,
            num_segments=operator_count, mode="drop")
        primary_write = jax.ops.segment_sum(
            task_write, task_owner,
            num_segments=operator_count, mode="drop")

        overflow_ids, overflow_count = compact_owner_ids(
            overflow_owner)
        overflow_read_init = jnp.zeros(
            (operator_count, d_route), dtype=jnp.float32)
        overflow_write_init = jnp.zeros_like(overflow_read_init)
        overflow_blocks = math.ceil(
            token_capacity / operator_edge_block_size)

        def overflow_cond(state):
            _, _, _, _, _, start = state
            return start < overflow_count

        def overflow_body(state):
            owner_read, owner_write, out_t, out_b, out_p, start = state
            group_owner = jax.lax.dynamic_slice_in_dim(
                overflow_ids, start, operator_task_group_size)
            group_ids = (
                start + jnp.arange(
                    operator_task_group_size, dtype=jnp.int32))
            group_live = group_ids < overflow_count
            safe_owner = jnp.minimum(
                group_owner, jnp.int32(operator_count - 1))
            block_init = (
                jnp.zeros(
                    (operator_task_group_size, d_route),
                    dtype=jnp.float32),
                jnp.zeros(
                    (operator_task_group_size, d_route),
                    dtype=jnp.float32),
            ...
            def overflow_block(block_index, block_carry):
                group_tokens = (
                    block_index * operator_edge_block_size
                    + edge_slots[None, :])
                group_tokens = jnp.broadcast_to(
                    group_tokens,
                    (operator_task_group_size,
                     operator_edge_block_size))
                group_valid = (
                    group_live[:, None]
                    & (group_tokens < jnp.int32(token_capacity)))
                block_gradients = operator_edge_group(
                    safe_owner,
                    group_tokens,
                    group_valid,
                    chunk_index)
                return tuple(
                    ...
                    for left, right in zip(
                        block_carry, block_gradients))

            block_gradients = jax.lax.fori_loop(
                0, overflow_blocks, overflow_block, block_init)
            owner_read = owner_read.at[safe_owner].add(
                jnp.where(
                    group_live[:, None],
                    block_gradients[0], 0.0),
                mode="drop")
            owner_write = owner_write.at[safe_owner].add(
                jnp.where(
                    group_live[:, None],
                    block_gradients[1], 0.0),
                mode="drop")
            return (
                owner_read,
                owner_write,
                out_t + block_gradients[2],
                out_b + block_gradients[3],
                out_p + block_gradients[4],
                start + jnp.int32(operator_task_group_size),
            )

        overflow_read, overflow_write, out_t, out_b, out_p, _ = (
            jax.lax.while_loop(
                overflow_cond,
                overflow_body,
                (overflow_read_init, overflow_write_init,
                 jnp.float32(0.0), jnp.float32(0.0),
                 jnp.float32(0.0), jnp.int32(0))))
        return (
            (primary_read + overflow_read).reshape(
    ...
    def token_edge_group(
            group_owner, group_edges, group_valid, chunk_index):
        safe_owner = jnp.minimum(
            group_owner, jnp.int32(token_owner_count - 1))
        space_id = safe_owner // jnp.int32(token_capacity)
        token_id = safe_owner % jnp.int32(token_capacity)
        route_id = group_edges // jnp.int32(chunk_size)
        local_operator = group_edges % jnp.int32(chunk_size)
        global_operator = (
            chunk_index * jnp.int32(chunk_size) + local_operator)
        edge_valid = (
            group_valid
            & (global_operator < jnp.int32(n_local))
            & token_valid[space_id, token_id][:, None])
        edge_local = local_f32[space_id, token_id, :]
        edge_norm_scalar = local_norm[space_id, token_id, 0]
        edge_read = reads[
            route_id, space_id[:, None], global_operator, :]
        edge_write = writes[
            route_id, space_id[:, None], global_operator, :]
        edge_raw_tau = raw_tau[
            route_id, space_id[:, None], token_id[:, None], 0]
        edge_raw_cotangent = raw_out_cotangent[
            route_id, space_id[:, None], token_id[:, None], :]
        edge_gate_cotangent = gate_mass_cotangent[
            route_id, space_id[:, None], token_id[:, None], 0]
        read_value = _xla_owner_row_dot(
            jnp.broadcast_to(edge_local[:, None, :], edge_read.shape),
            edge_read,
            throughput_bf16)
        mixed_cotangent = _xla_owner_row_dot(
            edge_raw_cotangent, edge_write, throughput_bf16)
        _, drive_gradients = _active_edge_drive_pullback(
            read_value,
            jnp.broadcast_to(edge_norm_scalar[:, None], read_value.shape),
            edge_raw_tau,
            soft_gate_temperature,
            soft_gate_boundary_power,
            execution_prune_eps,
            ...
            heat_kernel_beta=heat_kernel_beta,
            effective_active_eps=effective_active_eps)
        route_tau = jnp.stack(tuple(
            jnp.where(
                route_id == jnp.int32(route_index),
                drive_gradients[2],
                jnp.float32(0.0)
            ).sum(axis=1, dtype=jnp.float32)
            for route_index in range(n_routes)), axis=1)
        return (
            _xla_owner_reduce_rows(
                drive_gradients[0], edge_read, throughput_bf16),
            drive_gradients[1].sum(axis=1, dtype=jnp.float32),
            route_tau,
        )
    ...
    def token_owner_gradients(chunk_index):
        ...
        edge_slots = jnp.arange(
            token_edge_block_size, dtype=jnp.int32)
        task_local_init = jnp.zeros(
            (token_task_capacity, d_route), dtype=jnp.float32)
        task_norm_init = jnp.zeros(
            (token_task_capacity,), dtype=jnp.float32)
        task_tau_init = jnp.zeros(
            (token_task_capacity, n_routes), dtype=jnp.float32)
        ...
        def primary_body(state):
            task_local, task_norm, task_tau, start = state
            group_owner = jax.lax.dynamic_slice_in_dim(
                task_owner, start, token_task_group_size)
            group_block = jax.lax.dynamic_slice_in_dim(
                task_block, start, token_task_group_size)
            group_ids = (
                start + jnp.arange(
                    token_task_group_size, dtype=jnp.int32))
            safe_owner = jnp.minimum(
                group_owner, jnp.int32(token_owner_count - 1))
            count = primary_counts[safe_owner]
            positions = (
                edge_offsets[safe_owner, None]
                + group_block[:, None] * token_edge_block_size
                + edge_slots[None, :])
            safe_positions = jnp.minimum(
                positions, jnp.int32(edge_capacity - 1))
            group_edges = packed_edges[safe_positions]
            group_valid = (
                (group_ids[:, None] < total_tasks)
                & (group_block[:, None] * token_edge_block_size
                   + edge_slots[None, :] < count[:, None]))
            group_gradients = token_edge_group(
                safe_owner, group_edges, group_valid, chunk_index)
            task_local = jax.lax.dynamic_update_slice_in_dim(
                task_local, group_gradients[0], start, axis=0)
            task_norm = jax.lax.dynamic_update_slice_in_dim(
                task_norm, group_gradients[1], start, axis=0)
            task_tau = jax.lax.dynamic_update_slice_in_dim(
                task_tau, group_gradients[2], start, axis=0)
            return (
                task_local,
                task_norm,
                task_tau,
                start + jnp.int32(token_task_group_size),
            )

        task_local, task_norm, task_tau, _ = jax.lax.while_loop(
            primary_cond,
            primary_body,
            (task_local_init, task_norm_init,
             task_tau_init, jnp.int32(0)))
        primary_local = jax.ops.segment_sum(
            task_local, task_owner,
            num_segments=token_owner_count, mode="drop")
        primary_norm = jax.ops.segment_sum(
            task_norm, task_owner,
            num_segments=token_owner_count, mode="drop")
        primary_tau = jax.ops.segment_sum(
            task_tau, task_owner,
            num_segments=token_owner_count, mode="drop")
        ...
        overflow_ids, overflow_count = compact_owner_ids(
            overflow_owner)
        overflow_local_init = jnp.zeros(
            (token_owner_count, d_route), dtype=jnp.float32)
        overflow_norm_init = jnp.zeros(
            (token_owner_count,), dtype=jnp.float32)
        overflow_tau_init = jnp.zeros(
            (token_owner_count, n_routes), dtype=jnp.float32)
        edges_per_owner = n_routes * chunk_size
        overflow_blocks = math.ceil(
            edges_per_owner / token_edge_block_size)
        ...
        def overflow_body(state):
            owner_local, owner_norm, owner_tau, start = state
            group_owner = jax.lax.dynamic_slice_in_dim(
                overflow_ids, start, token_task_group_size)
            group_ids = (
                start + jnp.arange(
                    token_task_group_size, dtype=jnp.int32))
            group_live = group_ids < overflow_count
            ...
            block_init = (
                jnp.zeros(
                    (token_task_group_size, d_route),
                    dtype=jnp.float32),
                jnp.zeros(
                    (token_task_group_size,), dtype=jnp.float32),
                jnp.zeros(
                    (token_task_group_size, n_routes),
                    dtype=jnp.float32),
            )
            ...
            def overflow_block(block_index, block_carry):
                group_edges = (
                    block_index * token_edge_block_size
                    + edge_slots[None, :])
                group_edges = jnp.broadcast_to(
                    group_edges,
                    (token_task_group_size, token_edge_block_size))
                group_valid = (
                    group_live[:, None]
                    & (group_edges < jnp.int32(edges_per_owner)))
                block_gradients = token_edge_group(
                    safe_owner,
                    group_edges,
                    group_valid,
                    chunk_index)
            ...
            block_gradients = jax.lax.fori_loop(
                0, overflow_blocks, overflow_block, block_init)
            owner_local = owner_local.at[safe_owner].add(
                jnp.where(
                    group_live[:, None],
                    block_gradients[0], 0.0),
                mode="drop")
            owner_norm = owner_norm.at[safe_owner].add(
                jnp.where(
                    group_live, block_gradients[1], 0.0),
                mode="drop")
            owner_tau = owner_tau.at[safe_owner].add(
                jnp.where(
                    group_live[:, None],
                    block_gradients[2], 0.0),
                mode="drop")
            return (
                owner_local,
                owner_norm,
        ...
        overflow_local, overflow_norm, overflow_tau, _ = (
            jax.lax.while_loop(
                overflow_cond,
                overflow_body,
                (overflow_local_init, overflow_norm_init,
                 overflow_tau_init, jnp.int32(0))))
        token_local = (primary_local + overflow_local).reshape(
            (n_spaces, token_capacity, d_route))
        token_norm = (primary_norm + overflow_norm).reshape(
            (n_spaces, token_capacity, 1))
...
def _dense_rw_output_active_edge_vjp_sharded(
        local_f32: jax.Array,
        local_norm: jax.Array,
        token_valid: jax.Array,
        read_vectors: jax.Array,
        ...
        throughput_bf16: bool,
        active_edge_backward_profile: str):
    del active_edge_backward_profile
    return _dense_rw_output_impl_sharded(
        local_f32, local_norm, token_valid,
        read_vectors, write_vectors, raw_tau,
...
def _dense_rw_output_active_edge_vjp_sharded_fwd(
        local_f32: jax.Array,
        local_norm: jax.Array,
        token_valid: jax.Array,
        read_vectors: jax.Array,
        ...
        throughput_bf16: bool,
        active_edge_backward_profile: str):
    raw_out, gate_mass, packed_metadata = (
        _dense_rw_output_impl_sharded(
            local_f32, local_norm, token_valid,
            read_vectors, write_vectors, raw_tau,
            max_chunk_size=max_chunk_size,
            soft_gate_temperature=soft_gate_temperature,
            ...
            effective_active_eps=effective_active_eps,
            throughput_bf16=throughput_bf16,
            active_edge_backward_profile=active_edge_backward_profile))
    residual = (
        local_f32,
        local_norm,
        token_valid,
    ...
    return (raw_out, gate_mass), residual


def _dense_rw_output_active_edge_vjp_sharded_bwd(
        max_chunk_size: int,
        srw_composition_mode: str,
        heat_kernel_beta: float,
        effective_active_eps: float,
    ...
    (local_f32,
     ...
     soft_gate_boundary_power,
     execution_prune_eps,
     packed_metadata) = residual
    raw_out_cotangent, gate_mass_cotangent = cotangent

    gradients = _active_edge_owner_batched_rw_gradients_sharded(
        local_f32,
        local_norm,
        token_valid,
        read_vectors,
...
_dense_rw_output_active_edge_vjp_sharded.defvjp(
    _dense_rw_output_active_edge_vjp_sharded_fwd,
    _dense_rw_output_active_edge_vjp_sharded_bwd)
...
def _dense_rw_output_sharded(
    ...
    """Run dense forward with an optional exact active-edge custom VJP."""
    if active_edge_backward_profile is None:
        return _dense_rw_output_impl_sharded(
            local_f32, local_norm, token_valid,
            read_vectors, write_vectors, raw_tau,
            max_chunk_size=max_chunk_size,
            ...
            effective_active_eps=effective_active_eps,
            throughput_bf16=throughput_bf16)
    return _dense_rw_output_active_edge_vjp_sharded(
        local_f32,
        local_norm,
        token_valid,
        read_vectors,
        ...
        execution_prune_eps,
        int(max_chunk_size),
        str(srw_composition_mode),
        float(heat_kernel_beta),
        float(effective_active_eps),
        bool(throughput_bf16),
        str(active_edge_backward_profile))
...
def _make_sharded_attention_space_bundle_dense(
        ...
        def block_step(local_output, block_index):
            ...
            def execute_block(output_value):
                ...
                qk_raw_out, qk_gate_mass = _dense_rw_output_sharded(
                    ...
                    effective_active_eps=soft_gate_effective_active_eps,
                    throughput_bf16=throughput_bf16,
                    active_edge_backward_profile="qk")
                ...
                v_raw_out, v_gate_mass = _dense_rw_output_sharded(
                    ...
                    effective_active_eps=soft_gate_effective_active_eps,
                    throughput_bf16=throughput_bf16,
                    active_edge_backward_profile="v")
                grouped_raw_out = jnp.concatenate(
                    (qk_raw_out, v_raw_out), axis=0)
                grouped_gate_mass = jnp.concatenate(
                    (qk_gate_mass, v_gate_mass), axis=0)
    ...
    kernel._v4174_rw_backward_execution_mode = (
        "dual_owner_batched_dense_tasks_shared_capacity_"
        "compact_exact_overflow")
    kernel._v4174_active_edge_reduction_mode = (
        "operator_and_token_partial_reduce_then_single_segment")
    kernel._v4174_active_edge_task_schedule = (
        "xla_primary_task_tiles_plus_owner_exact_overflow")
    kernel._v4174_active_edge_overflow_mode = (
        "owner_parallel_dense_replay_drop_zero")
    kernel._v4174_active_edge_backward_profiles = ("qk", "v")
    kernel._v4174_active_edge_drop_count = 0
    kernel._v4174_throughput_precision = (
        "bf16_operands_f32_accum"
        if throughput_bf16 else "fp32_reference")
    kernel._v4174_output_collective_dtype = (
...
def _make_sharded_rst_space_bundle_dense(
        ...
        def block_step(local_output, block_index):
            ...
            def execute_block(output_value):
                ...
                raw_out, gate_mass = _dense_rw_output_sharded(
                    ...
                    effective_active_eps=soft_gate_effective_active_eps,
                    throughput_bf16=throughput_bf16,
                    active_edge_backward_profile="rst")
    ...
    kernel._v4174_active_edge_overflow_mode = (
        "owner_parallel_dense_replay_drop_zero")
    kernel._v4174_active_edge_backward_profiles = ("rst",)
    kernel._v4174_active_edge_drop_count = 0
    kernel._v4174_throughput_precision = (
        "bf16_operands_f32_accum"
        if throughput_bf16 else "fp32_reference")
...
def _validate_v4174_sharded_fns(
    ...
    if attention_mode == "bundle_dense":
        for name, kernel in (("attention", attention), ("rst", rst)):
            ...
            if getattr(
                    kernel, "_v4174_rw_backward_execution_mode", None
            ) != (
                    "dual_owner_batched_dense_tasks_shared_capacity_"
                    "compact_exact_overflow"
            ):
                raise ValueError(
                    f"v4174 {name} bundle executor requires dual-owner "
                    "task-parallel active-edge RW backward")
            if getattr(
                    kernel, "_v4174_active_edge_reduction_mode", None
            ) != "operator_and_token_partial_reduce_then_single_segment":
                raise ValueError(
                    f"v4174 {name} bundle executor requires owner-local "
                    "partial reduction before one segment reduction")
            if getattr(
                    kernel, "_v4174_active_edge_task_schedule", None
            ) != "xla_primary_task_tiles_plus_owner_exact_overflow":
                raise ValueError(
                    f"v4174 {name} bundle executor requires XLA primary "
                    "tasks plus owner-parallel exact overflow")
            if getattr(
                    kernel, "_v4174_active_edge_overflow_mode", None
            ) != "owner_parallel_dense_replay_drop_zero":
                raise ValueError(
                    f"v4174 {name} bundle executor requires exact owner "
                    "overflow replay with zero dropped edges")
            if getattr(
                    kernel, "_v4174_active_edge_drop_count", None) != 0:
                raise ValueError(
                    f"v4174 {name} active-edge backward must drop zero edges")
