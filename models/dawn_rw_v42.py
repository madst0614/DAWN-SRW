"""
DAWN Read-Write v4.2 (JAX/Flax)

Architecture: Read-Write (no separate sense/emb vectors).
Read vectors serve dual purpose: routing score AND feature extraction.
score = x @ read_unit.T (no proj, no emb).

Changelog:
  rw-v4.2 (2026-04-14):
    - Remove sense (emb) vectors entirely. Read = routing + extraction.
    - Remove proj_attn, proj_know (use_proj=False by default).
    - score = normed_x @ read_unit.T (direct matching).
    - Gate: GELU z × Φ(z) (from v4.0.1). Den floor=1.0.
    - Diversity loss on read/write replaces LB emphasis.
    - ~4M params saved (no emb, no proj). Same neuron counts.
...
def make_sharded_srw(mesh, max_chunk_size=2048):
    ...
    @partial(shard_map, mesh=mesh,
             in_specs=(P('data', None, None),    # x [B,S,D]
                       ...
                       P('model', None),          # read_norm [N_local, d_bn]
             ...
             check_rep=False)
    def fused_gate_srw(x, h, read_local, tau_offset, read_local, write_local):
        ...
        def stats_step(carry, i):
            ...
            s = i * cs
            ec = jax.lax.dynamic_slice_in_dim(read_bf, s, cs, axis=0)
            scores = h_bf @ ec.T
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048):
    ...
    @partial(shard_map, mesh=mesh,
             ...
             out_specs=(P('data', None, None, None), # out [B,S,2,D]
                        ...
                        P('data', None, None)),      # z_mean_active [B,S,1]
             check_rep=False)
    def fused_gate_srw_paired(x, h, read_local, tau_offset, read_local, write_local):
        N_local = read_local.shape[0]
        nc = max(1, N_local // max_chunk_size)
        while N_local % nc != 0 and nc < N_local:
            nc += 1
        ...
        (raw_out, total_weighted_cost, total_gate_max, total_active, total_strong, total_phi_binary, total_z_sum), _ = jax.lax.scan(
            ...
            (jnp.zeros((B, S, 2, D), dtype=jnp.float32),
             z1_r, jnp.full((B, S, 2, 1), -1e9), z1_r, z1_r, z1_r, z1_r),
            jnp.arange(nc))
...
def _srw_chunked(x, h, read_unit, tau_offset, w_read, w_write, n_chunks):
    ...
    h_bf = h.astype(jnp.bfloat16)
    x_bf = x.astype(jnp.bfloat16)
    read_bf = read_unit.astype(jnp.bfloat16)
...
# ================================================================
# 4. NeuronPool -- read + write (no emb/sense)
# ================================================================
...
class NeuronPool(nn.Module):
    ...
    n_know: int
    d_model: int
    d_route: int = 128  # unused in v4.2, kept for interface compat

    def setup(self):
        dm = self.d_model

        # Read (routing + feature extraction — dual purpose)
...
# ================================================================
# 5. Router -- tau only (no proj in v4.2)
# ================================================================

class Router(nn.Module):
    d_model: int
    d_route: int = 128  # unused, kept for interface
    n_qk: int = 1580
    n_v: int = 2600
    n_know: int = 25200
    ...
    def setup(self):
        if self.use_proj:
            self.proj_attn = nn.Dense(self.d_model * 3, name='proj_attn')
            self.proj_know = nn.Dense(self.d_model, name='proj_know')
        # tau only — always present
    ...
    def get_attention_gates(self, x, neuron_pool, deterministic, rng):
        qk_read_unit = neuron_pool.qk_read / (
            jnp.linalg.norm(neuron_pool.qk_read, axis=-1, keepdims=True) + 1e-8)
        ...
        if self.use_proj:
            ...
            # Direct matching: x is the query
            h_Q = h_K = h_V = x

        tau_all = self.tau_attn(x)
        g_Q = threshold_gate(h_Q @ qk_read_unit.T, tau_all[:, :, 0:1])
        g_K = threshold_gate(h_K @ qk_read_unit.T, tau_all[:, :, 1:2])
        g_V = threshold_gate(h_V @ v_read_unit.T, tau_all[:, :, 2:3])
    ...
    def get_knowledge_gates(self, x, neuron_pool, deterministic, rng):
        ...
        tau = self.tau_know(x)
        gate = threshold_gate(h @ know_read_unit.T, tau)
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    # v4.2: read serves as both routing and extraction
    qk_read_unit = qk_read / (jnp.linalg.norm(qk_read, axis=-1, keepdims=True) + 1e-8)
    v_read_unit = v_read / (jnp.linalg.norm(v_read, axis=-1, keepdims=True) + 1e-8)

    # v4.2: no proj — x is the routing query directly
    if 'proj_attn' in router_params:
        rng, rng_drop = jax.random.split(rng)
    ...
    if sharded_fns is not None:
        ...
        QK_out, qk_active, qk_raw_gmax, qk_lb, qk_sstd, qk_es, qk_anm, qk_smean, qk_strong, qk_phi_bin, qk_z_act = fused_paired(
            x, h_QK, qk_read_unit, tau_QK, qk_read, qk_write)
        ...
        V, v_active, v_raw_gmax, v_lb, v_sstd, v_es, v_anm, v_smean, v_strong, v_phi_bin, v_z_act = fused_single(
            x, h_V, v_read_unit, tau_all[:, :, 2:3], v_read, v_write)
...
def _know_forward(x, pool_params, router_params, rng,
    ...
    return (out, aux, active_frac, raw_gate_max, score_std, gate_sum, active_n_mean,
            read_norm_val, read_norm_val, write_norm_val, score_mean, know_out_norm,
...
class DAWN(nn.Module):
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        _read_norm = jnp.linalg.norm(self.token_emb.embedding, axis=-1).mean()
        ...
        result = {
            ...
            'know_read_norm': k_emb_n_all.mean(),
    ...
    def diversity_loss(self):
        ...
        return (_div(pool.qk_read) + _div(pool.qk_write) +
                _div(pool.v_read) + _div(pool.v_write) +
                _div(pool.know_read) + _div(pool.know_write)) / 6
    ...
    def get_model_info(self):
        return [
            ...
            f"  Per neuron: read[{self.d_model}] "
...
def _srw_inference(x, h, read_norm, tau_offset, w_read, w_write):
    """Non-chunked SRW for inference. Binary gate + xr²-weighted soft denominator."""
    scores = h @ read_norm.T
    scores_f32 = scores.astype(jnp.float32)
    s_mean = scores_f32.mean(axis=-1, keepdims=True)
...
def _srw_inference_with_gates(x, h, read_norm, tau_offset, w_read, w_write):
...
def _attn_forward_cached(x, pool_params, router_params, expand_O_kernel,
    ...
    qk_norm = pool_params['qk_read'] / (jnp.linalg.norm(pool_params['qk_read'], axis=-1, keepdims=True) + 1e-8)
    v_norm = pool_params['v_read'] / (jnp.linalg.norm(pool_params['v_read'], axis=-1, keepdims=True) + 1e-8)
...
def _know_forward_inference(x, pool_params, router_params):
    ...
    know_read_vec = pool_params['know_read']
    know_norm = know_read_vec / (jnp.linalg.norm(know_read_vec, axis=-1, keepdims=True) + 1e-8)
    # v4.2: no proj, direct matching
    if 'proj_know' in router_params:
        h = x @ router_params['proj_know']['kernel'] + router_params['proj_know']['bias']
    else:
        h = x
    tau = x @ router_params['tau_know']['kernel'] + router_params['tau_know']['bias']
...
def prefill(params, model_cfg, input_ids):
    ...
    qk_norm = pool_params['qk_read'] / (
        jnp.linalg.norm(pool_params['qk_read'], axis=-1, keepdims=True) + 1e-8)
    v_norm = pool_params['v_read'] / (
    ...
    def prefill_layer(carry, xs):
        ...
        h_all = (normed @ router_params['proj_attn']['kernel'] + router_params['proj_attn']['bias']) if 'proj_attn' in router_params else jnp.concatenate([normed, normed, normed], axis=-1)
        h_Q, h_K, h_V = jnp.split(h_all, 3, axis=-1)
...
def vectorized_eval(params, model_cfg, all_tokens, batch_size=32):
    ...
    v_norm = pool_params['v_read'] / (
        jnp.linalg.norm(pool_params['v_read'], axis=-1, keepdims=True) + 1e-8)
    know_norm = pool_params['know_read'] / (
    ...
    def forward_batch(input_ids):
        ...
        def layer_fn(x, bp):
            ...
            normed = _layer_norm(x, bp['norm2']['scale'], bp['norm2']['bias'])
            h_k = (normed @ router_params['proj_know']['kernel'] + router_params['proj_know']['bias']) if 'proj_know' in router_params else normed
...
def vectorized_neuron_health(params):
    ...
    results = {}
    for pool_name, read_key in [('QK', 'qk_read'), ('V', 'v_read'), ('Know', 'know_read')]:
        emb = pool[read_key]  # v4.2: read = routing vector
        read = pool[read_key]
...
def analysis_forward(params, model_cfg, input_ids, mode='full'):
    ...
    v_norm = pool_params['v_read'] / (
        jnp.linalg.norm(pool_params['v_read'], axis=-1, keepdims=True) + 1e-8)
    know_read_n = pool_params['know_read'] / (
        jnp.linalg.norm(pool_params['know_read'], axis=-1, keepdims=True) + 1e-8)
...
def build_suppressed_forward(params, model_cfg, suppress_masks):
    ...
    def forward_fn(input_ids):
        ...
        positions = jnp.arange(S)[jnp.newaxis, :]
        x = params['token_emb']['embedding'][input_ids] + params['pos_emb']['embedding'][positions]
        qk_n = pp['qk_read'] / (jnp.linalg.norm(pp['qk_read'], axis=-1, keepdims=True) + 1e-8)
        ...
        for i in range(n_layers):
            ...
            normed = _layer_norm(x, bp['norm1']['scale'], bp['norm1']['bias'])
            h_all = (normed @ rp['proj_attn']['kernel'] + rp['proj_attn']['bias']) if 'proj_attn' in rp else jnp.concatenate([normed, normed, normed], axis=-1)
            h_Q, h_K, h_V = jnp.split(h_all, 3, axis=-1)
            ...
            normed = _layer_norm(x, bp['norm2']['scale'], bp['norm2']['bias'])
            h_k = (normed @ rp['proj_know']['kernel'] + rp['proj_know']['bias']) if 'proj_know' in rp else normed
