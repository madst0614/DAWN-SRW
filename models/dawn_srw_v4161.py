"""
DAWN-SRW v4.1.6.1 DirectTau + CB1A 20260520: SRW neurons and Residual State Transition.

This file contains the active v4.1.6.1 DirectTau PureCore implementation. DAWN neurons are
...
* v4.1.6.x uses raw route signatures for routing diagnostics; Select
  unit-normalizes only the selection subspace inside the cosine relation.
* v4.1.6.x uses forward unit-normalized read/write vectors: stored read/write
...
* CB1A (CE-conditioned Boundary-1 Audition) is auxiliary-only: it observes the
  closest selected/unselected rho values around a frozen tau boundary, and
...
CB1A_DIAG_NAMES = (
    'cb1a_challenge_gap_mean',
    'cb1a_prune_gap_mean',
    'cb1a_valid_pair_frac',
    'cb1a_has_above_frac',
    'cb1a_has_below_frac',
)
CB1A_DIAG_COUNT = len(CB1A_DIAG_NAMES)
(
    CB1A_CHALLENGE_GAP_MEAN,
    CB1A_PRUNE_GAP_MEAN,
    CB1A_VALID_PAIR_FRAC,
    CB1A_HAS_ABOVE_FRAC,
    CB1A_HAS_BELOW_FRAC,
) = range(CB1A_DIAG_COUNT)


def _cb1a_boundary_gaps_from_rho_tau(
        rho, tau, tau_stopgrad=True, anchor_stopgrad=True,
        neg_large=-1.0e30, pos_large=1.0e30):
    """Pure local CB1A boundary helper used by smoke tests."""
    tau_ref = jax.lax.stop_gradient(tau) if tau_stopgrad else tau
    below_1 = jnp.max(
        jnp.where(rho < tau_ref, rho, jnp.asarray(neg_large, rho.dtype)),
        axis=-1, keepdims=True)
    above_1 = jnp.min(
        jnp.where(rho > tau_ref, rho, jnp.asarray(pos_large, rho.dtype)),
        axis=-1, keepdims=True)
    has_below = below_1 > jnp.asarray(neg_large * 0.5, rho.dtype)
    has_above = above_1 < jnp.asarray(pos_large * 0.5, rho.dtype)
    valid_pair = has_below & has_above
    challenge_anchor = (
        jax.lax.stop_gradient(above_1) if anchor_stopgrad else above_1)
    challenge_gap = jax.nn.relu(challenge_anchor - below_1)
    prune_gap = jax.nn.relu(above_1 - tau_ref)
    challenge_gap = jnp.where(valid_pair, challenge_gap, 0.0)
    prune_gap = jnp.where(has_above, prune_gap, 0.0)
    return below_1, above_1, challenge_gap, prune_gap, valid_pair, has_above, has_below


def _cb1a_local_winner_gaps_from_boundaries(
        local_below_best, local_above_best, tau,
        global_below_ref, global_above_ref,
        neg_large=-1.0e30, pos_large=1.0e30):
    """CB1A local winner gaps with stop-gradient global references."""
    global_below_sg = jax.lax.stop_gradient(global_below_ref)
    global_above_sg = jax.lax.stop_gradient(global_above_ref)
    tau_sg = jax.lax.stop_gradient(tau)

    has_below = global_below_sg > jnp.asarray(neg_large * 0.5, local_below_best.dtype)
    has_above = global_above_sg < jnp.asarray(pos_large * 0.5, local_above_best.dtype)
    valid_pair = has_below & has_above

    below_winner = jax.lax.stop_gradient(local_below_best == global_below_sg)
    above_winner = jax.lax.stop_gradient(local_above_best == global_above_sg)
    below_winner_f = below_winner.astype(local_below_best.dtype)
    above_winner_f = above_winner.astype(local_above_best.dtype)

    challenge_gap_local = (
        below_winner_f * jax.nn.relu(global_above_sg - local_below_best))
    prune_gap_local = (
        above_winner_f * jax.nn.relu(local_above_best - tau_sg))
    challenge_gap_local = jnp.where(valid_pair, challenge_gap_local, 0.0)
    prune_gap_local = jnp.where(has_above, prune_gap_local, 0.0)
    return (
        challenge_gap_local, prune_gap_local, valid_pair, has_above,
        has_below, below_winner, above_winner)
...
def make_sharded_srw(mesh, max_chunk_size=2048,
    ...
    if not bool(cb1a_tau_stopgrad):
        raise ValueError("CB1A requires cb1a_tau_stopgrad=true.")
    if not bool(cb1a_anchor_stopgrad):
        raise ValueError("CB1A requires cb1a_anchor_stopgrad=true.")
    _cb1a_tau_stopgrad = True
    _cb1a_anchor_stopgrad = True
    if bool(cb1a_forward_influence):
        raise ValueError("CB1A is auxiliary-only; cb1a_forward_influence must be false.")
    ...
    _cb1a_tensor_specs = (
        P('data', None, None),   # cb1a_challenge_gap [B,S,1]
        P('data', None, None),   # cb1a_prune_gap [B,S,1]
    )
    _cb1a_diag_specs = tuple(P() for _ in range(CB1A_DIAG_COUNT))
    _local_diag_specs = (
    ...
    def fused_gate_srw(x, h, emb_local, raw_tau,
        ...
        def update_cb1a_boundary(below_best, above_best, rho):
            tau_ref = jax.lax.stop_gradient(tau) if _cb1a_tau_stopgrad else tau
            below_local = jnp.max(
                jnp.where(rho < tau_ref, rho, diag_neg_inf),
                axis=-1, keepdims=True)
            above_local = jnp.min(
                jnp.where(rho > tau_ref, rho, diag_pos_inf),
                axis=-1, keepdims=True)
            return (
                jnp.maximum(below_best, below_local),
                jnp.minimum(above_best, above_local),
            )
        ...
        def spike_chunk_rows(start, rho, raw_tau_chunk, tau_chunk,
                             selection_margin, positive_margin, intensity,
                             gate, xr_f, read_norm, write_norm):
            contrib_proxy = (
            ...
            pos_margin_i = positive_margin[b_idx, pos_idx, n_idx]
            intensity_i = intensity[b_idx, pos_idx, n_idx]
            gate_i = gate[b_idx, pos_idx, n_idx]
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                (out, total_weighted_cost, total_gate_sq, total_gate_max, total_active,
                 ...
                 total_selection_margin_reg,
                 total_cb1a_below_best, total_cb1a_above_best,
                ...
                if _cb1a_enabled:
                    (total_cb1a_below_best, total_cb1a_above_best) = (
                        update_cb1a_boundary(
                            total_cb1a_below_best, total_cb1a_above_best, rho))
                ...
                if _spike_probe:
                    ...
                    spike_rows = spike_merge_rows(
                        ...
                        spike_chunk_rows(
                            ...
                            positive_margin, intensity, gate, xr_f,
                ...
                return (out + c_out,
                        ...
                        total_cb1a_below_best,
                        total_cb1a_above_best,
                        select_diag_carry,
                        diag_vals,
                        spike_rows), None
            ...
            (raw_out, total_weighted_cost, total_gate_sq, total_gate_max, total_active, total_strong,
             ...
             select_diag_carry, diag_vals, spike_rows), _ = jax.lax.scan(
                ...
                (jnp.zeros((B, S, D), dtype=jnp.float32),
                 ...
                 jnp.float32(0.0),
                 jnp.full((B, S, 1), diag_neg_inf, dtype=jnp.float32),
                 jnp.full((B, S, 1), diag_pos_inf, dtype=jnp.float32),
                 select_diag_carry0,
                 diag_vals_init,
                 spike_rows_init),
                jnp.arange(nc))
            ...
            (cb1a_challenge_gap_local, cb1a_prune_gap_local,
             valid_pair, has_above, has_below, _, _) = (
                _cb1a_local_winner_gaps_from_boundaries(
                    total_cb1a_below_best, total_cb1a_above_best, tau,
                    global_below_sg, global_above_sg,
                    neg_large=diag_neg_inf, pos_large=diag_pos_inf))
            cb1a_challenge_gap = jax.lax.psum(
                cb1a_challenge_gap_local, 'model')
            cb1a_prune_gap = jax.lax.psum(
                cb1a_prune_gap_local, 'model')
            cb1a_diag_out = (
                jax.lax.stop_gradient(cb1a_challenge_gap.mean()),
                jax.lax.stop_gradient(cb1a_prune_gap.mean()),
                jax.lax.stop_gradient(valid_pair.astype(jnp.float32).mean()),
                jax.lax.stop_gradient(has_above.astype(jnp.float32).mean()),
                jax.lax.stop_gradient(has_below.astype(jnp.float32).mean()),
            )
        else:
            cb1a_challenge_gap = jnp.zeros((B, S, 1), dtype=jnp.float32)
            cb1a_prune_gap = jnp.zeros((B, S, 1), dtype=jnp.float32)
            cb1a_diag_out = tuple(
                jnp.float32(0.0) for _ in range(CB1A_DIAG_COUNT))
        cb1a_tensor_out = (cb1a_challenge_gap, cb1a_prune_gap)
        ...
        if not analysis:
            return (slim_out + conc_out + select_diag_out
                    ...
                    + cb1a_diag_out + local_diag_out + spike_probe_out)
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048,
    ...
    _dead_exposure_diag_specs = tuple(
        P() for _ in range(DEAD_EXPOSURE_DIAG_COUNT))
    _cb1a_tensor_specs = (
        P('data', None, None, None),  # cb1a_challenge_gap [B,S,2,1]
        P('data', None, None, None),  # cb1a_prune_gap [B,S,2,1]
    )
    _cb1a_diag_specs = tuple(P() for _ in range(CB1A_DIAG_COUNT))
    _local_diag_specs = (
    ...
    def fused_gate_srw_paired(x, h, emb_local, raw_tau,
        ...
        def spike_chunk_rows_paired(start, rho, raw_tau_chunk, tau_chunk,
                                    selection_margin, positive_margin,
                                    intensity, gate, xr_f, read_norm,
                                    write_norm):
            xr_r = xr_f[:, :, None, :]
            contrib_proxy = (
            ...
            for route_idx in range(2):
                ...
                pos_margin_i = positive_margin[b_idx, pos_idx, route_idx, n_idx]
                intensity_i = intensity[b_idx, pos_idx, route_idx, n_idx]
                gate_i = gate[b_idx, pos_idx, route_idx, n_idx]
                xr_i = xr_f[b_idx, pos_idx, n_idx]
                h_norm_i = jnp.linalg.norm(
            ...
            (raw_out, total_weighted_cost, total_gate_sq, total_gate_max, total_active, total_strong,
             ...
             select_diag_carry, diag_vals, spike_rows), _ = jax.lax.scan(
                ...
                (jnp.zeros((B, S, 2, D), dtype=jnp.float32),
                 ...
                 jnp.float32(0.0),
                 jnp.full((B, S, 2, 1), diag_neg_inf, dtype=jnp.float32),
                 jnp.full((B, S, 2, 1), diag_pos_inf, dtype=jnp.float32),
                 select_diag_carry0,
                 diag_vals_init,
                 spike_rows_init),
                jnp.arange(nc))
        ...
        if _cb1a_enabled:
            ...
            cb1a_challenge_gap = jnp.zeros((B, S, 2, 1), dtype=jnp.float32)
            cb1a_prune_gap = jnp.zeros((B, S, 2, 1), dtype=jnp.float32)
            cb1a_diag_out = tuple(
                jnp.float32(0.0) for _ in range(CB1A_DIAG_COUNT))
        cb1a_tensor_out = (cb1a_challenge_gap, cb1a_prune_gap)
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    qk_cb1a_start = qk_exposure_start + DEAD_EXPOSURE_DIAG_COUNT
    (qk_cb1a_challenge_gap, qk_cb1a_prune_gap) = qk_ret[
        qk_cb1a_start:qk_cb1a_start + 2]
    qk_cb1a_diag = qk_ret[
        qk_cb1a_start + 2:qk_cb1a_start + 2 + CB1A_DIAG_COUNT]
    if spike_probe:
    ...
    v_cb1a_start = v_exposure_start + DEAD_EXPOSURE_DIAG_COUNT
    (v_cb1a_challenge_gap, v_cb1a_prune_gap) = v_ret[
        v_cb1a_start:v_cb1a_start + 2]
    v_cb1a_diag = v_ret[
        v_cb1a_start + 2:v_cb1a_start + 2 + CB1A_DIAG_COUNT]
    if spike_probe:
    ...
    def _focus_srw_rows_for_pool(pool_id, x_in, h_in, emb, raw_tau, read, write, pool_scale):
        ...
        d_total = int(hf.shape[-1])
        d_sel = min(max(_d_select, 1), d_total - 1)
        q_sel = _forward_unit_direction(hf[:, :d_sel])
        e_sel = _forward_unit_direction(emb_f[:, :d_sel])
        rho = q_sel @ e_sel.T
        h_int = hf[:, d_sel:]
        e_int = emb_f[:, d_sel:]
        ...
        active_n = (margin > 0.0).astype(jnp.float32).sum(axis=-1)
        ...
        return jnp.stack([
            ...
            take(gate) / jnp.maximum(den[:, None], 1e-8),
    ...
    attn_cb1a_challenge_gap = jnp.concatenate(
        [qk_cb1a_challenge_gap[..., 0], v_cb1a_challenge_gap], axis=-1)
    attn_cb1a_prune_gap = jnp.concatenate(
        [qk_cb1a_prune_gap[..., 0], v_cb1a_prune_gap], axis=-1)
    attn_cb1a_diag = jnp.stack(tuple(
        (qk_cb1a_diag[i] * 2.0 + v_cb1a_diag[i]) / 3.0
...
def _rst_forward(x, pool_params, router_params, rng,
    ...
    rst_cb1a_start = rst_exposure_start + DEAD_EXPOSURE_DIAG_COUNT
    (rst_cb1a_challenge_gap, rst_cb1a_prune_gap) = rst_ret[
        rst_cb1a_start:rst_cb1a_start + 2]
    rst_cb1a_diag = rst_ret[
        rst_cb1a_start + 2:rst_cb1a_start + 2 + CB1A_DIAG_COUNT]
    if spike_probe:
    ...
    slim_ret = (out, aux, active_frac, raw_gate_max, rho_std_slim, gate_sum, active_n_mean,
                ...
                *rst_select_diag,
                *rst_exposure_diag,
                rst_cb1a_challenge_gap,
                rst_cb1a_prune_gap,
                jnp.stack(rst_cb1a_diag).astype(jnp.float32))
    if not analysis:
        ret = slim_ret
        if local_diagnostics:
            ret = ret + (
...
class DAWN(nn.Module):
    """DAWN-SRW v4.1.6.1 with Attention Layers, RST Layers, and CB1A."""
    __version__ = "spatial-r1-v4.1.6.1"
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        if self.is_initializing():
            ...
            attn_v_exposure_diag_all = jnp.zeros(
                (1, DEAD_EXPOSURE_DIAG_COUNT), dtype=jnp.float32)
            attn_cb1a_challenge_gap_all = jnp.zeros(
                (self.n_layers, B, S, 3), dtype=jnp.float32)
            attn_cb1a_prune_gap_all = jnp.zeros(
                (self.n_layers, B, S, 3), dtype=jnp.float32)
            rst_cb1a_challenge_gap_all = jnp.zeros(
                (self.n_layers, B, S, 1), dtype=jnp.float32)
            rst_cb1a_prune_gap_all = jnp.zeros(
                (self.n_layers, B, S, 1), dtype=jnp.float32)
            attn_cb1a_diag_all = jnp.zeros(
                (self.n_layers, CB1A_DIAG_COUNT), dtype=jnp.float32)
            rst_cb1a_diag_all = jnp.zeros(
                (self.n_layers, CB1A_DIAG_COUNT), dtype=jnp.float32)
            attn_qk_cb1a_diag_all = jnp.zeros(
                (self.n_layers, CB1A_DIAG_COUNT), dtype=jnp.float32)
            attn_v_cb1a_diag_all = jnp.zeros(
                (self.n_layers, CB1A_DIAG_COUNT), dtype=jnp.float32)
            ...
            def scan_body(carry, xs):
                ...
                slim_ys = (attn_aux, rst_aux,
                           ...
                           a_cb1a_challenge_gap, a_cb1a_prune_gap,
                           k_cb1a_challenge_gap, k_cb1a_prune_gap,
                           a_cb1a_diag, k_cb1a_diag,
                           a_qk_cb1a_diag, a_v_cb1a_diag,
                           )
                if not analysis:
                    if local_diagnostics:
                        slim_ys = slim_ys + (
            ...
            _scan_offset += 5
            (attn_cb1a_challenge_gap_all,
             attn_cb1a_prune_gap_all,
             rst_cb1a_challenge_gap_all,
             rst_cb1a_prune_gap_all,
             attn_cb1a_diag_all,
             rst_cb1a_diag_all,
             attn_qk_cb1a_diag_all,
             attn_v_cb1a_diag_all) = scan_ys[_scan_offset:_scan_offset + 8]
            _scan_offset += 8
            if analysis:
        ...
        def _cb1a_mean(diag_all, idx):
            return diag_all[:, idx].mean()

        def _cb1a_global(idx):
            return (
                _cb1a_mean(attn_cb1a_diag_all, idx) * 3.0
                + _cb1a_mean(rst_cb1a_diag_all, idx)
            ) / 4.0
        ...
        result = {
            ...
            'attn_cb1a_challenge_gap': attn_cb1a_challenge_gap_all,
            'attn_cb1a_prune_gap': attn_cb1a_prune_gap_all,
            'rst_cb1a_challenge_gap': rst_cb1a_challenge_gap_all,
            'rst_cb1a_prune_gap': rst_cb1a_prune_gap_all,
            'cb1a_challenge': _cb1a_global(CB1A_CHALLENGE_GAP_MEAN),
            'cb1a_prune': _cb1a_global(CB1A_PRUNE_GAP_MEAN),
            'cb1a_valid': _cb1a_global(CB1A_VALID_PAIR_FRAC),
            'cb1a_has_above': _cb1a_global(CB1A_HAS_ABOVE_FRAC),
            'cb1a_has_below': _cb1a_global(CB1A_HAS_BELOW_FRAC),
            'cb1a_qk_challenge': _cb1a_mean(
                attn_qk_cb1a_diag_all, CB1A_CHALLENGE_GAP_MEAN),
            'cb1a_qk_prune': _cb1a_mean(
                attn_qk_cb1a_diag_all, CB1A_PRUNE_GAP_MEAN),
            'cb1a_v_challenge': _cb1a_mean(
                attn_v_cb1a_diag_all, CB1A_CHALLENGE_GAP_MEAN),
            'cb1a_v_prune': _cb1a_mean(
                attn_v_cb1a_diag_all, CB1A_PRUNE_GAP_MEAN),
            'cb1a_rst_challenge': _cb1a_mean(
                rst_cb1a_diag_all, CB1A_CHALLENGE_GAP_MEAN),
            'cb1a_rst_prune': _cb1a_mean(
                rst_cb1a_diag_all, CB1A_PRUNE_GAP_MEAN),
            'selection_margin_reg_qk': (
                attn_qk_selection_margin_reg_all.mean()),
            'selection_margin_reg_v': (
                attn_v_selection_margin_reg_all.mean()),
            'selection_margin_reg_rst': (
                rst_selection_margin_reg_all.mean()),
