"""
DAWN-SRW v4.0.5: Select-Read-Write (JAX/Flax)
    ...
    def fused_gate_srw(x, h, emb_local, tau_offset, read_local, write_local,
        ...
        def gate_srw_step(carry, i):
            (out, total_weighted_cost, total_gate_max, total_active,
             ...
             total_drop_count, total_boost_count) = carry
            s = i * cs
            ec = jax.lax.dynamic_slice_in_dim(emb_bf, s, cs, axis=0)
            ...
            return (out + c_out,
                    ...
                    total_drop_count + chunk_drop_count,
                    total_boost_count + chunk_boost_count), None
        ...
        (raw_out, total_weighted_cost, total_gate_max, total_active, total_strong,
         ...
         total_g_log_g, total_drop_count, total_boost_count), _ = jax.lax.scan(
            gate_srw_step,
        ...
        # v4.0.5: bidirectional-dropout rates across (B, S, N_total) cells
        _denom = jnp.float32(B * S * N_total) + 1e-8
        drop_rate_out = jax.lax.stop_gradient(
            jax.lax.psum(total_drop_count, 'model') / _denom)
        boost_rate_out = jax.lax.stop_gradient(
            jax.lax.psum(total_boost_count, 'model') / _denom)
        return (out.astype(jnp.float32), active_frac, global_gate_max, score_lb,
                ...
                score_kurt, drop_rate_out, boost_rate_out)
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048):
    ...
    v4.0.5: bidirectional gate dropout (drop on z>0, boost on z<0).
    drop_rate_eff / boost_rate_eff are runtime scalars; eval passes 0.
    """
    _model_axis_size = mesh.shape['model']
    _data_axis_size = mesh.shape['data']
    ...
    def fused_gate_srw_paired(x, h, emb_local, tau_offset, read_local, write_local,
        ...
        entropy_per_token = -global_g_log_g / gate_sum_eps + jnp.log(gate_sum_eps)
        gate_entropy = jax.lax.stop_gradient(entropy_per_token).mean()
        # v4.0.5: bidirectional-dropout rates across (B, S, 2, N_total) cells
        _denom = jnp.float32(B * S * 2 * N_total) + 1e-8
        drop_rate_out = jax.lax.stop_gradient(
            jax.lax.psum(total_drop_count, 'model') / _denom)
        boost_rate_out = jax.lax.stop_gradient(
            jax.lax.psum(total_boost_count, 'model') / _denom)
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
                  ...
                  sharded_fns, gate_drop_rate=0.0, gate_boost_rate=0.0):
    """v4.0.5: sharded-only. sharded_fns=(fused_single, fused_paired) required.
    gate_drop_rate / gate_boost_rate are config values; actual runtime scalars
    become 0.0 when deterministic (eval)."""
    B, S, D = x.shape
    ...
    attn_drop_rate = (qk_drop + v_drop) / 2
    attn_boost_rate = (qk_boost + v_boost) / 2
    return (out, aux, qk_active.mean(), v_active.mean(), attn_raw_gmax,
            ...
            attn_score_kurt,
            attn_drop_rate, attn_boost_rate)


def _know_forward(x, pool_params, router_params, rng,
                  router_dropout, dropout_rate, deterministic,
                  sharded_fns, gate_drop_rate=0.0, gate_boost_rate=0.0):
    """v4.0.5: sharded-only. drop/boost eff are 0 at eval (deterministic)."""
    ...
    return (out, aux, active_frac, raw_gate_max, score_std, gate_sum, active_n_mean,
            ...
            know_drop_rate, know_boost_rate)
...
class DAWN(nn.Module):
    ...
    gate_drop_rate: float = 0.0   # v4.0.5: Bernoulli drop prob on z>0 cells
    gate_boost_rate: float = 0.0  # v4.0.5: scale on Bernoulli boost for z<0 cells
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        if self.is_initializing():
            ...
            attn_drop_rate_all = _z
            attn_boost_rate_all = _z
            know_drop_rate_all = _z
            know_boost_rate_all = _z
            ...
            def scan_body(carry, xs):
                ...
                (attn_out, attn_aux, a_qk_active, a_v_active, a_raw_gmax,
                 ...
                 a_score_kurt, a_drop_rate, a_boost_rate
                ) = _attn_forward(
                    ...
                    gate_drop_rate=self.gate_drop_rate,
                    gate_boost_rate=self.gate_boost_rate)
                x = x + attn_out
                ...
                (know_out, know_aux, k_active, k_raw_gmax, k_sstd, k_gsum, k_active_n_mean,
                 ...
                 k_emb_n_min, k_score_kurt, k_drop_rate, k_boost_rate
                ) = _know_forward(
                ...
                return x, (attn_aux, know_aux,
                           ...
                           a_drop_rate, a_boost_rate,
                           k_drop_rate, k_boost_rate)
            ...
            x, (attn_auxes, know_auxes,
                ...
                attn_drop_rate_all, attn_boost_rate_all,
                know_drop_rate_all, know_boost_rate_all) = jax.lax.scan(
                scan_body, x, xs)
        ...
        result = {
            ...
            'attn_drop_rate': attn_drop_rate_all.mean(),
            'attn_boost_rate': attn_boost_rate_all.mean(),
            'know_drop_rate': know_drop_rate_all.mean(),
            'know_boost_rate': know_boost_rate_all.mean(),
