"""
DAWN-SRW v4.1.6.2 Soft Annealed DirectTau
...
# ================================================================
# V4.1.6.2 soft annealed DirectTau in cosine space.
#
...
def _compute_soft_gate(score, tau, intensity, temperature, tau_ce_grad_scale,
    ...
    """Clean v4162 soft annealed DirectTau gate."""
    temperature = jnp.maximum(jnp.asarray(temperature, dtype=jnp.float32),
                              jnp.float32(1.0e-4))
    ...
    tau_for_gate = tau_sg + tau_ce_grad_scale * (tau - tau_sg)
    margin = score - tau_for_gate
    z = jnp.clip(margin / temperature, -30.0, 30.0)
    soft_weight = jax.nn.sigmoid(z)
...
def make_sharded_srw(mesh, max_chunk_size=2048,
    ...
    def fused_gate_srw(x, h, emb_local, raw_tau,
        ...
        def route_rw_chunk(start):
            ...
            # v4.1.6.2: stored read/write params stay raw, but SRW
        ...
        # Soft annealed DirectTau. Fast train path does not scan rho
        ...
        # v4.1.6.2 so regular training does not require a rho-statistics pass.
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048,
    ...
    Soft annealed DirectTau gate: sigmoid((rho - tau) / T) * intensity.
...
class DAWN(nn.Module):
    """DAWN-SRW v4.1.6.2 with soft annealed tau gates, Attention Layers, and RST Layers."""
    __version__ = "spatial-r1-v4.1.6.2"
...
def _angular_gate_kwargs_from_model_cfg(model_cfg):
    """Extract angular Select/Intensity and soft-gate settings for inference."""
    ...
    if d_select_cfg is None:
        ...
        if intensity_route_dim <= 0:
            raise ValueError(
                "v4.1.6.2 angular inference requires model.d_select "
    ...
    # so they use the explicit final soft-gate defaults unless the caller
    # supplies current values through model_cfg.
    return {
        'd_select': d_select,
        'intensity_beta': float(model_cfg.get('intensity_beta', 0.5)),
        'soft_gate_temperature': float(
            model_cfg.get('soft_gate_temperature', 0.07)),
        'tau_ce_grad_scale': float(model_cfg.get('tau_ce_grad_scale', 0.0)),
...
def _angular_gate(h, emb, raw_tau, raw_scan_offset=None, d_select=None,
                  ...
                  tau_ce_grad_scale=0.0, execution_prune_eps=0.0,
                  soft_gate_effective_active_eps=1.0e-6):
    """Canonical v4162 soft gate for non-sharded inference helpers."""
...
def _srw_inference(x, h, emb, raw_tau, raw_scan_offset, w_read, w_write,
    ...
    # v4.1.6.2: inference uses read/write directions; params stay raw.
...
def _srw_inference_with_gates(x, h, emb, raw_tau, raw_scan_offset, w_read,
    ...
    # v4.1.6.2: analysis inference uses read/write directions; params stay raw.
    r_n = _forward_unit_direction(w_read.astype(jnp.float32))
    w_n = _forward_unit_direction(w_write.astype(jnp.float32))
    gate = _angular_gate(
        h, emb, raw_tau, raw_scan_offset, **angular_gate_kwargs)
...
def build_suppressed_forward(params, model_cfg, suppress_masks):
    ...
    def _srw_sup(x, h, emb, tau_off, raw_scan_offset, w_read, w_write, mult):
        ...
        # v4.1.6.2: suppressed forward uses read/write directions.
        r_n = _forward_unit_direction(w_read.astype(jnp.float32))
        w_n = _forward_unit_direction(w_write.astype(jnp.float32))
        gate = _angular_gate(
