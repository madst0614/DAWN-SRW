...
#   tau_sg           = stop_gradient(tau)
#   margin, r        = rho - tau, margin / boundary_scale
#   d_neg            = lambda_neg * softplus(-r / lambda_neg)
#   admission        = exp(-(d_neg ** boundary_power))
#   drive            = softplus(margin / B) * B / max(1 - tau_sg, eps)
...
def _boundary_soft_weight_from_margin(margin, boundary_scale, boundary_power):
    ...
    return jnp.exp(-jnp.power(d_neg, boundary_power))


def _drive_scale_from_tau(tau_sg, depth_scale, eps=1.0e-6):
    depth_scale = jnp.maximum(
        jnp.asarray(depth_scale, dtype=jnp.float32),
        jnp.float32(1.0e-4))
    tau_sg = jnp.asarray(tau_sg, dtype=jnp.float32)
    return depth_scale / jnp.maximum(
        jnp.float32(1.0) - tau_sg,
        jnp.float32(eps))


def _drive_from_margin(margin, depth_scale, scale):
    depth_scale = jnp.maximum(
        jnp.asarray(depth_scale, dtype=jnp.float32),
        jnp.float32(1.0e-4))
    return jax.nn.softplus(margin / depth_scale) * scale
...
def _compute_admission_depth(score, tau, boundary_scale,
    ...
    return margin, admission, drive, weight, active_mask
...
class DAWN(nn.Module):
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        result = {
            ...
            'load_mean': (
                attn_den_cost_mean_all.mean() + rst_den_cost_mean_all.mean()) / 2.0,
            'attn_load_mean': attn_den_cost_mean_all.mean(),
            'attn_qk_load_mean': _attn_core_mean(
                ATTN_SPLIT_QK_GATE_DEN_SUM_MEAN),
            'attn_v_load_mean': _attn_core_mean(
                ATTN_SPLIT_V_GATE_DEN_SUM_MEAN),
            ...
            'attn_qk_drive_max': _attn_core_max(ATTN_SPLIT_QK_INT_MAX),
            'attn_v_drive_max': _attn_core_max(ATTN_SPLIT_V_INT_MAX),
            'rst_drive_max': rst_int_max_all.max(),
            'weight_mean': (
                _attn_core_mean(ATTN_SPLIT_QK_GATE_SUM)
                / jnp.maximum(jnp.float32(self.n_qk), jnp.float32(1.0))
                + _attn_core_mean(ATTN_SPLIT_V_GATE_SUM)
                / jnp.maximum(jnp.float32(self.n_v), jnp.float32(1.0))
                + rst_gsum_all.mean()
                / jnp.maximum(jnp.float32(n_rst_eff), jnp.float32(1.0))
            ) / jnp.float32(3.0),
            'attn_qk_weight_mean': (
                _attn_core_mean(ATTN_SPLIT_QK_GATE_SUM)
                / jnp.maximum(jnp.float32(self.n_qk), jnp.float32(1.0))),
            ...
            'rst_weight_mean': (
                rst_gsum_all.mean()
                / jnp.maximum(jnp.float32(n_rst_eff), jnp.float32(1.0))),
            'weight_max': jnp.maximum(
                attn_raw_gmax_all.max(), rst_raw_gmax_all.max()),
            'attn_weight_max': attn_raw_gmax_all.max(),
            'attn_qk_weight_max': _attn_core_max(
                ATTN_SPLIT_QK_RAW_GATE_MAX),
            'attn_v_weight_max': _attn_core_max(
                ATTN_SPLIT_V_RAW_GATE_MAX),
            'rst_weight_max': rst_raw_gmax_all.max(),
            'admission_mean': (
                _attn_core_mean(ATTN_SPLIT_QK_GATE_DEN_SUM_MEAN)
                / jnp.maximum(jnp.float32(self.n_qk), jnp.float32(1.0))
                + _attn_core_mean(ATTN_SPLIT_V_GATE_DEN_SUM_MEAN)
                / jnp.maximum(jnp.float32(self.n_v), jnp.float32(1.0))
                + rst_den_cost_mean_all.mean()
                / jnp.maximum(jnp.float32(n_rst_eff), jnp.float32(1.0))
            ) / jnp.float32(3.0),
            'attn_qk_admission_mean': (
                _attn_core_mean(ATTN_SPLIT_QK_GATE_DEN_SUM_MEAN)
                / jnp.maximum(jnp.float32(self.n_qk), jnp.float32(1.0))),
            'attn_v_admission_mean': (
                _attn_core_mean(ATTN_SPLIT_V_GATE_DEN_SUM_MEAN)
                / jnp.maximum(jnp.float32(self.n_v), jnp.float32(1.0))),
            'rst_admission_mean': (
                rst_den_cost_mean_all.mean()
                / jnp.maximum(jnp.float32(n_rst_eff), jnp.float32(1.0))),
            'admission_max': jnp.maximum(
                jnp.maximum(
                    _exposure_max(
                        attn_qk_exposure_diag_all, DEAD_EXPOSURE_MAX),
                    _exposure_max(
                        attn_v_exposure_diag_all, DEAD_EXPOSURE_MAX)),
                rst_angular_exposure_max_all.max()),
            'attn_qk_admission_max': _exposure_max(
                attn_qk_exposure_diag_all, DEAD_EXPOSURE_MAX),
            'attn_v_admission_max': _exposure_max(
                attn_v_exposure_diag_all, DEAD_EXPOSURE_MAX),
            'rst_admission_max': rst_angular_exposure_max_all.max(),
...
def _angular_gate_kwargs_from_model_cfg(model_cfg):
    """Extract v4165 FairDrive/TauSG boundary settings for inference."""
    ...
    return {
        ...
        'drive_mix': float(model_cfg.get('drive_mix', 1.0)),
        'drive_ref': float(model_cfg.get('drive_ref', 1.0)),
    }
...
def _tau_init_calibration_scores(params, input_ids, max_tokens=128):
    ...
    states and the shared v4165 router/pool parameters. Rho follows the
...
def _angular_gate(h, emb, raw_tau, raw_scan_offset=None,
                  ...
                  drive_mix=1.0,
                  drive_ref=1.0):
    """Canonical v4165 weight for non-sharded inference helpers."""
