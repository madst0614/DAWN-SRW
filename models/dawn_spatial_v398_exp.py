"""
DAWN-Spatial v3.9.8: Sense-Read-Write (JAX/Flax)
...
def threshold_gate(scores, tau_offset):
    ...
    gate = jax.nn.sigmoid(raw.astype(jnp.float32) / s_std)
    active_n = (gate > 0.5).sum(axis=-1, keepdims=True).astype(jnp.float32)
...
def make_sharded_srw(mesh, max_chunk_size=2048):
    ...
    def fused_gate_srw(x, h, emb_local, tau_offset, read_local, write_local):
        ...
        def gate_srw_step(carry, i):
            ...
            gate = jax.nn.sigmoid(raw / s_std)
            gate_bf = gate.astype(jnp.bfloat16)
            xr = x_bf @ rc.T
            c_out = ((gate_bf * xr) @ wc).astype(jnp.float32)
            xr_f32 = xr.astype(jnp.float32)
            chunk_weighted = (gate * xr_f32 ** 2).sum(axis=-1, keepdims=True)
            chunk_active = (gate > 0.5).astype(jnp.float32).sum(axis=-1, keepdims=True)
            return (out + c_out,
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048):
    ...
    def fused_gate_srw_paired(x, h, emb_local, tau_offset, read_local, write_local):
        ...
        def gate_srw_step(carry, i):
            ...
            gate = jax.nn.sigmoid(raw / s_std)  # s_std [B,S,2,1] broadcasts to [B,S,2,N]
            gate_bf = gate.astype(jnp.bfloat16)
            xr = x_bf @ rc.T  # [B,S,N]
            c_out = jnp.einsum('bsrn,nd->bsrd', gate_bf * xr[:, :, None, :], wc).astype(jnp.float32)
            xr_f32 = xr.astype(jnp.float32)
            chunk_weighted = (gate * (xr_f32 ** 2)[:, :, None, :]).sum(axis=-1, keepdims=True)  # [B,S,2,1]
            chunk_active = (gate > 0.5).astype(jnp.float32).sum(axis=-1, keepdims=True)
            return (out + c_out,
                    total_weighted_cost + chunk_weighted,
                    jnp.maximum(total_gate_max, gate.max(axis=-1, keepdims=True)),
...
def _srw_inference(x, h, emb_norm, tau_offset, w_read, w_write):
    ...
    xr_f32 = xr.astype(jnp.float32)
    weighted = (gate * xr_f32 ** 2).sum(axis=-1, keepdims=True)
    den = weighted + 1e-6
    den = jnp.maximum(den, 1e-3)
