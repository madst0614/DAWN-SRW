"""
DAWN-SRW v4.1.7.0 Canonical RW-Relation SRW

Official v4170 model path.

Implemented concepts:
- cosine-space tau reference with bounded sigmoid min/max mapping
- compressed RW outer-relation live-gradient operator-key selection
...
def _effective_pool_output_scales(pool_params, d_model, n_layers):
    ...
    v4170 has no learned pool scale parameters; scale is fixed by d_model/layers.
    """
...
MODEL_VERSION = "spatial-r1-v4.1.7.0"
OPERATOR_KEY_MODE = "rw_role_outer_relation"
...
def _rw_relation_operator_key(
        read, write, read_role_proj, write_role_proj, *, eps=1e-6):
    """Compressed live-gradient RW relation map for v4170 selection.
    ...
    # Axis order is [operator, write_role, read_role]. C-order flattening
    # therefore produces the canonical [write_role_axis, read_role_axis] key.
    relation = jnp.einsum('nw,nr->nwr', write_role, read_role)
    op_key = relation.reshape(
...
def _boundary_gate_from_margin(margin, tau, boundary_power=None):
    """Compatibility hook for diagnostics; v4170 has no projected gate."""
    del boundary_power
...
def _compute_admission_drive(score, tau, boundary_scale,
    ...
    """v4170 canonical linear angular-depth execution.
    ...
    tau = jnp.asarray(tau, dtype=jnp.float32)
    margin = score - tau
    angular_depth_unpruned = _linear_angular_depth_from_margin(margin, tau)
...
def make_sharded_srw(mesh, max_chunk_size=2048,
    ...
    v4170 canonical DirectTau execution:
        rho              = cosine(q, RW-derived operator key)
        margin           = rho - tau
    ...
    del admission_den_power, admission_den_grad_scale
    ...
    def fused_gate_srw(x, h, op_key_local, raw_tau,
        ...
        # v4170 does not require a rho-statistics pass for regular train.
        ...
        def angular_compose_parts(rho, valid_mask):
            (selection_margin, admission, angular_depth, execution_weight,
            ...
            angular_depth = jnp.where(valid_mask, angular_depth, 0.0)
    ...
    def fused_gate_srw_minimal(x, h, op_key_local, raw_tau,
        ...
        def gate_srw_step(carry, i):
            (raw_out, total_gate_mass, total_gate_sq,
             total_gate_max, total_active_count) = carry
            s = i * cs
            op_key, rc, wc, valid_chunk = op_key_rw_chunk(s)
            valid_bsn = valid_chunk[None, None, :]
        ...
        (raw_out, total_gate_mass, total_gate_sq,
         total_gate_max, total_active_count), _ = jax.lax.scan(
            gate_srw_step,
            (jnp.zeros((B, S, D), dtype=jnp.float32),
        ...
        gate_den = jnp.maximum(global_gate_mass, 1.0)
        ...
        depth_active_mean = jnp.where(
            ...
            global_gate_mass / jnp.maximum(global_active_count, 1.0),
            0.0).mean()
        ...
        top1_gate_frac_mean = jnp.where(
            ...
            0.0).mean()
        den_floor_frac = (global_gate_mass < 1.0).astype(jnp.float32).mean()
...
def make_sharded_srw_paired_minimal(mesh, max_chunk_size=2048,
    ...
    def fused_gate_srw_paired_minimal(x, h, op_key_local, raw_tau,
        ...
        top1_gate_frac = jnp.where(
            ...
            global_gate_max / jnp.maximum(global_gate_mass, 1.0e-8),
            0.0)
        den_floor = (global_gate_mass < 1.0).astype(jnp.float32)
...
class NeuronPool(nn.Module):
    ...
    def setup(self):
        ...
        product = read_dim * write_dim
        if read_dim <= 0 or write_dim <= 0 or product != int(self.d_route):
            raise ValueError(
                "Invalid v4170 role factorization: "
                f"model.rw_role_read_dim={read_dim}, "
                f"model.rw_role_write_dim={write_dim}, "
                f"model.d_route={self.d_route}, product={product}")
...
class Router(nn.Module):
    ...
    def setup(self):
        ...
        if missing_tau:
            raise ValueError(
                "v4170 requires explicit cosine-space tau_init_attn_qk/v/rst; "
...
def _attn_forward_minimal(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    (QK_out,
     ...
     q_tau_mean, k_tau_mean) = fused_paired(
    ...
    (V, v_active_frac, v_active_n_mean, v_gate_mass_mean, v_gate_den_mean,
     ...
     v_den_floor_frac, v_tau_mean) = fused_single_v(
...
def _rst_forward_minimal(x, pool_params, router_params, rng,
    ...
    (out, rst_active_frac, rst_active_n_mean, rst_gate_mass_mean,
     ...
     rst_top1_gate_frac_mean, rst_den_floor_frac, rst_tau_mean) = fused_single(
...
class DAWN_SRW_V4170(nn.Module):
    """DAWN-SRW v4.1.7.0 with RW-relation keys and direct queries."""
    ...
    def setup(self):
        ...
        product = (
            int(read_dim) * int(write_dim)
            if read_dim is not None and write_dim is not None else None)
        if (read_dim is None or write_dim is None
                or int(read_dim) <= 0 or int(write_dim) <= 0
                or product != int(self.d_route)):
            raise ValueError(
                "Invalid v4170 role factorization: "
                f"model.rw_role_read_dim={read_dim}, "
                f"model.rw_role_write_dim={write_dim}, "
            ...
            if minimal_train:
                def scan_body_minimal(carry, xs):
                    ...
                    (attn_out, q_active_frac, k_active_frac, v_active_frac,
                     ...
                     attn_out_norm) = _attn_forward_minimal(
                    ...
                    (rst_out, rst_active_frac, rst_active_n_mean,
                     ...
                     rst_tau_mean, rst_out_norm) = _rst_forward_minimal(
    ...
    def get_model_info(self):
        ...
        return [
            ...
            "  Relation map: "
            f"{self.rw_role_write_dim}x{self.rw_role_read_dim} "
            "[write,read]",
            ...
            "  Selection: live-gradient compressed RW relation maps with "
            "direct state-to-operation queries",
            "  Gate: linear angular-depth DirectTau; den=max(sum(gate), 1)",
...
def _angular_execution_kwargs_from_model_cfg(model_cfg):
    """Extract v4170 canonical execution settings for inference."""
    return {
...
def _tau_init_calibration_scores(params, input_ids, max_tokens=128):
    ...
    states and the shared v4170 router/pool parameters. Rho follows the
    train path exactly: direct state projections against live-gradient
    compressed RW outer-relation keys.
    """
...
def _query_geometry_diagnostics(params, input_ids, max_tokens=4096):
    """Heavy v4170 query geometry on deterministic first-block tokens."""
    max_tokens = int(max_tokens)
    input_ids = jnp.asarray(input_ids, dtype=jnp.int32)
...
def _split_admission_den_kwargs(angular_execution_kwargs):
    ...
    execution_kwargs.pop('admission_den_power', None)
    admission_den_power = jnp.float32(1.0)
...
def build_suppressed_forward(params, model_cfg, suppress_masks):
    ...
    def _srw_sup(x, h, op_key, tau_off, raw_scan_offset, w_read, w_write, mult):
        ...
        out = a @ w_n
        del admission_den_power
        admission_den = jnp.maximum(
            admission.sum(axis=-1, keepdims=True), 1.0)
