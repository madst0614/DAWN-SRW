...
### Denominator
    ...
    den = 1.0 + sum(activation_i) + sum(sqrt(a_i^2 + epsilon^2) - epsilon)

  v4.1.4 uses activation as participation cost and smooth abs(a_i)
  as current cost, while the numerator uses unit write vectors.
...
def make_sharded_srw(mesh, max_chunk_size=2048, dead_threshold=0.01,
    ...
    def fused_gate_srw(x, h, emb_local, tau_offset, scan_bias, read_local, write_local):
        ...
        # emb used as-is (v4.0.6-style). ||emb|| is a learnable DoF ??        # WD controls size; per-neuron magnitude drives competitive
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                rc = (rc.astype(jnp.float32) / (
                    jnp.linalg.norm(rc.astype(jnp.float32), axis=-1, keepdims=True) + 1e-8)
                      ).astype(jnp.bfloat16)
                wc = (wc.astype(jnp.float32) / (
                    jnp.linalg.norm(wc.astype(jnp.float32), axis=-1, keepdims=True) + 1e-8)
                      ).astype(jnp.bfloat16)
                scores = h_bf @ ec.T
                scores_f = scores.astype(jnp.float32)
                raw = scores_f - tau
                ...
                a_cost = jnp.sqrt(a * a + _eps ** 2) - _eps
                ...
                chunk_activation_cost = activation.sum(axis=-1, keepdims=True)
                chunk_current_cost = a_cost.sum(axis=-1, keepdims=True)
                chunk_den_cost = chunk_activation_cost + chunk_current_cost
                ...
                return (out + c_out,
                        ...
                        total_activation_cost + chunk_activation_cost,
...
class DAWN(nn.Module):
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        result = {
            ...
            'attn_den_cost_mean': attn_den_cost_mean_all.mean(),
            'know_den_cost_mean': know_den_cost_mean_all.mean(),
            'attn_act_cost_mean': attn_activation_cost_mean_all.mean(),
            'know_act_cost_mean': know_activation_cost_mean_all.mean(),
            'attn_current_cost_mean': attn_current_cost_mean_all.mean(),
            'know_current_cost_mean': know_current_cost_mean_all.mean(),
        }
        ...
        if analysis and not self.is_initializing():
            ...
            result.update({
                ...
                'attn_den_cost': attn_den_cost_all.mean(),
                'know_den_cost': know_den_cost_all.mean(),
                'attn_activation_cost': attn_activation_cost_all.mean(),
                'know_activation_cost': know_activation_cost_all.mean(),
                'attn_current_cost': attn_current_cost_all.mean(),
                'know_current_cost': know_current_cost_all.mean(),
...
def _srw_inference(x, h, emb_norm, tau_offset, scan_bias, w_read, w_write):
    """Non-chunked SRW for inference with the v4.1.4 denominator."""
    scores = h @ emb_norm.T
    ...
    a = gate * xr
    raw_out = a @ w_n
    a_cost = jnp.sqrt(a * a + EPSILON * EPSILON) - EPSILON
    den = 1.0 + activation.sum(axis=-1, keepdims=True) + a_cost.sum(axis=-1, keepdims=True)
