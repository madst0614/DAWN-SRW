"""
DAWN-SRW v4.1.6.9 Canonical RW-Matched SRW

Official v4169 model path.
...
def _effective_pool_output_scales(pool_params, d_model, n_layers):
    ...
    v4169 has no learned pool scale parameters; scale is fixed by d_model/layers.
    """
...
MODEL_VERSION = "spatial-r1-v4.1.6.9"
...
def _boundary_gate_from_margin(margin, tau, boundary_power=None):
    """Compatibility hook for diagnostics; v4169 has no projected gate."""
    del boundary_power
...
def _compute_admission_drive(score, tau, boundary_scale,
    ...
    """v4169 canonical linear angular-depth execution.
...
def make_sharded_srw(mesh, max_chunk_size=2048,
    ...
    v4169 canonical DirectTau execution:
        rho              = cosine(q, RW-derived operator key)
        margin           = rho - tau
    ...
    def fused_gate_srw(x, h, op_key_local, raw_tau,
        ...
        # v4169 does not require a rho-statistics pass for regular train.
...
class Router(nn.Module):
    ...
    def setup(self):
        ...
        if missing_tau:
            raise ValueError(
                "v4169 requires explicit cosine-space tau_init_attn_qk/v/rst; "
...
class DAWN_SRW_V4169(nn.Module):
    """DAWN-SRW v4.1.6.9 with RW-matched keys and linear SRW gating."""
...
def _angular_execution_kwargs_from_model_cfg(model_cfg):
    """Extract v4169 canonical execution settings for inference."""
    return {
