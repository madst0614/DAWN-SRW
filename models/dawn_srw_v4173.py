"""DAWN-SRW v4.1.7.3 operation-projection execution architecture.
...
def _effective_pool_output_scales(pool_params, d_model, n_layers):
    ...
    v4173 has no learned pool scale parameters; scale is fixed by d_model/layers.
    """
...
MODEL_VERSION = "spatial-r1-v4.1.7.3"
...
_V4173_SRW_COMPOSITION_MODES = frozenset((
...
def _validate_v4173_srw_composition_mode(value, *, context="v4173"):
    """Validate the static Python-side v4173 SRW composition selector."""
    if isinstance(value, jax.core.Tracer):
        raise ValueError(
            f"{context} srw_composition_mode must be a static Python string; "
            "v4173 does not support a traced or dynamic composition mode")
    if not isinstance(value, str):
        raise ValueError(
            f"{context} srw_composition_mode must be one of "
            f"{sorted(_V4173_SRW_COMPOSITION_MODES)}, got {value!r}")
    if value not in _V4173_SRW_COMPOSITION_MODES:
        raise ValueError(
            f"{context} unsupported srw_composition_mode={value!r}; "
            f"expected one of {sorted(_V4173_SRW_COMPOSITION_MODES)}")
    return value


def _validate_v4173_admission_den_power(value, *, context="v4173"):
    ...
    if isinstance(value, jax.core.Tracer):
        raise ValueError(
            ...
            "v4173 does not support a traced or scheduled denominator power")
...
def _resolve_v417x_admission_den_powers(
    ...
    legacy = _validate_v4173_admission_den_power(
    ...
    for pool, value in (
        ...
        resolved.append(_validate_v4173_admission_den_power(
            legacy if value is None else value,
            context=f"{context}.admission_den_power_{pool}"))
    return legacy, *resolved


def _validate_v4173_heat_kernel_beta(value, *, context="v4173"):
...
def _validate_v4173_composition_settings(
        srw_composition_mode, admission_den_power,
        heat_kernel_beta=DEFAULT_HEAT_KERNEL_BETA, *, context="v4173"):
    mode = _validate_v4173_srw_composition_mode(
        srw_composition_mode, context=context)
    power = _validate_v4173_admission_den_power(
        admission_den_power, context=context)
    beta = _validate_v4173_heat_kernel_beta(
        heat_kernel_beta, context=context)
    return mode, power, beta


def _validate_v4173_admission_den_grad_scale(value, *, context="v4173"):
    if isinstance(value, jax.core.Tracer):
        raise ValueError(
...
def _mark_v4173_srw_factory_output(
    ...
    srw_composition_mode, admission_den_power, heat_kernel_beta = (
        _validate_v4173_composition_settings(
            srw_composition_mode, admission_den_power,
            heat_kernel_beta,
            context="v4173 sharded factory metadata"))
    fn._v4173_srw_composition_mode = srw_composition_mode
    fn._v4173_admission_den_power = float(admission_den_power)
    fn._v4173_heat_kernel_beta = float(heat_kernel_beta)
    fn._v4173_admission_den_grad_scale = 1.0
    return fn


def _validate_v4173_sharded_fns(
    ...
    for name, (pool, route_power) in wrapper_pools.items():
        ...
        if fn is None:
            continue
        actual_mode = getattr(fn, '_v4173_srw_composition_mode', None)
        if actual_mode is None:
            raise ValueError(
                f"v4173 sharded function {name!r} is missing canonical "
                "srw_composition_mode metadata")
        actual_mode = _validate_v4173_srw_composition_mode(
            actual_mode, context=f"v4173 sharded_fns[{name!r}]")
        if actual_mode != expected_mode:
            raise ValueError(
                "v4173 model/sharded closure srw_composition_mode mismatch: "
                f"sharded_fns[{name!r}]={actual_mode!r}, "
                f"model={expected_mode!r}")
        actual = getattr(fn, '_v4173_admission_den_power', None)
        if actual is None:
            raise ValueError(
                f"v4173 sharded function {name!r} is missing canonical "
        ...
        if float(actual) != float(route_power):
            raise ValueError(
                "v4173 closure/runtime admission_den_power mismatch for "
                f"{pool} pool: sharded_fns[{name!r}]={actual}, "
                f"runtime={route_power}")
        actual_beta = getattr(fn, '_v4173_heat_kernel_beta', None)
        if actual_beta is None:
            raise ValueError(
                f"v4173 sharded function {name!r} is missing canonical "
                "heat_kernel_beta metadata")
        actual_beta = _validate_v4173_heat_kernel_beta(
            actual_beta, context=f"v4173 sharded_fns[{name!r}]")
        if actual_beta != expected_heat_kernel_beta:
            raise ValueError(
                "v4173 model/sharded closure heat_kernel_beta mismatch: "
                f"sharded_fns[{name!r}]={actual_beta}, "
                f"model={expected_heat_kernel_beta}")
    profile = sharded_fns.get('_v4173_kernel_profile')
    if profile is not None and profile not in {
            'production', 'production_diagnostics',
            'retention', 'suppression', 'trajectory'}:
        raise ValueError(f"unknown v4173 minimal kernel profile: {profile!r}")
...
def _operation_projection_read(operator_query, read_directions):
    """Read unnormalized execution amplitudes from a single raw route query."""
    operator_query_bf = operator_query.astype(jnp.bfloat16)
    read_directions_bf = read_directions.astype(jnp.bfloat16)
    if operator_query.ndim != 3 or read_directions.ndim != 2:
        raise ValueError(
            "v4173 single-route read requires query [B,S,d_route] and read "
            f"[N,d_route], got query={operator_query.shape}, "
            f"read={read_directions.shape}")
    if int(operator_query.shape[-1]) != int(read_directions.shape[-1]):
        raise ValueError(
            "v4173 single-route read width mismatch: "
            f"query={operator_query.shape}, read={read_directions.shape}")
    return (operator_query_bf @ read_directions_bf.T).astype(jnp.float32)


def _operation_projection_read_paired(operator_queries, read_directions):
    """Read independent Q and K amplitudes from raw paired route queries."""
    if operator_queries.ndim != 4 or read_directions.ndim != 2:
        raise ValueError(
            "v4173 paired read requires queries [B,S,2,d_route] and read "
            f"[N,d_route], got queries={operator_queries.shape}, "
            f"read={read_directions.shape}")
    if int(operator_queries.shape[2]) != 2:
        raise ValueError(
            "v4173 paired read route axis must contain Q and K, got "
            f"{operator_queries.shape}")
    if int(operator_queries.shape[-1]) != int(read_directions.shape[-1]):
        raise ValueError(
            "v4173 paired read width mismatch: "
            f"queries={operator_queries.shape}, read={read_directions.shape}")
    return jnp.einsum(
        'bsrd,nd->bsrn',
        operator_queries.astype(jnp.bfloat16),
        read_directions.astype(jnp.bfloat16)).astype(jnp.float32)
...
def _generalized_bilinear_operator_key_components(
    ...
    if read_vectors.ndim != 2 or write_vectors.ndim != 2:
        raise ValueError(
            "v4173 generalized bilinear read/write must have rank-2 "
            f"[N, d] shapes, got read={read_vectors.shape}, "
            f"write={write_vectors.shape}")
    if int(read_vectors.shape[0]) != int(write_vectors.shape[0]):
        raise ValueError(
            "v4173 generalized bilinear read/write operator row mismatch: "
            f"read={read_vectors.shape}, write={write_vectors.shape}")
    if read_probes.ndim != 2 or write_probes.ndim != 2:
        raise ValueError(
            "v4173 generalized bilinear probes must have rank 2 "
            "[d_input, d_route], "
            f"got read_probes={read_probes.shape}, "
            f"write_probes={write_probes.shape}")
    expected_read = int(read_vectors.shape[1])
    expected_write = int(write_vectors.shape[1])
    if expected_read != expected_write:
        raise ValueError(
            "v4173 generalized bilinear local RW width mismatch: "
            f"read={read_vectors.shape}, write={write_vectors.shape}")
    if (int(read_probes.shape[0]) != expected_read
            or int(write_probes.shape[0]) != expected_write
            or int(read_probes.shape[1]) != int(write_probes.shape[1])):
        raise ValueError(
            "v4173 generalized bilinear probe shape mismatch: expected "
            f"[{expected_read}, d_route] read and "
            f"[{expected_write}, d_route] write probes, got "
            f"read_probes={read_probes.shape}, "
            f"write_probes={write_probes.shape}")
...
def symbolic_parameter_count(model_cfg):
    """Return the exact parameter breakdown for fixed v4173 architecture."""
    ...
    if mode != OPERATOR_KEY_MODE:
        raise ValueError(
            "v4173 symbolic parameter count requires "
...
def _pool_operator_keys(pool_params, operator_key_mode=None):
    ...
    if mode == OPERATOR_KEY_MODE_LEARNED:
        ...
        if missing:
            raise ValueError(
                "v4173 neuron_pool is missing learned operator embeddings: "
                + ", ".join(missing))
        keys = {
        ...
        if (read_probe.ndim != 2 or write_probe.ndim != 2
                or tuple(read_probe.shape) != tuple(write_probe.shape)
                or int(read_probe.shape[0]) != int(read_probe.shape[1])):
            raise ValueError(
                "v4173 generalized bilinear probes must both have shape "
                "[d_route, d_route]")
        keys = {}
...
def _composition_den(
    ...
    srw_composition_mode = _validate_v4173_srw_composition_mode(
        srw_composition_mode, context="v4173 composition denominator")
...
def _composition_den_floor_mass(
        srw_composition_mode=DEFAULT_SRW_COMPOSITION_MODE):
    mode = _validate_v4173_srw_composition_mode(
        srw_composition_mode, context="v4173 composition denominator floor")
...
def _boundary_gate_from_margin(margin, tau, boundary_power=None):
    """Compatibility hook for diagnostics; v4173 has no projected gate."""
    del boundary_power
...
def _compute_admission_drive(score, tau, boundary_scale,
                             ...
                             srw_composition_mode=DEFAULT_SRW_COMPOSITION_MODE,
                             heat_kernel_beta=DEFAULT_HEAT_KERNEL_BETA):
    """v4173 canonical angular selection and mode-specific composition.
    ...
    srw_composition_mode = _validate_v4173_srw_composition_mode(
        srw_composition_mode, context="v4173 admission drive")
...
def make_sharded_srw(mesh, max_chunk_size=2048,
    ...
    v4173 canonical DirectTau execution:
        rho               = cosine(operator_query, operator_keys)
        margin            = rho - tau
        angular_amplitude = clip(margin / max(1 - tau, 1e-4), 0, 1)
    ...
    def fused_gate_srw(x, operator_query, operator_keys_local, raw_tau,
        ...
        # v4173 does not require a rho-statistics pass for regular train.
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                xr_f = _operation_projection_read(operator_query, rc)
    ...
    return _mark_v4173_srw_factory_output(
...
def make_sharded_srw_paired(mesh, max_chunk_size=2048,
    ...
    raw_tau is [B,S,2,1].
    Q and K read independently from their own raw operation projections.
    Scores stats computed independently per route.
    Returns out [B,S,2,D], active [B,S,1], gate_max [B,S,1].

    v4173 canonical execution uses the same statically selected composition
    ...
    def fused_gate_srw_paired(x, operator_query, operator_keys_local, raw_tau,
        ...
        if analysis:
            ...
            def gate_srw_step(carry, i):
                ...
                chunk_int_max = angular_amplitude.max()
                chunk_int_cap_count = jnp.float32(0.0)
                xr_f = _operation_projection_read_paired(operator_query, rc)
                a = execution_weight * xr_f
...
_V4173_MINIMAL_KERNEL_BUNDLES = {}


def _v4173_minimal_bundle_key(
...
def _cached_v4173_minimal_bundle(route_kind, builder, *factory_args):
    key = _v4173_minimal_bundle_key(route_kind, *factory_args)
    bundle = _V4173_MINIMAL_KERNEL_BUNDLES.get(key)
    if bundle is None:
        bundle = builder(*factory_args)
        _V4173_MINIMAL_KERNEL_BUNDLES[key] = bundle
    return bundle
...
def _make_sharded_srw_training_fast_minimal_impl(
    ...
    def training_core(
        ...
        def gate_srw_step(carry, chunk_index):
            ...
            read_value = _operation_projection_read(
                operator_query, read_chunk)
        ...
        (raw_out, total_gate_mass), _ = jax.lax.scan(
            ...
            (
                jnp.zeros(
                    (batch_size, sequence_length, d_local),
                    dtype=jnp.float32),
                jnp.zeros(
    ...
    _mark_v4173_srw_factory_output(
        ...
        heat_kernel_beta)
    training_kernel._v4173_kernel_profile = "production"
    training_kernel._v4173_production_shard_map_kernel = training_kernel
...
def _make_sharded_srw_production_diagnostics_minimal_impl(
    ...
    def production_core(
        ...
        initial_carry = (
            jnp.zeros(
                (batch_size, sequence_length, d_local), dtype=jnp.float32),
            jnp.zeros(
                (batch_size, sequence_length, 1), dtype=jnp.float32),
            jnp.zeros(
                (batch_size, sequence_length, 1), dtype=jnp.float32),
            jnp.zeros(
                (batch_size, sequence_length, 1), dtype=jnp.float32),
        ...
        (raw_out, total_gate_mass, total_gate_sq,
         ...
         total_angular_amplitude), _ = jax.lax.scan(
             gate_srw_step, initial_carry, jnp.arange(n_chunks))
        ...
        return (
            ...
            jax.lax.stop_gradient(
                normalized_srw_out_norm.astype(jnp.float32)),
        )
    ...
    _mark_v4173_srw_factory_output(
        ...
        heat_kernel_beta)
    production_kernel._v4173_kernel_profile = "production_diagnostics"
    production_kernel._v4173_diagnostics_shard_map_kernel = production_kernel
...
def _make_sharded_srw_minimal_impl(
    ...
    def _sharded_srw_minimal_core(
        ...
        def gate_srw_step(carry, i):
            ...
            xr = _operation_projection_read(operator_query, rc)
            a = execution_for_numerator * xr
            c_out = (a.astype(jnp.bfloat16) @ wc).astype(jnp.float32)
    ...
    for wrapper in (
        ...
        _mark_v4173_srw_factory_output(
            ...
            heat_kernel_beta)
        wrapper._v4173_canonical_shard_map_kernel = canonical_single_kernel
        wrapper._v4173_canonical_factory_token = factory_token
    fused_gate_srw_trajectory_minimal._v4173_trajectory_shard_map_kernel = (
        canonical_single_trajectory_kernel)
    fused_gate_srw_trajectory_minimal._v4173_trajectory_capture_width = (
        _trajectory_capture_width)
    fused_gate_srw_minimal._v4173_suppression_wrapper = (
        fused_gate_srw_suppression_minimal)
    fused_gate_srw_suppression_minimal._v4173_production_wrapper = (
        fused_gate_srw_minimal)
    fused_gate_srw_trajectory_minimal._v4173_production_wrapper = (
        fused_gate_srw_minimal)
...
def make_sharded_srw_minimal(mesh, max_chunk_size=2048,
    ...
    return _cached_v4173_minimal_bundle(
...
def make_sharded_srw_retention_minimal(
    ...
    kernel = _cached_v4173_minimal_bundle(
        "single_retention", _make_sharded_srw_minimal_impl,
        mesh, max_chunk_size, dead_exposure_target,
        soft_gate_effective_active_eps, admission_den_power,
        admission_den_grad_scale, srw_composition_mode,
        heat_kernel_beta)[0]
    kernel._v4173_kernel_profile = "retention"
    return kernel
...
def make_sharded_srw_suppression_minimal(
    ...
    kernel = _cached_v4173_minimal_bundle(
        ...
        admission_den_grad_scale, srw_composition_mode,
        heat_kernel_beta)[1]
    kernel._v4173_kernel_profile = "suppression"
    return kernel
...
def make_sharded_srw_trajectory_minimal(
    ...
    kernel._v4173_kernel_profile = "trajectory"
    kernel._v4173_production_wrapper._v4173_kernel_profile = "trajectory"
    return kernel
...
def _make_sharded_srw_paired_training_fast_minimal_impl(
    ...
    def training_core(
        ...
        def gate_srw_step(carry, chunk_index):
            ...
            execution_weight = jnp.where(
                valid_mask, execution_weight, 0.0)
            read_value = _operation_projection_read_paired(
                operator_query, read_chunk)
            amplitude = execution_weight * read_value
            chunk_out = jnp.einsum(
        ...
        (raw_out, total_gate_mass), _ = jax.lax.scan(
            ...
            (
                jnp.zeros(
                    (batch_size, sequence_length, 2, d_local),
                    dtype=jnp.float32),
...
def _make_sharded_srw_paired_production_diagnostics_minimal_impl(
    ...
    def production_core(
        ...
        q_active_frac = q_active_n_mean / jnp.maximum(
            global_operator_count, 1.0)
        k_active_frac = k_active_n_mean / jnp.maximum(
            global_operator_count, 1.0)
...
def _make_sharded_srw_paired_minimal_impl(
    ...
    def _sharded_srw_paired_minimal_core(
        ...
        def gate_srw_step(carry, i):
            ...
            admission_for_den = jnp.where(
                autonomous_retention & ~effective_keep,
                jnp.float32(0.0), admission)
            xr = _operation_projection_read_paired(operator_query, rc)
            a = execution_for_numerator * xr
            c_out = jnp.einsum(
                'bsrn,nd->bsrd',
            ...
            selected_execution = jnp.where(
                token_match & operator_match & route_match & valid_bsrn,
                execution_weight, jnp.float32(0.0))
            selected_a = selected_execution * xr
    ...
    for wrapper in (
        ...
        _mark_v4173_srw_factory_output(
            wrapper, admission_den_power, _srw_composition_mode,
            heat_kernel_beta)
        wrapper._v4173_canonical_shard_map_kernel = canonical_paired_kernel
        wrapper._v4173_canonical_factory_token = factory_token
    fused_gate_srw_paired_trajectory_minimal._v4173_trajectory_shard_map_kernel = (
        canonical_paired_trajectory_kernel)
    fused_gate_srw_paired_trajectory_minimal._v4173_trajectory_capture_width = (
        _trajectory_capture_width)
    fused_gate_srw_paired_minimal._v4173_suppression_wrapper = (
        fused_gate_srw_paired_suppression_minimal)
    fused_gate_srw_paired_suppression_minimal._v4173_production_wrapper = (
        fused_gate_srw_paired_minimal)
    fused_gate_srw_paired_trajectory_minimal._v4173_production_wrapper = (
        fused_gate_srw_paired_minimal)
...
class NeuronPool(nn.Module):
    ...
    def setup(self):
        ...
        if operator_key_mode != OPERATOR_KEY_MODE:
            raise ValueError(
                "v4173 NeuronPool requires generalized_bilinear_rw, got "
                f"{operator_key_mode!r}")
        if d_route <= 0:
            raise ValueError(
                f"v4173 model.d_route must be > 0, got {d_route}")
        ...
        self.attn_qk_write = self.param(
            'attn_qk_write', unit_norm_init(), (self.n_qk, d_route))
        ...
        self.rw_key_read_probe = self.param(
            'rw_key_read_probe', probe_init, (d_route, d_route))
        self.rw_key_write_probe = self.param(
            'rw_key_write_probe', probe_init, (d_route, d_route))
...
def _v4173_minimal_kernel_profile(sharded_fns, route_key):
    if not isinstance(sharded_fns, dict):
        return None
    profile = sharded_fns.get('_v4173_kernel_profile')
    ...
    return getattr(fn, '_v4173_kernel_profile', None)
...
def _attn_forward_training_fast(
    ...
    if any(getattr(fn, '_v4173_kernel_profile', None) != 'production'
    ...
    value_local = fused_single_v(
        ...
        soft_gate_boundary_power, soft_gate_boundary_power_final,
        execution_prune_eps)
    qk_state = (qk_local * qk_scale) @ router_params['up_qk']['kernel']
    value_state = (value_local * v_scale) @ router_params['up_v']['kernel']
    query = qk_state[:, :, 0, :]
    key = qk_state[:, :, 1, :]
    value = value_state
...
def _attn_forward_production_diagnostics(
    ...
    if not isinstance(sharded_fns, dict):
        raise ValueError("production minimal attention requires dict sharded_fns")
    fused_paired = sharded_fns.get('attn_qk_paired_minimal')
    fused_single_v = sharded_fns.get('attn_v_single_minimal')
    if fused_paired is None or fused_single_v is None:
        raise ValueError(
            "production minimal attention requires pure QK and V kernels")
    if any(getattr(fn, '_v4173_kernel_profile', None)
    ...
    (qk_state_transitions,
     ...
     q_normalized_srw_out_norm,
     k_normalized_srw_out_norm) = qk_result
    ...
    qk_state_transitions = (
        qk_state_transitions * qk_scale) @ router_params['up_qk']['kernel']
    attention_v = (
        attention_v * v_scale) @ router_params['up_v']['kernel']
    attention_q = qk_state_transitions[:, :, 0, :]
    attention_k = qk_state_transitions[:, :, 1, :]
    q_debug = attention_q
    k_debug = attention_k
    v_debug = attention_v
    d_head = d_model // n_heads
    ...
    zero_contribution = jnp.zeros(
        (batch_size, d_hidden), dtype=jnp.float32)
    result = (
        out,
        q_active_frac, k_active_frac, v_active_frac,
        ...
        q_depth_active_mean, k_depth_active_mean, v_depth_active_mean,
        q_gate_eff_n_mean, k_gate_eff_n_mean, v_gate_eff_n_mean,
        q_top1_gate_frac_mean, k_top1_gate_frac_mean,
        ...
        jnp.float32(0.5) * (
            jnp.linalg.norm(q_debug.astype(jnp.float32), axis=-1).mean()
            + jnp.linalg.norm(k_debug.astype(jnp.float32), axis=-1).mean()),
        v_admission_mass_max,
        v_composition_den_min, v_composition_den_max,
        v_raw_srw_out_norm, v_normalized_srw_out_norm,
        jnp.linalg.norm(v_debug.astype(jnp.float32), axis=-1).mean(),
        zero_contribution, zero_contribution, zero_contribution,
        zero_contribution, zero_contribution, zero_contribution,
    )
    if parity_debug:
        return result + ((
...
def _attn_forward_analysis_minimal(x, pool_params, router_params, expand_O_kernel, rng,
                          n_qk, n_v,
    ...
    canonical_paired = getattr(
        fused_paired, '_v4173_canonical_shard_map_kernel', None)
    canonical_single_v = getattr(
        fused_single_v, '_v4173_canonical_shard_map_kernel', None)
    if canonical_paired is None or canonical_single_v is None:
        raise ValueError(
            "minimal attention requires canonical v4173 shard-map kernels")
    ...
    qk_up = router_params['up_qk']['kernel']
    v_up = router_params['up_v']['kernel']
    attention_q = (
        qk_state_transitions[:, :, 0, :] * qk_scale) @ qk_up
    attention_k = (
        qk_state_transitions[:, :, 1, :] * qk_scale) @ qk_up
    attention_v = (attention_v * v_scale) @ v_up
    suppressed_q = (
        qk_suppressed_transitions[:, :, 0, :] * qk_scale) @ qk_up
    suppressed_k = (
        qk_suppressed_transitions[:, :, 1, :] * qk_scale) @ qk_up
    suppressed_v = (v_suppressed_transition * v_scale) @ v_up
    ...
    if analysis_trajectory_enabled:
        qk_trajectory_trace = tuple(
            (value @ qk_up if index < 4 else value)
            for index, value in enumerate(qk_trajectory_trace))
        v_trajectory_trace = tuple(
            (value @ v_up if index < 4 else value)
            for index, value in enumerate(v_trajectory_trace))
        result += ((
            qk_trajectory_trace,
            v_trajectory_trace,
...
def _rst_forward_training_fast(
    ...
    if getattr(fused_single, '_v4173_kernel_profile', None) != 'production':
...
def _rst_forward_production_diagnostics(
    ...
    if not isinstance(sharded_fns, dict):
        raise ValueError("production minimal RST requires dict sharded_fns")
    fused_single = sharded_fns.get('rst_single_minimal')
    if fused_single is None:
        raise ValueError("production minimal RST requires the pure RST kernel")
    if (getattr(fused_single, '_v4173_kernel_profile', None)
            != 'production_diagnostics'):
        raise ValueError(
    ...
    out = (out * rst_scale) @ router_params['up_rst']['kernel']
    rng, rng_out = jax.random.split(rng)
    out = safe_dropout(out, dropout_rate, deterministic, rng_out)
    ...
    output = (
        out,
        rst_active_frac, rst_active_n_mean,
        ...
        jnp.linalg.norm(out.astype(jnp.float32), axis=-1).mean(),
        zero_contribution, zero_contribution,
    )
    if parity_debug:
        return output + ((out, x, operator_query, raw_tau),)
    return output


def _rst_forward_analysis_minimal(x, pool_params, router_params, rng,
                         router_dropout, dropout_rate, deterministic,
                         sharded_fns,
                         d_model=None, n_layers=None,
                         n_operation_spaces=1, operation_space_top_k=1,
    ...
    canonical_single = getattr(
        fused_single, '_v4173_canonical_shard_map_kernel', None)
    if canonical_single is None:
        raise ValueError(
            "minimal RST requires a canonical v4173 shard-map kernel")
    ...
    suppressed_out = jax.lax.cond(
        ...
        lambda _: out,
        operand=None)
    rst_up = router_params['up_rst']['kernel']
    out = (out * rst_scale) @ rst_up
    suppressed_out = (suppressed_out * rst_scale) @ rst_up
    ...
    rst_selected_target = jnp.where(
        effective_reference_enabled,
        effective_rst_target,
        (rst_selected_target * rst_scale) @ rst_up)
    rst_selected_target = jnp.where(
    ...
    if analysis_trajectory_enabled:
        rst_trajectory_trace = tuple(
            (value @ rst_up if index < 4 else value)
            for index, value in enumerate(rst_trajectory_trace))
        result += ((
            rst_trajectory_trace,
...
def _attn_forward_minimal(*args, **kwargs):
    ...
    profile = _v4173_minimal_kernel_profile(
    ...
    return _attn_forward_analysis_minimal(*args, **kwargs)
...
def _rst_forward_minimal(*args, **kwargs):
    ...
    return _rst_forward_analysis_minimal(*args, **kwargs)
...
def _attn_forward(x, pool_params, router_params, expand_O_kernel, rng,
    ...
    attention_q = (
        qk_state_transitions[:, :, 0, :] * qk_scale
    ) @ router_params['up_qk']['kernel']
    attention_k = (
        qk_state_transitions[:, :, 1, :] * qk_scale
    ) @ router_params['up_qk']['kernel']
...
class DAWN_SRW_V4173(nn.Module):
    """DAWN-SRW v4.1.7.3 with operation-projection execution reads."""
    ...
    def setup(self):
        operator_key_mode = _validate_operator_key_mode(
            self.operator_key_mode, context="DAWN_SRW_V4173 constructor")
        if operator_key_mode != OPERATOR_KEY_MODE:
            raise ValueError(
                "v4173 requires operator_key_mode="
        ...
        legacy_power, _, _, _ = _resolve_v417x_admission_den_powers(
            ...
            context="DAWN_SRW_V4173 constructor")
        _validate_v4173_composition_settings(
        ...
        if int(self.d_route) <= 0:
            raise ValueError(
                f"v4173 model.d_route must be > 0, got {self.d_route}")
    ...
    def __call__(self, input_ids, labels=None, attention_mask=None,
        ...
        """
        operator_key_mode = _validate_operator_key_mode(
            self.operator_key_mode, context="DAWN_SRW_V4173 forward")
        ...
        (runtime_srw_composition_mode, runtime_admission_den_power,
         runtime_heat_kernel_beta) = (
            _validate_v4173_composition_settings(
                ...
                context="DAWN_SRW_V4173 forward"))
        if runtime_srw_composition_mode != model_srw_composition_mode:
            raise ValueError(
                "v4173 constructor/forward srw_composition_mode mismatch: "
        ...
        if minimal_kernel_profile in {
            ...
            production_analysis_request = (
                ...
                or analysis_contribution is not None
                or analysis_keep_qk is not None
                or analysis_keep_v is not None
        ...
        if self.is_initializing():
            ...
            _ = self.router.up_v(route_local_zeros)
            _ = self.router.up_rst(route_local_zeros)
    ...
    def get_config(self):
        ...
        (legacy_power, qk_power, v_power, rst_power) = (
            _resolve_v417x_admission_den_powers(
                ...
                self.admission_den_power_v,
                self.admission_den_power_rst,
                context="DAWN_SRW_V4173 config serialization"))
        cfg = {
    ...
    def get_model_info(self):
        ...
        (admission_den_power, qk_den_power, v_den_power,
         rst_den_power) = _resolve_v417x_admission_den_powers(
            ...
            self.admission_den_power_v,
            self.admission_den_power_rst,
            context="DAWN_SRW_V4173 model info")
        ...
        return [
            ...
            "  probe_scope=shared_across_qk_v_rst",
            "  live_gradient=true",
            "  execution_read=raw_operation_projection",
            f"  execution_read_width={self.d_route}",
            ...
            "  up_qk shared by Q/K; up_v and up_rst route-specific",
            "  routes=Q,K,V,RST",
            "Composition:",
            *composition_info,
            ("  den_power["
...
def _angular_execution_kwargs_from_model_cfg(model_cfg):
    """Extract v4173 canonical execution settings for inference."""
    (admission_den_power, admission_den_power_qk,
    ...
    (srw_composition_mode, admission_den_power,
     heat_kernel_beta) = (
        _validate_v4173_composition_settings(
            ...
            context="v4173 inference model config"))
    return {
...
def _tau_init_calibration_scores(params, input_ids, max_tokens=128):
    ...
    states and the shared v4173 router/pool parameters. Rho follows the
...
def _query_geometry_diagnostics(params, input_ids, max_tokens=4096):
    """Heavy v4173 query geometry on deterministic first-block tokens."""
    max_tokens = int(max_tokens)
    input_ids = jnp.asarray(input_ids, dtype=jnp.int32)
...
def _srw_inference(
    ...
    """Non-chunked v4173 SRW with raw operation-projection reads."""
    ...
    read_activations = _operation_projection_read(
        operator_query, read_directions)
...
def _srw_inference_with_gates(
    ...
    # Selection uses normalized queries; reads use raw operation projections.
    read_directions = _forward_unit_direction(
        read_vectors.astype(jnp.float32))
    write_directions = _forward_unit_direction(
        write_vectors.astype(jnp.float32))
...
def _attn_forward_cached(x, pool_params, router_params, expand_O_kernel,
    ...
    _qk_s, _v_s, _ = _effective_pool_output_scales(
        pool_params, d_model, n_layers)
    attention_q = (
        attention_q * _qk_s) @ router_params['up_qk']['kernel']
    attention_k_new = (
        attention_k_new * _qk_s) @ router_params['up_qk']['kernel']
    attention_v_new = (
        attention_v_new * _v_s) @ router_params['up_v']['kernel']
...
def _rst_forward_inference(x, pool_params, router_params,
    ...
    if d_model is None or n_layers is None:
        raise ValueError(
            "depth-scaled pool outputs require d_model and n_layers.")
    _, _, rst_scale = _pool_output_scales(d_model, n_layers)
    return (out * rst_scale) @ router_params['up_rst']['kernel']
...
def prefill(params, model_cfg, input_ids):
    ...
    def prefill_layer(carry, xs):
        ...
        attention_q = (
            attention_q * _qk_s) @ router_params['up_qk']['kernel']
        attention_k = (
            attention_k * _qk_s) @ router_params['up_qk']['kernel']
        attention_v = (
            attention_v * _v_s) @ router_params['up_v']['kernel']
...
def build_suppressed_forward(params, model_cfg, suppress_masks):
    ...
    def _srw_sup(
        ...
        # Suppressed forward uses the same v4173 operation-space read.
        read_directions = _forward_unit_direction(
            read_vectors.astype(jnp.float32))
        write_directions = _forward_unit_direction(
            write_vectors.astype(jnp.float32))
    ...
    def forward_fn(input_ids):
        ...
        for i in range(n_layers):
            ...
            _v_s = v_scale_eff
            attention_q = (
                attention_q * _qk_s) @ rp['up_qk']['kernel']
            attention_k = (
                attention_k * _qk_s) @ rp['up_qk']['kernel']
            attention_v = (
                attention_v * _v_s) @ rp['up_v']['kernel']
