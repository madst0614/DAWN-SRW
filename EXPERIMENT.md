...
- Current blocker: no local implementation blocker. The user explicitly authorized the existing TPU `spatial-se-400m` for the IOI validation task; only that exact resource may be contacted. Pre-intervention runtime failures and their fixes are preserved in `2026-07-29/#13-#15`; source commit `170a4b2f73bc1fad73c5faf83f1552caeb2eb0b7` now uses the canonical 128-row validation selection and derives the unrelated-output mask from immutable prompt/name/tokenizer fields when the published build lacks optional semantic-span metadata.
- Next experiment: publish the unrelated-output mask compatibility correction, verify the clean remote SHA under `madst0614`, then rerun only `ioi_frozen_validation` on `spatial-se-400m`: intact, frozen suppression, four controls times 100, and exact restoration over the 123 paired-correct validation units. Finalize and publish that record before any held-out test accessor is called.
