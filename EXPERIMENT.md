...
- Publication state: `origin/codex/v4167-poc` is at `ec744a2a095227b4d928cdc058816976d10e98f0`. It contains the accepted bundle4/exact-top-2 execution line, the explicit operation-space structure, the measured dense RW analytic pullback, and the independent-basis/fallback/metric-contract change. The repository-wide POC synchronization rule is being published with experiment `2026-07-26/#23`.
...
- Current conclusion: bundle4 dense forward, exact selected-top-2 backward, and the accepted analytic pullback remain unchanged. Fresh v4174 initialization now gives every operation space an independently sampled semi-orthogonal `D x R` read basis; write maps still start as exact transposes, shapes/counts are unchanged, and read/write remain independently trainable.
- Baseline state: `docs/v4174_dense_rw_baselines.yaml` generation `g2` remains the latest measured execution baseline and is explicitly pinned to the old shared-base/row-sign-derived initializer. The new independent initializer changes the accepted-source/initialization fingerprint, so `g2` is historical performance evidence, not from-scratch learning evidence for the new source.
- Active run: the last observed generation `g2` workload at commit `7b14a456` initialized from scratch with the old sign-derived bases. Its live TPU state was not inspected or changed in this repository-only integration.
- Current blocker: no local implementation blocker. A matched from-scratch TPU comparison for the independent initializer is not measured.
- Next experiment: start a new from-scratch v4174 run on an explicitly authorized existing TPU and establish the next baseline generation; do not resume an old checkpoint when evaluating the initializer because restore bypasses fresh parameter initialization.
...
- Status: repository policy recorded; remote branch and accepted-source audit passed; TPU not used.
...
- Evidence: `git branch --remotes --no-merged origin/codex/v4167-poc`; ancestry checks for `213001e3`, `8187c00d`, `db16ffdf`, `7b14a456`, and `fe3d82de`; model-blob comparison between `8187c00d` and `001c2c27`; `AGENTS.md`; `docs/v4174_dense_rw_baselines.yaml`.
