...
- Next experiment: resolve and freeze one committed numeric checkpoint from the registry-designated dense Transformer 400M run, verify its exact parameter/config/runtime identity, then run the same locked 10M-token C4 validation and the same unlimited six-task zero-shot protocol used for DAWN-SRW 400M. Do not claim matched performance retention until both dense rows are complete and comparable.
