...
- Publication state: accepted QKV+RST outer analytic-pullback source is integrated by merge commit `62417b239d5b79c5dfa8684563532958ed212385` on `codex/v4167-poc`. Experiments #35–#36 and baseline generation `g4` record its promotion. Experiment #37 centrally records the rejected operator-bank row-norm-cache candidate. Benchmark-only stage-attribution source is published separately at `f3859cc61d00cd303172b9f4337a2b44364ff8a7`; production g4 model/trainer source remains unchanged.
...
- Current conclusion: keep generation `g4` unchanged. Hoisting shared operator-bank scalar row norms reduced two-layer forward by `1.9536%`, but cache-inclusive combined F+B improved only `0.2311%` and inferred backward regressed `0.4824%`; it failed the fixed 10% gate and was not promoted.
...
- Active run: no TPU benchmark is currently claimed complete for stage attribution. The benchmark-only factory is locally validated and published; the next run is a two-layer attribution-only pass on the explicitly authorized existing `spatial-se-400m`.
- Current blocker: no runtime blocker. Stage-level TPU time and HBM are `not measured`; no optimization candidate has been selected.
- Next experiment: run the generation-`g4` two-layer stage attribution, separate QKV/RST, forward/backward, bundle/primary/overflow, and RW chunks, then select exactly one measured bottleneck. Do not run a full step during attribution.
...
- Status: implementation committed and pushed on its isolated profiling branch; local shape, parity, overflow, sharding, and detailed-profiler integration validated; TPU attribution is `not measured`.
...
- Decision/next: run only `--fast --backward-profile --stage-attribution --detailed-profile-layers 2 --no-xla-dump` on `spatial-se-400m`. Use the resulting QKV/RST F/B, primary/overflow, and chunk table to select one largest bottleneck. Do not alter g4 or run full-step until one subsequent single candidate passes the fixed 10% combined F+B gate.
- Evidence: candidate commit and the two benchmark source files; local validation output from 2026-07-26 KST; this central notebook.
