...
- Active run: the old-sign generation `g2` from-scratch workload was observed through step 400, archived, and replaced as authorized. Both `g3` benchmark protocols completed successfully; the TPU benchmark tmux wrappers are idle after completion.
- Current blocker: no baseline or local implementation blocker. The exact top-2 forward candidate is not yet implemented or measured.
- Next experiment: replace bundle4 forward with exact selected-top-2 space buckets using `Tcap=3072` plus compact exact overflow, while retaining the accepted analytic backward. Screen against `g3` with the two-layer gate, then the matched five-step full benchmark, and require retained benefit under step-100-skew routing before promotion.
