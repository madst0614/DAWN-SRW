...
- Current blocker: no IOI infrastructure blocker. The final 400M IOI held-out artifact used clean analysis commit `75b0e0157bce670d1ca6f7801a8393014ce09ffb`, checkpoint step `76293`, mesh `16x2`, frozen-spec hash `6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`, and result root ending `20260729T123731Z-3cb89374`; all preregistered test gates passed. RAVEL is blocked from confirmatory work by its discovery specification, and ARC is blocked from held-out test by its negative frozen validation result. The matched 400M performance rows and DAWN-SRW v4172 1.3B C4 and zero-shot rows are complete. The 1.3B IOI input contract completed in `2026-07-30/#12` without opening any split, and its discovery-only implementation/specification was hash-bound in `2026-07-30/#13`. The remaining paper experiment is to execute that discovery and, only if eligible, frozen validation/test. Literal 400M operator IDs must not be transplanted across scale.
- Next experiment: deploy the exact clean pushed descendant of implementation commit `8bd0a717a002595e25060121d35b31f09410961d` on `spatial-se-400m` and run only preset `ioi_scale_discovery_localization` for target `v4172_1B` with capture widths `1024/4096/8192 -> 4096/16384/16384`. Proceed to frozen validation and held-out test only if the unchanged discovery gates pass. Do not modify the completed 400M IOI circuit and do not evaluate the still-training baseline-JAX 1.3B run.
...
| D. Scale replication | 1.3B performance and selected causal effect with the same control design | DAWN 1.3B C4 and zero-shot complete; selected causal replication pending | Report the 1.3B performance rows as larger-scale results; do not yet claim complete scale replication |
...
| DAWN-SRW v4172 1.3B | `1,272,038,404` measured | `19,999,981,568` in the evaluation manifest | Step `87,193` restored; C4 and zero-shot measured | Larger-scale performance and selected causal replication |
