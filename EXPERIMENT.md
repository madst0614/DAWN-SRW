...
- Paper execution state: the v4172 400M Phase 0 gate is complete. Step 76,293 identity, clean restore, and immutable input contracts passed in `2026-07-29/#03`; two independent fixed-batch runs in `2026-07-29/#04` passed deterministic production/adapter parity. The full unlimited stock zero-shot suite completed as a comparable paper result in `2026-07-29/#05`, and the locked 10M-token C4 validation completed in `2026-07-29/#06`. The preregistered behavioral mechanistic screen completed in `2026-07-29/#07`: IOI, MCQA, ARC, and RAVEL passed the minimum-known-correct gate in discovery, validation, and test, while arithmetic did not. IOI discovery-only operator localization completed in `2026-07-29/#09` with rank stability `0.91398` and froze a 4,096-site Q/K-dominant circuit. The exact frozen-spec validation in `2026-07-29/#17` is a strong success: suppression, separation from all four 100-replicate matched controls, and exact restoration passed every preregistered gate over 123 independent paired-correct units. This supports a validation-level causal Q/K-centered RW-operator-circuit claim. The first held-out launch in `2026-07-29/#19` opened exactly 114 paired-correct test units but stopped at a leftover validation-only phase guard before intact scoring; it is an operational incident with no test effect estimate. Held-out confirmation and promotion to the paper's central result remain pending. Retain RAVEL and ARC as the following primary/secondary tasks. DAWN 1.3B work remains sequential and selective, and no 1.3B baseline work is currently allowed.
...
- Current blocker: the first held-out run exposed and then stopped at a stale validation-only phase guard after reproducing exactly 114 paired-correct test units; no intact, suppression, control, or restoration condition ran. Source commit `1a643d330d8032008baf3c276471773599bc8b7d` changes only that phase admission to allow one homogeneous validation or test batch. The completed validation artifact remains bound to clean analysis commit `209147bbbdaae00ebdf64655b917a91f77d50b26`, finalized canonical record commit `3b2f04aae3afe49fd1040c99e667acf50daadefc`, checkpoint step `76293`, mesh `16x2`, frozen-spec hash `6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`, and result root ending `20260729T120132Z-ac1fc0cb`. The user explicitly authorized the existing TPU `spatial-se-400m`, and only that exact resource may be contacted.
- Next experiment: publish the phase-admission correction and `2026-07-29/#19`, then rerun only `ioi_frozen_test` on `spatial-se-400m` from the exact clean pushed POC descendant. Require the same 128 canonical rows and 114 paired-correct independent units, and apply the unchanged 4,096 sites, full-denominator suppression, four control families times 100 with the same seeds/rules, frozen thresholds, and exact restoration. Finalize the result without any outcome-dependent change, including if negative.
...
| B. Causal RW unit | Localized operators, suppression, matched controls, restoration, held-out confirmation | IOI localization plus validation suppression/control separation/restoration passed; held-out test pending | Claim a validation-level causal Q/K-centered RW operator circuit; do not call it held-out confirmed or the final paper result |
| C. Reusable circuit | Discovery/confirmatory separation and transfer to matched held-out inputs with negative controls | The frozen IOI circuit generalized from discovery selection to independent validation effects; held-out test and variable transfer pending | Report discovery-to-validation causal reuse only; do not claim held-out or cross-variable transfer |
