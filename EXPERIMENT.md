...
- Current blocker: no IOI infrastructure blocker. The final IOI held-out artifact used clean analysis commit `75b0e0157bce670d1ca6f7801a8393014ce09ffb`, checkpoint step `76293`, mesh `16x2`, frozen-spec hash `6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`, and result root ending `20260729T123731Z-3cb89374`; all preregistered test gates passed. RAVEL is blocked from confirmatory work by its discovery specification: every variable missed the `0.80` stability gate and no audited prefix jointly reached cumulative importance `0.70` and split overlap `0.95`. ARC is blocked from held-out test by its frozen validation result rather than by infrastructure: suppression and frozen-minus-control both failed in the opposite direction, so the preregistered protocol does not permit test access. The matched 400M C4 and zero-shot rows are complete. DAWN-SRW v4172 1.3B exact-step resolution, clean restore, and C4 row are complete; its unchanged unlimited six-task zero-shot evaluation is the remaining performance blocker. Selected 1.3B IOI replication remains downstream of that performance gate.
- Next experiment: publish the complete 1.3B C4 artifact, then deploy its exact clean pushed notebook descendant on `spatial-se-400m` and run only target `v4172_1B` with the unchanged unlimited `zero_shot` preset, mesh `16x2`, and no dependency or protocol change. Do not evaluate the still-training baseline-JAX 1.3B run.
...
| D. Scale replication | 1.3B performance and selected causal effect with the same control design | DAWN 1.3B C4 complete; zero-shot and selected causal replication pending | Report the 1.3B C4 row as a larger-scale result; do not yet claim complete scale replication |
...
| DAWN-SRW v4172 1.3B | `1,272,038,404` measured | `19,999,981,568` in the evaluation manifest | Step `87,193` restored; C4 measured; zero-shot pending | Larger-scale performance and selected causal replication |
...
|---|---|---|
| Table 1: model/training specification | Parameters, tokens, dimensions, pools, tokenizer, training conditions | This ledger; DAWN 400M from `2026-07-29/#03/#05/#06`, dense 400M from `2026-07-30/#03/#05/#07`; 1.3B pending |
...
| Table 3: zero-shot | Six tasks, mean, standard errors, model comparison | Matched 400M rows complete in `2026-07-29/#05` and `2026-07-30/#07`; DAWN 1.3B pending |
...
| Figure 2: performance | C4 and benchmark comparison with uncertainty | Matched 400M C4 and zero-shot complete; DAWN 1.3B pending |
...
- Supported now: the exact v4172 400M checkpoint restores reproducibly; the
  ...
  `0.3220` percentage point mean zero-shot accuracy; and IOI, MCQA, ARC, and
