...
- Current blocker: no local implementation blocker. The user explicitly authorized the existing TPU `spatial-se-400m` for the IOI validation task; only that exact resource may be contacted. An initial launcher connection was stopped before validation access because the launcher omitted the required explicit remote SSH user; commit `cfb9d32947b426f6005441c3019b3eb74a6d48ab` fixes that deployment identity.
- Next experiment: publish the explicit-user launcher correction, verify the clean remote SHA under `madst0614`, then run only `ioi_frozen_validation` on `spatial-se-400m`: intact, frozen suppression, four controls times 100, and exact restoration over the 123 paired-correct validation units. Finalize and publish that record before any held-out test accessor is called.
