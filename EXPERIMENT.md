...
- Next experiment: deploy the exact clean dense-evaluation pre-effect commit on `spatial-se-400m`, restore only baseline-JAX 400M step `76293`, run the unchanged locked 10M-token C4 validation first, then run the unchanged unlimited six-task zero-shot preset. Do not change the checkpoint, model, C4 cap, task set, harness, batch, mesh, or protocol between results.
