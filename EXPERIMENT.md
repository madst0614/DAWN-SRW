...
- Current blocker: no paper-analysis infrastructure blocker remains. The final 400M IOI held-out artifact used clean analysis commit `75b0e0157bce670d1ca6f7801a8393014ce09ffb`, checkpoint step `76293`, mesh `16x2`, frozen-spec hash `6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`, and result root ending `20260729T123731Z-3cb89374`; all preregistered test gates passed. RAVEL is blocked from confirmatory work by its discovery specification, ARC is blocked from held-out test by its negative frozen validation result, and selected 1.3B IOI replication is blocked from confirmatory work by its preregistered discovery prefix gate. The matched 400M performance rows and DAWN-SRW v4172 1.3B C4 and zero-shot rows are complete. Literal 400M operator IDs were not transplanted across scale.
- Next experiment: paper experiment execution is closed at the current preregistered boundaries. Build the final tables, figures, and manuscript from the ledger below; do not reopen RAVEL, ARC held-out test, or 1.3B IOI validation/test, and do not evaluate the still-training baseline-JAX 1.3B run unless the user declares it complete.
...
|---|---|---|---|
| A. Performance retention | Matched C4 validation and six-task zero-shot versus dense 400M | Complete: DAWN CE is `0.004319` above dense and six-task mean is `0.3220` percentage point above dense | Claim matched 400M performance retention; do not claim formal equivalence or superiority |
...
| D. Scale replication | 1.3B performance and selected causal effect with the same control design | DAWN 1.3B C4 and zero-shot complete; selected IOI discovery was stable but failed the preregistered cumulative-importance prefix gate, so no confirmatory circuit was eligible | Report the 1.3B performance rows and the negative scale-localization result; do not claim causal scale replication |
...
| Dense Transformer 400M | `393,896,960` measured | `39,999,504,384` in step-76,293 metadata | Step `76,293` restored; C4 and zero-shot measured | Matched 400M performance control |
| DAWN-SRW v4172 1.3B | `1,272,038,404` measured | `19,999,981,568` in the evaluation manifest | Step `87,193` restored; C4 and zero-shot measured; IOI scale discovery closed with no frozen circuit | Larger-scale performance and negative selected causal-replication result |
...
Its exact numeric step, full C4 validation, and unlimited six-task zero-shot
evaluation are complete. The two zero-shot manifests match exactly on protocol,
software, tokenizer, task configuration hashes, dataset fingerprints, batch,
length buckets, mesh, and task limits.
...
- `limit=1` executions are parity-only and must never populate a paper
  performance table. Both eligible 400M runs have `limit=null`,
...
supports matched C4 performance retention and is interpreted jointly with the
completed zero-shot comparison below.
...
### Table-ready DAWN-SRW v4172 1.3B C4 validation

| Model | Training tokens | Valid target tokens | Cross-entropy | Perplexity | Token accuracy |
|---|---:|---:|---:|---:|---:|
| DAWN-SRW v4172 1.3B | `19,999,981,568` | `9,974,720` | `2.835879303` | `17.045381766` | `0.437098685` (`43.7099%`) |

This uses the same `packed_c4_validation_v1` protocol, capped dataset view,
610 complete global batches, sequence length 512, batch 32, validation-data
hash, float32 scoring, and `16x2` mesh as both 400M rows. Relative to the DAWN
400M row, 1.3B CE is lower by `0.104333555` (`3.5485%`), perplexity is lower
by `1.874491366`, and token accuracy is higher by `0.012186808` absolute
(`1.2187` percentage points). This comparison is descriptive: the 1.3B model
was trained for about 20B tokens while the 400M model was trained for about
40B, so it is not a controlled scaling curve.

The paper-eligible 1.3B C4 result root is
`gs://dawn-tpu-data-c4/checkpoints/train_config_v4172_1p3B_c4_20B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260717_173556_3201/side_analysis/run_analysis_000000087193_c4_validation_20260729T162030Z-1a691644`.
Its result hash is
`f5549fffe65c48822f7eff3a241bdf398d9660f54d46809c7fb501103043da46`;
the run used clean evaluation commit
`f93d7ed5612525c8a22a3f695814681ca58d9157`.

### Table-ready DAWN-SRW v4172 1.3B zero-shot performance

| Benchmark | Split (`n`) | Metric | Score | Standard error |
|---|---:|---|---:|---:|
| LAMBADA OpenAI | test (`5,153`) | accuracy | `0.395885892` | `0.006813285` |
| LAMBADA OpenAI | test (`5,153`) | perplexity | `17.635757173` | `0.538975353` |
| HellaSwag | validation (`10,042`) | normalized accuracy | `0.510157339` | `0.004988752` |
| PIQA | validation (`1,838`) | normalized accuracy | `0.723068553` | `0.010440500` |
| ARC-Easy | test (`2,376`) | normalized accuracy | `0.509680135` | `0.010257861` |
| ARC-Challenge | test (`1,172`) | normalized accuracy | `0.268771331` | `0.012955066` |
| WinoGrande | validation (`1,267`) | accuracy | `0.507498027` | `0.014050906` |
| Six-task aggregate | six primary accuracy metrics | arithmetic mean | `0.485843546` | not aggregated |

The paper-eligible 1.3B zero-shot result root is
`gs://dawn-tpu-data-c4/checkpoints/train_config_v4172_1p3B_c4_20B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260717_173556_3201/side_analysis/run_analysis_000000087193_zero_shot_20260729T163122Z-73a8b2d7`.
The backend host-result hash is
`84b211080997fdd0ddb8e842ac958e987e54634209af70c7bcb038c48995a9f6`;
`results_summary.json`, backend `run_manifest.json`, outer `summary.json`, and
outer `run_manifest.json` have SHA-256
`df14faa266b8114344524c9c5d4f88eff447283bc1a4c2da82d7e08d5ec13e41`,
`90a96b4822e582553928d8b33ded118bae3640cccef23aeaf66b731d20613cf0`,
`53d28b804eea9dc3b7df4fafff2966ca8cdadc37bf5d3302ab9ca6f67f74e065`,
and `0809e90e3442a0ddb2ebf435a75572bb9a5a78f2383dbcab86483bad2b140325`.
The run used clean evaluation commit
`f028b36bfa1fd4fae8956b6655916d7a00012a1b`.

Relative to DAWN 400M, the 1.3B six-task mean is higher by `0.022971086`
(`2.2971` percentage points). Its task differences are LAMBADA
`+0.043857947`, HellaSwag `+0.051782513`, PIQA `+0.010881393`, ARC-Easy
`+0.025673401`, ARC-Challenge `+0.011945392`, and WinoGrande
`-0.006314128`; LAMBADA perplexity is lower by `6.441626513`. Relative to
the matched dense 400M row, its mean is higher by `0.026191581` (`2.6192`
percentage points). These are descriptive comparisons only: the 1.3B model
was trained for about 20B tokens while both 400M models were trained for about
40B, and no completed matched dense 1.3B result is in scope.
...
### Table-ready dense Transformer 400M performance

| Benchmark | Split (`n`) | Metric | Score | Standard error |
|---|---:|---|---:|---:|
| LAMBADA OpenAI | test (`5,153`) | accuracy | `0.353192315` | `0.006658943` |
| LAMBADA OpenAI | test (`5,153`) | perplexity | `24.004181895` | `0.807297727` |
| HellaSwag | validation (`10,042`) | normalized accuracy | `0.457080263` | `0.004971364` |
| PIQA | validation (`1,838`) | normalized accuracy | `0.698041349` | `0.010711733` |
| ARC-Easy | test (`2,376`) | normalized accuracy | `0.475589226` | `0.010247549` |
| ARC-Challenge | test (`1,172`) | normalized accuracy | `0.249146758` | `0.012639407` |
| WinoGrande | validation (`1,267`) | accuracy | `0.524861878` | `0.014035103` |
| Six-task aggregate | six primary accuracy metrics | arithmetic mean | `0.459651965` | not aggregated |

The paper-eligible dense zero-shot result root is
`gs://dawn-tpu-data-c4/checkpoints/baseline_400M_c4_40B_v4_64/run_vbaseline-JAX_20260713_194029_3201/side_analysis/run_analysis_000000076293_zero_shot_20260729T155031Z-89157be7`.
The backend host-result hash is
`e941f5115a99ed3dd53c3c883a4e896eaa4eeda60ebcba819788e2eb7a37b99c`;
`results_summary.json` and its backend `run_manifest.json` have SHA-256
`31661e30f3c8780ae9e29f96949aa321e9c68271c789e121d6a132a479e37247`
and `b32eeab6e787889828e1181fa0594783de0ad71dd2f88b6613c6c4f294ff7507`.
The run used clean evaluation commit
`5f6abc6a42020bcb2f9729afc240c6541660b238`.

### Matched 400M zero-shot comparison

| Primary metric | DAWN-SRW | Dense Transformer | DAWN minus dense |
|---|---:|---:|---:|
| LAMBADA accuracy | `0.352027945` | `0.353192315` | `-0.001164370` |
| HellaSwag normalized accuracy | `0.458374826` | `0.457080263` | `+0.001294563` |
| PIQA normalized accuracy | `0.712187160` | `0.698041349` | `+0.014145811` |
| ARC-Easy normalized accuracy | `0.484006734` | `0.475589226` | `+0.008417508` |
| ARC-Challenge normalized accuracy | `0.256825939` | `0.249146758` | `+0.007679181` |
| WinoGrande accuracy | `0.513812155` | `0.524861878` | `-0.011049724` |
| Six-task mean | `0.462872460` | `0.459651965` | `+0.003220495` |

The maximum absolute task difference is `0.014145811` (`1.4146` percentage
points). DAWN is higher on four primary metrics and dense is higher on two;
the mean favors DAWN by `0.3220` percentage point. LAMBADA perplexity is
`24.077383687` for DAWN and `24.004181895` for dense. These descriptive
matched results support performance retention. No equivalence margin or
model-superiority test was preregistered, so they do not establish statistical
...
language model suitable for causal analysis. Against the 393.9M-parameter
dense control trained for the same 39,999,504,384 tokens, DAWN differs by
`+0.004319` C4 CE and `-0.2188` percentage point token accuracy, while its
six-task zero-shot mean is `+0.3220` percentage point higher. The largest
absolute task difference is `1.4146` percentage points. This supports matched
400M language-model performance retention alongside the causal analysis; it is
not a formal statistical equivalence or superiority result.

At the fixed step-87,193 checkpoint, the 1,272.0M-parameter DAWN-SRW model
obtained C4 validation cross-entropy `2.8359` and six-task zero-shot mean
accuracy `48.58%`. These larger-scale DAWN measurements are paper-eligible,
but they are not a controlled scaling curve or a matched dense comparison
because the 1.3B checkpoint has a different pretraining-token budget and the
dense 1.3B run remains excluded.
...
Validation compared intact execution with full-denominator circuit suppression,
100 replicates each of equal-count random, same-layer random,
activation-matched, and route-frequency-matched controls, then exact
same-example restoration. The frozen validation completed in
`2026-07-29/#17`; necessity, all-control separation, and restoration are now
measured on validation. Held-out test was not opened during localization or
validation, so held-out generalization remains `not measured`.
...
### IOI frozen validation causal result

The confirmatory validation applied the unchanged frozen circuit to the 123
paired-correct independent validation units. The intervention graph's intact
margin signs agreed with the production-diagnostics reference for all 123
units. Its frozen primary score is the correct-minus-source summed
log-probability margin. Frozen suppression changed that margin from `3.465456`
to `-4.399331`, a mean drop of `7.864787` with 95% bootstrap CI
`[7.490080, 8.241263]`, and reduced exact accuracy from `1.000000` to
`0.008130`.

| Validation condition | Mean margin | Mean margin drop | Exact accuracy | Flip fraction | Correct log-prob change | Source-direction change | Unrelated log-prob damage |
|---|---:|---:|---:|---:|---:|---:|---:|
| Intact | 3.465456 | — | 1.000000 | — | — | — | — |
| Frozen suppression | -4.399331 | 7.864787 | 0.008130 | 0.991870 | -6.612009 | 7.864787 | 0.445388 |
| Equal-count random, mean of 100 | — | 0.070886 | 0.989024 | 0.010976 | -0.013691 | 0.070886 | -0.001095 |
| Same-layer random, mean of 100 | — | 0.039389 | 0.990081 | 0.009919 | 0.000080 | 0.039389 | 0.004069 |
| Activation-matched, mean of 100 | — | 5.400490 | 0.189268 | 0.810732 | -2.708797 | 5.400490 | 0.082198 |
| Route-frequency-matched, mean of 100 | — | 0.219162 | 0.981545 | 0.018455 | 0.024401 | 0.219162 | 0.002669 |
| Exact restoration | 3.462791 | 0.002665 | 0.991870 | 0.008130 | -0.001809 | 0.002665 | 0.000585 |

| Frozen drop minus control drop | Mean difference | 95% bootstrap CI | Two-sided paired permutation p | Frozen exceeded every replicate |
|---|---:|---:|---:|---|
| Equal-count random | 7.793902 | [7.437851, 8.161572] | 0.00049975 | Yes |
| Same-layer random | 7.825399 | [7.440629, 8.180288] | 0.00049975 | Yes |
| Activation-matched | 2.464297 | [2.076200, 2.832697] | 0.00049975 | Yes |
| Route-frequency-matched | 7.645626 | [7.289512, 8.031625] | 0.00049975 | Yes |

Every control family contained 100 unique 4,096-site samples, excluded all
frozen sites, and retained its frozen sampling rule and seed. Exact
same-example selected-numerator restoration recovered `0.999661` of the
suppression effect, with 95% bootstrap CI `[0.998655, 1.000584]`, above the
frozen `0.80` lower-CI gate. Its residual mean margin drop was `0.002665`, and
accuracy returned to `0.991870`. All three preregistered gates passed.

Suppression lowered the correct-answer log probability by `6.612009` on
average, raised source log probability by `1.252778`, flipped `99.1870%` of
predictions, and moved the margin toward the source by `7.864787`. The
unrelated-output audit measured damage `0.445388`, versus correct-answer
log-probability damage `6.612009`, for an absolute ratio of `0.067360`.
Unrelated damage was not a frozen formal gate, so this ratio is reported as a
selectivity audit rather than used to alter the decision.

The completed result is bound to clean analysis commit
`209147bbbdaae00ebdf64655b917a91f77d50b26`, checkpoint step `76293`, mesh
`16x2`, frozen-spec hash
`6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`,
artifact protocol hash
`f2906889ebf290b3373d315c2c49b4e15352dadba02c71ddfabc0fd49c085bec`,
and result root
`gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_ioi_frozen_validation_20260729T120132Z-ac1fc0cb`.
The aggregate result explicitly records `test_data_accessor_called=false`; a
...
### ARC auxiliary causal replication: frozen validation negative result

ARC discovery produced a stable descriptive localization in `2026-07-29/#24`,
but its unchanged frozen circuit did not reproduce the IOI causal direction.
The validation-only run selected exactly 51 paired-correct independent
`example_id` units from the canonical first 128 validation rows. Intact
correct-minus-source margin was `1.735534` at `100%` exact accuracy. Suppressing
the frozen 4,096 Q/K sites moved the margin up to `2.819139`, so the
preregistered margin drop was `-1.083605` with bootstrap CI
`[-1.472180, -0.690348]`; accuracy was `98.0392%` and prediction flip rate was
`1.9608%`. The suppression gate therefore failed in the opposite direction.

The 100 random controls each contained exactly 4,096 sites, matched every
layer-by-route count, excluded all frozen sites, and had unique identity hashes.
Their mean margin drop was `-0.034123` with replicate range
`[-0.323254, 0.311018]`. Frozen-minus-control margin drop was `-1.049482`,
bootstrap CI `[-1.440538, -0.643905]`, and paired two-sided permutation
`p=0.00049975`. Thus the frozen circuit was significantly smaller than the
matched-control effect in the required damage direction; zero of 100 control
replicates lay below the frozen effect, and the control-separation gate failed.

Exact selected-numerator restoration returned the margin to `1.731110`, exact
accuracy to `100%`, and prediction flips to zero. The recovery fraction was
`1.004083`, bootstrap CI `[0.994587, 1.013652]`, so the restoration gate passed.
That verifies exact intervention reversibility but does not rescue causal
necessity when the suppressed effect has the wrong sign. Mean correct-answer
log probability changed by `+0.303204`, while the unrelated base-prompt audit
measured damage `1.872223`; its absolute ratio to the correct-answer change was
`6.174800`. This unrelated audit had no formal gate and was not used to alter
the decision.

The artifact decision is `mixed_inconclusive`, all-preregistered-gates is
`false`, and `strongest_supported_claim` is null. Scientifically, this is a
completed negative result: ARC localization was stable, but the frozen
operator set was not causally necessary in the preregistered direction and
does not support auxiliary ARC replication. The result used clean analysis
commit `453cd03095f2cb0763a19892674ea1c0e7fd6cba`, checkpoint step `76293`,
mesh `16x2`, frozen-spec hash
`49f3a7dc83d3212f786666d74f77d62c7ccb749fb3df6388ac524c9209728600`,
artifact protocol hash
`5d124ef8b26c9afe3f7188aa48b05cfc2f653830438432c1443235d7322a6c2a`,
and result root
`gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_arc_frozen_validation_20260729T150935Z-18d6efcb`.
The 49,143-byte item JSON has SHA-256
`b4b33f0120154a7ab8ca078a4f5e7a522790fda28af6fdba3b75294847b97136`.
It explicitly records `test_data_accessor_called=false`,
`test_evaluated=false`, and `test_evaluation_count=0`; ARC held-out test is
closed and must not be opened under this frozen protocol.

### Manuscript table and figure map

| Output | Required content | Current evidence source |
|---|---|---|
| Table 1: model/training specification | Parameters, tokens, dimensions, pools, tokenizer, training conditions | DAWN 400M from `2026-07-29/#03/#05/#06`, dense 400M from `2026-07-30/#03/#05/#07`, and DAWN 1.3B from `2026-07-30/#08/#10/#11` |
| Table 2: C4 validation | Loss, perplexity, token accuracy for DAWN/dense 400M and DAWN 1.3B | Matched 400M rows complete in `2026-07-29/#06` and `2026-07-30/#05`; DAWN 1.3B complete in `2026-07-30/#10` |
| Table 3: zero-shot | Six tasks, mean, standard errors, model comparison | Matched 400M rows complete in `2026-07-29/#05` and `2026-07-30/#07`; DAWN 1.3B complete in `2026-07-30/#11` |
...
| Figure 2: performance | C4 and benchmark comparison with uncertainty | Matched 400M C4 and zero-shot plus DAWN 1.3B C4 and zero-shot complete |
| Figure 3: circuit localization | Layer x operator discovery contribution, split overlap, trajectory, held-out overlap | IOI localization/validation/test complete in `2026-07-29/#09/#17/#20`; RAVEL discovery negative in `2026-07-29/#22`; ARC localization passed in `2026-07-29/#24` but validation was negative in `2026-07-30/#02`; 1.3B IOI discovery was stable but failed the frozen-prefix gate in `2026-07-30/#14` |
...
| Figure 6: scale replication | 400M versus 1.3B discovery stability and frozen-prefix eligibility | 400M froze and confirmed a circuit; 1.3B discovery in `2026-07-30/#14` froze no circuit, so no 1.3B causal effect size is eligible |
...
- Supported now: the exact v4172 400M checkpoint restores reproducibly; the
  ...
  reproducible and paper-eligible; the matched dense 400M C4 and zero-shot rows
  are complete and support performance retention within `0.004319` CE and
  `0.3220` percentage point mean zero-shot accuracy; the DAWN 1.3B C4 and
  zero-shot rows are complete and paper-eligible as descriptive larger-scale
  measurements; and IOI, MCQA, ARC, and
  RAVEL have enough paired-correct independent examples for mechanistic
  follow-up under the preregistered threshold. IOI has a stable discovery
  aggregate operator
  ...
  and no frozen variable circuit. ARC has a stable discovery ranking and a
...
- Not supported: auxiliary ARC causal replication or variable-level transfer.
  Formal dense equivalence or superiority is not supported because no such
  inferential margin/test was preregistered. Not supported yet: IOI trajectory
...
#### #10 — Replace v4175 route-owned keys with one shared operation-space address table

- Status: requested in-place v4175 architecture revision implemented, locally validated, committed, and published on canonical `codex/v4167-poc`; TPU behavior is `not measured`.
- Hypothesis/change: keep model identity `spatial-r1-v4.1.7.5`, but make the atlas own both coordinate interfaces and address identity. Replace `space_selector/route_space_keys [4,M,R]` with `space_selector/operation_space_keys [M,R]`; retain `route_query_proj [4,D,R]`, route-independent top-k calls, shared `space_read_proj [M,D,R]` and `space_write_proj [M,R,D]`, and route/space-specific RW banks and tau maps. Set the 40M config's QK/V/RST denominator powers to `0.5/0.5/0.5`; denominator powers remain ordinary config fields with no new flag or hard-coded architecture gate.
- Identity: branch `codex/v4167-poc`; exact clean parent `4d88291f64181e7fca04b4c222a3f3d7ef83eacc`; accepted source/config commit `4da8bd8891fb1413ffb8570241ab817a9791a8e5`. SHA-256 values are model `4a102d5619aaee9e4f427df7a7930af917943ee3a6aabc0c0ffac4dad0c5c2bd`, trainer `9c1c0b91bf5a4a25c4401bdfe45ac1816c6b8d13d921dc3c7130ca59bd8c88bc`, and config `c988f021791cdb4db5284ac031dc59fd4520850213a850a3b797eb6573f9d2a7`. The config uses new checkpoint root `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4175_shared_keys_40M_c4_5B_den0p5`. No checkpoint, TPU, or mesh was used.
- Parameter/checkpoint contract: the symbolic counter now charges four `D x R` query projections plus one `M x R` address table. The 40M abstract Flax tree and symbolic total both equal `40,341,088`; selector shapes are exactly `[4,432,128]` and `[8,128]`. This is 3,072 parameters below the former route-key tree, and 2,528 below the 40,343,616 Transformer reference. Resume validation explicitly rejects legacy `route_space_keys`, missing or wrongly shaped `operation_space_keys`, and instructs a fresh scratch run; no conversion or fallback was added.
- Local validation: the resolved interpreter was `C:\Users\USER\Desktop\DAWN-SRW\.venv\Scripts\python.exe`, Python/JAX/Flax/Optax `3.12.7/0.6.2/0.10.7/0.2.8`. The repository-requested machine-specific `C:\Users\MADST\Desktop\dawn-spatial\.venv-jax062\Scripts\python.exe` did not exist in this checkout, so the equivalent repository-local JAX 0.6.2 environment was used. `py_compile` and `git diff --check` passed. A deterministic selector fixture used the same key table and produced distinct Q/K top-1 spaces `0/1`. A reduced one-layer JIT forward/backward returned finite loss `3.4986653`, nonzero shared-key gradient norm `2.2443e-09`, finite route-query gradients, and all 24 Q/K/V/RST selector metrics. Four focused v4174 config/tree/symbolic/trainer regression tests passed (`4 passed, 19 deselected`); pytest emitted only a permission warning for its optional cache.
- Logging/decision/next: startup reporting now emits `shared_operation_space_keys=true`, the `[M,R]` key shape, independent query scope, and the configured QK/V/RST denominator summary. Accept source commit `4da8bd8891fb1413ffb8570241ab817a9791a8e5` as the canonical v4175 implementation. Do not resume any pre-change v4175 checkpoint; use only the new root in a fresh scratch run on an explicitly named existing TPU and record compile/HBM/finite-step evidence.

#### #11 — Rebalance shared-key v4175 denominator powers and initial active mass

- Status: `user-provided` early-training observation from an active 40M scratch run; step-1 and step-100 behavior are available, while the 500-step and 1,000-step validations and final performance decision remain pending.
- Hypothesis/architecture: keep the shared operation-space address and coordinate system introduced in `2026-07-29/#10`: `operation_space_keys [M,R]`, `space_read_proj [M,D,R]`, and `space_write_proj [M,R,D]` are atlas-owned, while `route_query_proj [4,D,R]`, top-2 space selection, tau maps, and RW banks remain route-specific. The run uses `M=8`, `top_k=2`, `d_route=128`, and route-specific Q/K/V/RST all-space dense reference execution. The new hypothesis is that space-owned projections and banks already provide enough functional separation that the stronger denominator powers inherited from the global-pool formulation may suppress useful operator-composition differences.
- Preceding all-`0.5` comparison: the first shared-key run used QK/V/RST powers `0.5/0.5/0.5` with approximate initial active targets `100/100/300/600`. Training remained finite, but lowering V and RST powers increased their initial absolute gain: V started at approximately `3.7x` and RST at approximately `10x` their former output scale. Step 1 reported attention/RST/residual norms `0.210/4.409/16.735`; step 100 reported `10.945/8.636/91.434`; step-1,000 validation was loss `6.0446`, accuracy `0.1365`. V top-1 space concentration remained approximately `0.91-0.95`. Because the current composition law couples mass response and initial branch gain, reject all-`0.5` as the active comparison candidate.
- Current change: set QK/V/RST denominator powers to `0.5/0.8/1.0` without adding an output multiplier. Keep square-root composition for Q/K alignment, retain some mass-sensitive differentiation in V while limiting attention-input amplification, and make the residual RST path mass-average rather than retaining the former `1.2` multi-activation penalty. Reduce the initial Q/K/V active targets from approximately `100/100/300` to `67/67/207`; keep RST near `600`.
- Repository identity: at transcription time, canonical branch `codex/v4167-poc` and `origin/codex/v4167-poc` both resolve to clean commit `4fe89796c6960c7a8e6fb8715a24daf7d4720041`. Config `configs/train_config_v4175_40M_c4_5B.yaml` has SHA-256 `2ec7e434e07a59c0306840634fedc40ccba3fdb66b5154e3613c16afbca20067`, parameter count `40,341,088`, mesh `4x1` on four TPU v4-4 devices, and configured scratch root `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4175_shared_keys_40M_c4_5B_den_modify`. The user-provided observation did not include the exact TPU resource name, deployed commit verification, generated run folder, or concrete checkpoint step, so those identities are `not provided` rather than inferred from the local checkout.
- Step-1 result: finite loss `10.3957` and gradient norm `2.67`. Active Q/K/V/RST counts were `66.49/66.86/207.03/599.92`. QK gate mass/denominator were `3.20/1.78`, V `8.98/5.79`, and RST `27.36/27.36`. Attention/RST/residual norms were `0.102/0.845/3.498`, versus `0.210/4.409/16.735` for all-`0.5`; the RST output spike was largely removed and the initial residual returned much closer to a stable range.
- Step-100 result: loss `8.1172`, gradient norm `1.12`, active counts approximately Q/K `67`, V `206`, and RST `608`. Attention/RST/residual norms were `3.753/1.458/21.719`; route-space top-1 shares were Q `0.817`, K `0.808`, V `0.923`, and RST `0.861`. Loss improved over the all-`0.5` step-100 value `8.3421`, and RST norm fell from `8.636` to `1.458`. Attention and residual norms nevertheless grew during early training, and each route still concentrated strongly on one space; step 100 alone is insufficient to call collapse because earlier v4175 runs also relaxed initial concentration later.
- Interpretation: `0.5/0.8/1.0` is an intermediate regime that removes the all-`0.5` branch-gain surge while preserving more operator-composition differentiation than the former `0.5/1.0/1.2` law. This experiment changes three coupled axes—shared atlas keys, denominator powers, and reduced QK/V initial active targets—so any later performance difference must be attributed to the integrated design rather than to denominator power alone.
- Decision/next: make no further change before the 500-step and 1,000-step validations. Compare against route-key v4175 losses `6.3938/5.9113` and shared-key all-`0.5` losses `6.4173/6.0446` at steps `500/1,000`. Continue if the current run reaches approximately `6.40` at step 500 and `<=5.95` at step 1,000 while attention/RST/residual peaks subside, V top-1 falls below `0.9`, active/gate mass remains stable, and dead-operator/fallback counts do not rise. Reconsider the denominator and active-target combination if step-1,000 loss remains `>=6.0` with V top-1 still above `0.9`.
- Evidence boundary: every runtime number in this entry is user-provided and was not independently refreshed. Compile/HBM, steady step time, throughput, generated checkpoint identity, raw remote log location, and the 500/1,000-step results are `not measured` in this entry. No TPU command was run while translating and recording the report.

#### #12 — Implement and freeze the IOI validation intervention path

- Status/hypothesis: `local validated`; causal effects remain `not measured`. The discovery-frozen 4,096-site Q/K-centered circuit should reduce paired-correct IOI validation performance more than four matched control families, and exact same-example selected-numerator restoration should recover the intact margin. This entry fixes the executable path before validation and makes no causal claim.
- Source identity/change: canonical branch `codex/v4167-poc`, clean parent `a66e4d968b087e9ec60f6c6ab9e3f9a03fa95496`, source commit `1e82cd2dca8c32c9ce04682129da67729d3c2408`. The implementation adds a strict loader for `configs/paper_ioi_frozen_circuit_v4172_400m.yaml`, validation-only execution isolation, intact/frozen/control/restoration scoring on one shared retention graph, full-production-denominator suppression, exact selected-numerator reinsertion, aggregate confirmatory statistics, and the dedicated `ioi_frozen_validation` launcher preset. No frozen YAML field, selected site, layer, route, operator ID, score, suppression/restoration definition, seed, control family, replicate count, or threshold changed.
- Frozen/control contract: parsed frozen-spec hash `6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`; localization protocol hash `e0b7c4cb0245f1ed3a1e6be1a0c4ca4b837b6ff78d25224a8f588b8e77c1885c`; ranked-site hash `84f0ee8fa0a6aaa959a6140f0fa04b9b00b3b2c5c10e3dffba995eddd20f7d60`; selected-row hash `1439f6f3c3288818f6398f10a38620c9e1c348d7f064f8b1ac4fa59c05cfe0c1`; selected-site identity hash `573cfff883069bee821542bf24cee7a618a43195e8c22f4573f2ef7e18f60200`; Q/K/V/RST counts `1,744/2,350/2/0`. The frozen `nearest_bin_without_replacement` activation control is operationalized before validation as 100 equal-width empirical midrank-quantile bins within exact layer and route, using the frozen replicate count as the bin count and the frozen seed plus replicate index as the deterministic RNG identity; depleted target bins use the nearest nonempty bin without replacement.
- Local evidence: `C:\Users\MADST\Desktop\dawn-spatial\.venv-jax062\Scripts\python.exe` resolved Python/JAX/Flax/NumPy `3.12.7/0.6.2/0.10.7/2.5.1`. Module compilation, launcher `bash -n`, preset/dependency resolution, and `git diff --check` passed. A read-only local copy of the exact 101,232,291-byte discovery artifact resolved exactly 4,096 frozen sites and all expected hashes. Every one of the four control families produced 100 reproducible, unique 4,096-site identity hashes with zero frozen-site overlap; activation replicate 0 had mean/maximum empirical quantile distance `0.030013/0.126767`. These are structural/local checks, not TPU causal measurements.
- Storage/isolation/decision: the result path persists aggregate statistics and content hashes only; raw per-example logits, margins, operator vectors, and control site IDs remain transient. The preset accepts only its IOI input-contract dependency, directly opens only the physical validation shard, requires exactly `128` rows and `123` paired-correct independent examples, and emits an explicit `test_data_accessor_called=false` audit. Accept the implementation as ready for the user-authorized existing TPU `spatial-se-400m`; launch from one clean pushed canonical commit, preserve any negative result, finalize the validation record, and do not open held-out test in this attempt.

#### #13 — Stop an ambiguous-user IOI launcher attempt before validation access

- Status/result: `blocked before validation`; no IOI intervention metric was measured and held-out test was not opened. The first launch request targeted only the user-authorized existing TPU `spatial-se-400m`, target/runtime/preset `v4172_400M` / `v4-64` / `ioi_frozen_validation`, branch `codex/v4167-poc` at pushed commit `f07575eea5b4008b546e791e2f82db950066e568`, session `train`, and log `~/train.log`.
- Failure evidence: the TPU control plane reported exact accelerator `v4-64`, state `READY`, and health `HEALTHY`. The launcher reached its eight-worker SSH/setup call but its local gcloud command line was `tpu-vm ssh spatial-se-400m` without the required `madst0614@` target. It produced no validation cohort, condition, control, restoration, result-root, or test-access line. A separate status watcher used the correct explicit user but was still in SSH connection setup; both exact local gcloud SSH processes were stopped after their process IDs and command lines were verified.
- Fix/decision: source commit `cfb9d32947b426f6005441c3019b3eb74a6d48ab` adds one immutable `REMOTE_USER=madst0614` launcher constant and calls `tpu-vm ssh "$REMOTE_USER@$TPU_NAME"`; launcher `bash -n` and `git diff --check` passed. This changes only deployment identity, not the frozen YAML, sites, controls, thresholds, model intervention, or validation data. Publish the fix, verify the clean remote checkout under `/home/madst0614/DAWN-SRW`, and repeat the same validation-only preset; preserve this failed attempt as non-measurement.

#### #14 — Clear stale TPU ownership and restore the canonical 128-row validation selector

- Status/result: `failed before any intervention condition`; no intact, suppression, control, restoration, or held-out-test metric was measured. After all eight rotated TPU host keys were verified against authoritative `hostkeys/ssh-ed25519` guest attributes and updated locally, the explicit-user launcher deployed clean canonical commit `4939e02827ca9831e8af900d853f9854ed00aedb` on `spatial-se-400m`.
- Runtime attempt 1: a detached v4174 training process remained on every worker after its old tmux session was replaced and retained TPU ownership. The first analysis processes therefore timed out in `/tensorflow.CoordinationService/RegisterTask` before printing a run manifest. The exact residual command matched `scripts/train_jax.py --config configs/train_config_v4174_400M_c4_20B_v4_64.yaml`; only that command was terminated on all eight workers, after which both train and analysis residual-process checks returned empty.
- Runtime attempt 2: the clean relaunch restored checkpoint step `76293`, resolved 393,800,708 parameters, passed the immutable IOI input contract, and created result root `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_ioi_frozen_validation_20260729T113410Z-3acbc4e3`. It then failed at validation cohort loading with `expected=128 actual=10000`: the new path directly opened the complete prepared validation file and incorrectly treated its physical size as the preregistered runtime cap. No condition executable ran. Only validation was accessed; test was not accessed.
- Fix/decision: source commit `0e898f3fc837d9fc0b89d0bfd8e41e2272badcac` routes the frozen path through the existing canonical phase loader, which sorts by `(canonical_hash(example_id), example_id)` and takes the frozen runtime cap of 128; the already established paired-correct requirement remains exactly 123. It removes the false physical-shard-size claim and reports `runtime_selected_row_count=128`. Module compilation and `git diff --check` passed. No YAML, site, layer, route, operator ID, suppression/restoration definition, control rule/seed/count, threshold, or test-access policy changed. Publish this fix and rerun the same validation-only preset.

#### #15 — Make the unrelated-output audit compatible with the immutable IOI build

- Status/result: `failed before intact scoring`; no suppression, control, restoration, or held-out-test metric was measured. Clean canonical commit `96aaeeefdba820dbd7a482a5cf454933dabb4372` restored the v4172 400M checkpoint, passed the immutable input contract, selected the canonical 128 validation rows, and reproduced exactly 123 paired-correct units at result root `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_ioi_frozen_validation_20260729T113807Z-ff28c66a`. Batch preparation then failed because the published build has neither optional `semantic_token_spans` nor `semantic_char_spans`.
- Fix: source commit `170a4b2f73bc1fad73c5faf83f1552caeb2eb0b7` keeps optional exact spans when present and otherwise derives the three official IOI name mentions from immutable `base_prompt`, `subject`, and `indirect_object`, with alphabetic token boundaries and the pinned fast tokenizer offset mapping. The unrelated audit is now the mean next-token log probability on pre-S2 base-prompt tokens after excluding every subject/indirect-object name span; all tokens from S2 onward are excluded.
- Local evidence/decision: the exact immutable validation JSONL hash-bound by build `f056bb4f133cde4fc0ae02f9` was checked only for this schema compatibility. All canonical 128 rows produced valid masks, retained 6-13 unrelated pre-S2 target tokens, and reproduced the stored input IDs under tokenizer revision `86b5e0934494bd15c9632b12f734a8a67f723594`; module compilation and `git diff --check` passed. No outcome-dependent value was inspected, no condition executable ran, and test was not accessed. No frozen site, intervention, control, seed, replicate count, statistical threshold, or restoration rule changed. Publish and rerun the same validation-only preset.

#### #16 — Replace a false cross-graph equality assertion with an explicit numeric audit

- Status/result: `failed after the intact graph executed but before any confirmatory condition was accepted`. The run reproduced exactly 123 paired-correct validation units and executed the intervention graph's mode-0 intact condition, then stopped at a self-imposed cross-graph absolute-error assertion. Frozen suppression, all four control families, exact restoration, and held-out test were not executed. The only retained measurement is the operational graph-comparison diagnostic below; no causal IOI metric or claim is supported.
- Exact runtime identity: canonical branch `codex/v4167-poc`, clean deployed commit `a706e56634b2e017d7d0e0a90a0c38ddc09bed17`, user-authorized existing TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `16x2`, Python/JAX/Flax `3.10.12/0.6.2/0.10.7`, checkpoint step `76293`, 393,800,708 parameters, checkpoint-config hash `08733ae4fefdfcda2bb8e61e51a6e6fce40c0b0e4d84cb80d715085da645039b`, and result root `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_ioi_frozen_validation_20260729T115054Z-22b9e8c5`. The primary process mapped to host suffix `w-6`; the clean checkout had zero tracked changes and no analysis process remained after failure.
- Failure evidence/diagnosis: the old implementation required the production-diagnostics behavior graph and the separately compiled intervention-retention graph to match every correct/source log probability at absolute tolerance `1e-6`. Their observed maximum absolute difference was `0.08589529991149902`, so the run raised before the intact summary line. This tolerance is not present in `configs/paper_ioi_frozen_circuit_v4172_400m.yaml` and is not one of the frozen statistical gates. The confirmatory effects are paired differences against mode-0 intact values from the same retention graph used for suppression, controls, and restoration; the production-diagnostics values determine the frozen 123-unit eligibility cohort and remain a separate numerical reference rather than a bitwise-equivalence target.
- Correction: source commit `165b1d97a09278ce9abe6b63c864040e79c76559` removes only the invented `1e-6` abort. It records aggregate maximum/mean absolute log-probability and margin differences, both graphs' aggregate margin/accuracy, and the fraction of the 123 margin signs that agree. Exact sign agreement over all 123 is a non-preregistered execution-validity requirement for `strong_success` or `partial_success`; the three frozen statistical gates and their conjunction remain unchanged. No frozen YAML field, selected site, layer, route, operator ID, suppression/restoration definition, control rule/seed/count, resampling rule, threshold, input row, or test-access rule changed.
- Local evidence/decision: repository-local Python/JAX/Flax were `3.12.7/0.6.2/0.10.7`; module compilation and `git diff --check` passed. Publish the correction and this failure record on POC, deploy that exact clean SHA, and rerun only the validation preset. Preserve the numeric audit in the completed aggregate artifact and do not call any held-out-test accessor before that record is finalized.

#### #17 — Validate the frozen IOI Q/K-centered RW operator circuit

- Status/decision: `TPU measured; strong_success`. The unchanged discovery-frozen 4,096-site circuit passed suppression, all four 100-replicate matched-control comparisons, and exact restoration over 123 independent paired-correct validation units. The strongest supported statement is `validation_level_causal_ioi_qk_centered_rw_operator_circuit`. Held-out test was not opened, evaluated, selected on, or used to change the specification; this is validation-level evidence rather than the final paper-central result.
- Exact identity: canonical branch `codex/v4167-poc`, clean analysis commit `209147bbbdaae00ebdf64655b917a91f77d50b26`, implementation commit `1e82cd2dca8c32c9ce04682129da67729d3c2408` plus pre-effect plumbing corrections through `165b1d97a09278ce9abe6b63c864040e79c76559`, user-authorized existing TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, checkpoint step `76293`, 393,800,708 parameters, checkpoint identity `a7ce8afcd0242bc4e9b567c9e5066c36ca223461eaa6ae6f251e6525d1f91c17`, and checkpoint-config hash `08733ae4fefdfcda2bb8e61e51a6e6fce40c0b0e4d84cb80d715085da645039b`. Remote Python/JAX/Flax/Optax/NumPy were `3.10.6/0.6.2/0.10.7/0.2.8/2.2.6`; this corrects the `3.10.12` Python value mistakenly transcribed in `2026-07-29/#16`.
- Frozen/input contract: parsed spec hash `6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`; selected-row hash `1439f6f3c3288818f6398f10a38620c9e1c348d7f064f8b1ac4fa59c05cfe0c1`; selected-site hash `573cfff883069bee821542bf24cee7a618a43195e8c22f4573f2ef7e18f60200`; Q/K/V/RST counts `1,744/2,350/2/0`; all 18 layers; selection recomputed `false`; operator IDs, layers, and routes changed `false`. Immutable benchmark build `f056bb4f133cde4fc0ae02f9` and canonical first-128 selector reproduced base/source/pair accuracy `0.9765625/0.984375/0.9609375` and exactly 123 paired-correct units with ID hash `0fca6266052fd5660e5cbae0f2d47ed75833a213225311c0bba3aeb270e3d4e4`.
- Intact/numeric audit: the intervention-graph intact correct-minus-source summed log-probability margin was `3.465456200`, exact accuracy `1.0`, mean correct/source log probability `-1.147867836/-4.613324035`, and mean unrelated log probability `-3.772021728`. All 123 intact margin signs matched the production-diagnostics reference; mean/max absolute cross-graph log-probability error was `0.022984965/0.085895300`, and mean/max absolute margin error was `0.034557109/0.141799927`. These values are an execution audit, not a new frozen statistical threshold.
- Frozen suppression: mean margin fell to `-4.399331298`, a drop of `7.864787498` with 95% bootstrap CI `[7.490079843, 8.241263373]`. Exact accuracy fell from `1.0` to `0.008130081`; prediction flip fraction was `0.991869919`. Mean correct log probability changed `-6.612009400` with CI `[-6.932586042, -6.320412833]`, source log probability changed `+1.252778098`, and source-minus-correct direction changed `+7.864787498` with CI `[7.493997796, 8.252282615]`.
- Matched controls: every family generated 100 unique 4,096-site identity hashes, excluded all frozen sites, and retained its frozen seed and sampling rule. Equal-count, same-layer, activation-matched, and route-frequency-matched mean margin drops were `0.070885847`, `0.039388633`, `5.400490372`, and `0.219161851`. Frozen-minus-control differences were respectively `7.793901651`, `7.825398865`, `2.464297126`, and `7.645625648`; their 95% bootstrap CIs were `[7.437851418, 8.161572163]`, `[7.440628550, 8.180287869]`, `[2.076200078, 2.832697266]`, and `[7.289511950, 8.031625154]`. Every frozen effect exceeded every replicate, and each two-sided paired permutation p-value was `0.0004997501` with 2,000 permutations. Activation matching used the frozen 100-bin rule; across replicates its mean empirical-quantile distance was `0.030023305`, maximum distance was `0.128506197`, mean bin distance was `2.934570313`, and maximum bin distance was `12`.
- Restoration/selectivity: exact same-example selected-numerator restoration returned mean margin to `3.462791257` and exact accuracy to `0.991869919`; residual mean margin drop was `0.002664943`. Recovery fraction was `0.999661155` with 95% bootstrap CI `[0.998654544, 1.000584125]`, passing the frozen CI-low minimum `0.80`. Suppression unrelated-output damage was `0.445387783` with CI `[0.346816056, 0.534373224]`; relative to correct-log-probability damage `6.612009400`, its absolute ratio was `0.067360428`. Restoration reduced unrelated damage to `0.000584753`. The frozen spec defines no formal unrelated-output threshold, so this remains a reported selectivity audit.
- Gate result: suppression CI-low above zero passed at `7.490079843`; every frozen-minus-control CI-low above zero passed, with activation-matched the tightest at `2.076200078`; restoration recovery CI-low passed at `0.998654544`; and the non-preregistered all-123 sign-agreement execution guard passed. The preregistered three-gate conjunction is `true`; validation data changed no specification field.
- Runtime/storage audit: result root `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_ioi_frozen_validation_20260729T120132Z-ac1fc0cb`; artifact protocol hash `f2906889ebf290b3373d315c2c49b4e15352dadba02c71ddfabc0fd49c085bec`; validation JSON SHA-256 `c29124836394f6c51ad034dda9141ec3a187e75736b27bdbefca4992b63b2b05`. All eight workers resolved the same clean analysis SHA with zero tracked changes, and a 5,000-line all-worker error-pattern audit returned no match. The root has six objects totaling 210,040 bytes. Semantic traversal found 44 arrays, none of length 123 and none longer than 100; the length-100 arrays are aggregate control distributions or control identity hashes. Raw per-example logits, margins, operator vectors, dense capture rows, control site IDs, and raw parameters were not persisted.
- Decision/next: accept the validation-level causal identification of an IOI-supporting Q/K-centered RW operator circuit. Freeze this record without changing the 4,096 sites, score, intervention, controls, seeds, resampling, thresholds, or restoration. Only after this entry is committed and pushed may the 114-unit held-out test accessor be opened; test must use the same frozen hashes and must be reported separately even if negative.

#### #18 — Open only the frozen IOI held-out test path after final validation publication

- Status/split boundary: `local validated; held-out test not yet accessed`. The strong validation result was already finalized and pushed on canonical POC at record commit `3b2f04aae3afe49fd1040c99e667acf50daadefc` before this implementation began. Its analysis commit `209147bbbdaae00ebdf64655b917a91f77d50b26`, checkpoint step `76293`, frozen parsed-spec hash `6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`, result root ending `20260729T120132Z-ac1fc0cb`, and validation-artifact SHA-256 `c29124836394f6c51ad034dda9141ec3a187e75736b27bdbefca4992b63b2b05` are now explicit non-computational lineage metadata in the test artifact.
- IOI source change: IOI-specific commit `bf45c76b` adds only the dedicated `mib_ioi.frozen_circuit_test` item and `ioi_frozen_test` launcher preset, test constants `128/114`, split-isolation reporting, and a phase argument around the existing frozen-circuit body. Validation and test call the same `_run_frozen_circuit_phase` implementation and therefore the same frozen loader, circuit, full-denominator suppression, `FrozenIOIControlSampler`, four control seeds and sampling rules, 100 replicates, bootstrap/permutation counts and seeds, gate calculations, exact restoration, unrelated-output audit, and aggregate-only storage path. The test positive claim label changes only after the same three-gate conjunction passes.
- Frozen-diff audit: `configs/paper_ioi_frozen_circuit_v4172_400m.yaml`, `analysis/operator_interpretability/intervention.py`, `models/dawn_srw_v4171.py`, `models/dawn_srw_v4172.py`, and the v4172 400M target config have no IOI test diff. Selected sites remain exactly 4,096 with Q/K/V/RST `1,744/2,350/2/0`; selection, operator IDs, layers, routes, candidate score, suppression, controls, seeds, resampling, thresholds, and restoration were not changed or recomputed.
- Isolation/local evidence: repository-local Python/JAX/Flax were `3.12.7/0.6.2/0.10.7`. Module compilation, launcher `bash -n`, `git diff --check`, and launcher dry-run passed. The parsed YAML reproduced hash `6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`; test constants resolved `128/114`; `ioi_frozen_test` resolved only `mib_ioi.frozen_circuit_test`, and its complete dependency closure was exactly `mib_ioi.input_contract` plus that test item. The validation preset retained its separate two-item closure. No local or remote command opened a test shard in this entry.
- Shared-worktree boundary: while the four IOI files were being committed, user-owned staged changes to `configs/train_config_v4175_40M_c4_5B.yaml` were concurrently included in parent commit `bf45c76b`; the user then added and pushed v4175-only commit `b4679e4284ece37470b57cc9502704bed6aeac82`. Rewriting that published shared history or reverting the user's config would be destructive, so both are preserved. Those changes affect only the v4175 40M config and cannot affect the v4172 400M target, checkpoint, model, frozen circuit, or intervention execution. The test run must record both the IOI-specific implementation commit and its actual clean deployed descendant SHA.
- Decision/next: publish this pre-test record, verify validation evidence still exists at the pinned root, deploy one exact clean POC descendant on the user-authorized existing TPU `spatial-se-400m`, and run only `ioi_frozen_test`. Preserve the completed result unchanged whether it is strong, partial, mixed, or negative.

#### #19 — Stop the first held-out run at a stale validation-only phase guard

- Status/result: `operational failure after test eligibility, before every intervention condition`; this is not a scientific test result. The held-out accessor was opened for the first time, the canonical first 128 test rows were evaluated behaviorally, and exactly 114 paired-correct independent units were reproduced. Batch preparation then raised `ValueError: frozen IOI circuit evaluation accepts validation examples only`. Intact, frozen suppression, all four matched-control families, restoration, bootstrap/permutation inference, and every frozen gate were not run, so no effect estimate or pass/fail claim exists.
- Exact runtime identity: canonical branch `codex/v4167-poc`, exact clean deployed commit `f0008b3279cf066826c12582acba670899b5b745`, IOI implementation commit `bf45c76b`, existing user-authorized TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, target `v4172_400M`, checkpoint step `76293`, 393,800,708 parameters, checkpoint-config hash `08733ae4fefdfcda2bb8e61e51a6e6fce40c0b0e4d84cb80d715085da645039b`, immutable benchmark build `f056bb4f133cde4fc0ae02f9`, preset `ioi_frozen_test`, tmux `train`, and log `~/train.log`. The failed result root is `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_ioi_frozen_test_20260729T123225Z-61b57ea1`.
- Diagnosis/correction: the dedicated runner correctly enforced split isolation, loaded phase `test`, and checked the frozen `128/114` counts, but two lower-level batch/condition guards still admitted only phase `validation`. Source commit `1a643d330d8032008baf3c276471773599bc8b7d` replaces only those guards with admission for one homogeneous `mib_ioi` phase in `{validation, test}`. The scoring executable, retention modes, masks, full production denominator, selected sites, control sampler, seeds, replicate counts, statistics, thresholds, restoration, and storage semantics are unchanged.
- Local evidence/decision: repository-local Python/JAX/Flax were `3.12.7/0.6.2/0.10.7`; module compilation and `git diff --check` passed. No test/debug file was created or run. Publish this operational incident and correction on POC, then rerun the exact same test preset. Do not interpret or tune against this attempt, and preserve the completed rerun unchanged even if negative.

#### #20 — Confirm the frozen IOI circuit on fully held-out test

- Status/decision: `TPU measured; strong_success`. The unchanged discovery-frozen 4,096-site circuit passed frozen suppression, separation from all four 100-replicate matched controls, and exact restoration over 114 independent paired-correct held-out test units. The artifact claim is `held_out_test_confirmed_causal_ioi_qk_centered_rw_operator_circuit`. Together with `2026-07-29/#17`, this supports the paper-central statement that the discovery-identified Q/K-centered RW operator circuit is causally necessary for IOI computation on independent validation and fully held-out test and is recoverable by exact selected-numerator restoration.
- Exact identity: canonical branch `codex/v4167-poc`, clean deployed analysis commit `75b0e0157bce670d1ca6f7801a8393014ce09ffb`, phase-admission correction commit `1a643d330d8032008baf3c276471773599bc8b7d`, user-authorized existing TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, target/config `v4172_400M` / `configs/train_config_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2.yaml`, checkpoint step `76293`, 393,800,708 parameters, checkpoint identity `a7ce8afcd0242bc4e9b567c9e5066c36ca223461eaa6ae6f251e6525d1f91c17`, and checkpoint-config hash `08733ae4fefdfcda2bb8e61e51a6e6fce40c0b0e4d84cb80d715085da645039b`. Remote Python/JAX/Flax/Optax/NumPy were `3.10.6/0.6.2/0.10.7/0.2.8/2.2.6`.
- Frozen/input contract: parsed spec hash `6ec2661823bb52aa3bc336fb69fed1eef317f9cced02f836eaaade8c116a418c`; selected-row hash `1439f6f3c3288818f6398f10a38620c9e1c348d7f064f8b1ac4fa59c05cfe0c1`; selected-site hash `573cfff883069bee821542bf24cee7a618a43195e8c22f4573f2ef7e18f60200`; Q/K/V/RST counts `1,744/2,350/2/0`; selection recomputed `false`; operator IDs, layers, and routes changed `false`. Immutable build `f056bb4f133cde4fc0ae02f9` and canonical first-128 test selector produced base/source/pair accuracy `0.9921875/0.8984375/0.890625`, exactly 114 paired-correct units, and eligible-ID hash `0e899f03d6c45901335ddaaa5fe16699e4f06db0c6382553740ed533c8f7e3b6`.
- Intact and frozen suppression: the primary score is correct-minus-source summed log-probability margin. Intact margin/accuracy were `3.254719971/1.0`; suppression moved margin to `-4.155612523`, a drop of `7.410332494` with 95% bootstrap CI `[7.016484716, 7.818472028]`. Exact accuracy fell to `0.008771930`, and prediction flip fraction was `0.991228070`. Correct log probability changed `-6.807139746` with CI `[-7.107149407, -6.510976167]`; source log probability changed `+0.603192748`; source-minus-correct direction changed `+7.410332494` with CI `[7.027271035, 7.789024565]`.
- Matched controls: equal-count, same-layer, activation-matched, and route-frequency-matched mean margin drops were `0.080275917`, `0.041072708`, `5.472578673`, and `0.242664667`. Frozen-minus-control differences were respectively `7.330056576`, `7.369259786`, `1.937753821`, and `7.167667826`; their 95% CIs were `[6.922408166, 7.716879142]`, `[6.977447319, 7.754538899]`, `[1.564817525, 2.319706012]`, and `[6.779836211, 7.545537209]`. Frozen exceeded every replicate in every family; each two-sided paired permutation p-value was `0.0004997501` with 2,000 permutations. Every family retained its frozen seed/rule, produced 100 unique 4,096-site hashes, and excluded every frozen site. Activation matching retained the 100-bin rule; mean empirical-quantile distance was `0.030023305`, maximum distance was `0.128506197`, mean bin distance was `2.934570313`, and maximum bin distance was `12`.
- Restoration/selectivity: exact selected-numerator restoration returned margin to `3.242905266` and accuracy to `1.0`; residual margin drop was `0.011814704`. Recovery fraction was `0.998405645` with 95% bootstrap CI `[0.997323737, 0.999385266]`, far above the frozen `0.80` CI-low threshold. Suppression unrelated-output damage was `0.465383629` with CI `[0.388851815, 0.544581609]`; relative to correct-log-probability damage `6.807139746`, its absolute ratio was `0.068366986`. Restoration unrelated damage was `-0.000760603`. The frozen spec defines no formal unrelated-output threshold, so this remains a selectivity audit rather than a preregistered gate.
- Gate/split audit: suppression CI-low above zero, every frozen-minus-control CI-low above zero, restoration-recovery CI-low above `0.80`, and the non-preregistered all-114 intact-sign execution guard all passed. The three preregistered gates and their conjunction are `true`, with thresholds read unchanged from `frozen_spec.evaluation.validation_gates`. The artifact binds prior validation analysis/record commits `209147bbbdaae00ebdf64655b917a91f77d50b26` / `3b2f04aae3afe49fd1040c99e667acf50daadefc`, validation JSON SHA-256 `c29124836394f6c51ad034dda9141ec3a187e75736b27bdbefca4992b63b2b05`, and the validation result root. It records test accessor/evaluation `true`, evaluation count `114`, validation used to change specification `false`, and configuration changed after validation `false`.
- Runtime/storage audit: result root `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_ioi_frozen_test_20260729T123731Z-3cb89374`; artifact protocol hash `da2a734c1597aa30b0e6fbc3b9e3e067c117cbf08b1dfa19012c6186051a861a`; test JSON SHA-256 `f559383fba409e5c9d171b6012b7c872457992ab6da78f18cab4c7568c4b207f`, size 162,686 bytes. The root has six objects totaling 206,117 bytes. Semantic traversal found 79 arrays, none of length 114 and none longer than 100; the maximum-length arrays are aggregate control distributions or identity hashes. Raw per-example logits/margins/operator vectors, control site IDs, dense capture rows, and raw parameters were not persisted. All eight workers resolved the same clean analysis SHA with zero tracked changes, and every completed-run log had zero error-pattern matches.
- Decision/next: promote IOI to the paper's central causal result without changing or rerunning its frozen specification. Preserve the pre-condition operational incident in `2026-07-29/#19` separately from this valid test result. Proceed to RAVEL localization and variable transfer, then ARC auxiliary causal replication.

#### #21 — Preregister the RAVEL discovery-only variable-circuit localization

- Status/split boundary: `implementation committed and locally validated; discovery not yet measured`. IOI is closed and no IOI configuration, circuit, intervention, control, result, model, or v4172 target file changed. The new preset may access only the RAVEL discovery shard; validation and test accessors are forbidden and are not dependencies of the localization item.
- Exact identity: canonical branch `codex/v4167-poc`, exact implementation commit `ac37902f67ca2148250f9ff1b1f4974cb45a870e`, target/config `v4172_400M` / `configs/train_config_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2.yaml`, checkpoint step `76293`, 393,800,708 parameters, checkpoint identity `a7ce8afcd0242bc4e9b567c9e5066c36ca223461eaa6ae6f251e6525d1f91c17`, checkpoint-config hash `08733ae4fefdfcda2bb8e61e51a6e6fce40c0b0e4d84cb80d715085da645039b`, model-config hash `053a0b3d7cb8bdfffa2aa24441012b809dbd5f450728af6df0269645b266f52f`, immutable benchmark build `f056bb4f133cde4fc0ae02f9`, and benchmark-manifest hash `f3fc05c12ac26c83e0cdf8089ef46bae7f26b517265545521605d767a8eafcb6`.
- Discovery cohort: use the canonical balanced 512-row discovery selector and require exactly 142 paired-correct independent `pair_group_id` units before operator capture. Rank `Continent`, `Country`, and `Language` separately, with discovery groups balanced within causal variable and official counterfactual source column. The pooled ranking and pooled stability are audit-only and cannot select a variable circuit.
- Preregistered freeze rule: parsed specification `configs/paper_ravel_discovery_localization_v4172_400m.yaml` has canonical content hash `812ea210137a7fc4b2d5956f59975a5c8efbb8881569110455add15a67aa10df`. Use seed `4172`, production-precision contribution capture, capture-mass threshold `0.95`, initial QK/V/RST widths `512/2048/4096`, and maxima `2048/8192/8192`. For each variable, require split-rank stability at least `0.80`, then choose the smallest audited prefix in `32/64/128/256/512/1024/2048/4096` whose cumulative absolute discovery importance is at least `0.70` and split top-k overlap is at least `0.95`. If stability or every prefix fails, freeze no circuit for that variable and do not retune. Layer, route, and operator composition—including any V or RST presence—is a discovery result and cannot be manually included or excluded.
...
#### #23 — Preregister ARC discovery-only operator localization

- Status/split boundary: `implementation committed and locally validated; discovery not yet measured`. IOI remains closed and the RAVEL negative result remains frozen. The ARC preset may access only the `mib_arc` discovery shard; validation and test accessors are forbidden and are not dependencies of the localization item.
- Exact identity: canonical branch `codex/v4167-poc`, exact implementation commit `eae8b0fdb32d5f1fdfe4568604e8c843f634af3b`, target/config `v4172_400M` / `configs/train_config_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2.yaml`, checkpoint step `76293`, 393,800,708 parameters, checkpoint identity `a7ce8afcd0242bc4e9b567c9e5066c36ca223461eaa6ae6f251e6525d1f91c17`, checkpoint-config hash `08733ae4fefdfcda2bb8e61e51a6e6fce40c0b0e4d84cb80d715085da645039b`, model-config hash `053a0b3d7cb8bdfffa2aa24441012b809dbd5f450728af6df0269645b266f52f`, immutable benchmark build `f056bb4f133cde4fc0ae02f9`, and benchmark-manifest hash `f3fc05c12ac26c83e0cdf8089ef46bae7f26b517265545521605d767a8eafcb6`.
- Discovery cohort and split: use the canonical first 128 ARC discovery rows and require exactly 50 paired-correct independent `example_id` units before capture. Sort those units by the seed-4172 hash of benchmark and example identity, then assign alternating splits, requiring exact split counts `25/25`. The split rule is fixed before any localization value is observed.
- Preregistered freeze rule: parsed specification `configs/paper_arc_discovery_localization_v4172_400m.yaml` has canonical content hash `70ff8bcab6b1ad0448c29c582f0fc204e834a4b7c9800e925591d32f82cc6b7b`. Use production-precision contribution capture, capture-mass threshold `0.95`, initial QK/V/RST widths `512/2048/4096`, and maxima `2048/8192/8192`. Require split-rank stability at least `0.80`, then choose the smallest audited prefix in `32/64/128/256/512/1024/2048/4096` whose cumulative absolute discovery importance is at least `0.70` and split top-k overlap is at least `0.95`. If stability or every prefix fails, set `selected_k=0`, freeze no circuit, and do not retune routes, layers, operators, prefix counts, thresholds, seed, or cohort.
- Confirmatory contract fixed before discovery: only if a circuit freezes, validation uses the already-screened 51 paired-correct independent units and exactly `intact`, full-denominator selected-numerator suppression, one matched-random family with 100 replicates, and exact same-example selected-numerator restoration. Matched random uses seed `24172`, matches the frozen count exactly in every layer-by-route cell, samples uniformly without replacement from that cell's frozen-site complement, excludes every frozen site, forbids within-replicate duplicates, and requires 100 unique control-site hashes. Use 2,000 bootstrap and permutation samples at alpha `0.05`; require suppression margin-drop CI low above zero, frozen-minus-control margin-drop CI low above zero, and restoration-recovery CI low at least `0.80`. Unrelated-output damage is a selectivity audit without a formal threshold. Test may open only after the validation record is final and must reuse the same circuit, suppression, control rule, restoration, and thresholds over the fixed 57 paired-correct units.
- Implementation/storage: dedicated item `mib_arc.discovery_operator_localization` has complete dependency closure exactly `mib_arc.input_contract` plus that item. It evaluates behavior only on discovery, verifies the exact paired-correct count and unique independent identities, captures all Q/K/V/RST routes, applies the fixed stability/prefix gates, and persists only aggregate rankings, circuit identities, and hashes. Raw per-example behavior vectors, per-example operator vectors, raw capture rows, dense captures, and parameters are not durable artifacts.
- Local evidence: repository-local Python/JAX/Flax were `3.12.7/0.6.2/0.10.7`. Module compilation, spec hash and semantic validation, exact item dependency closure, Git Bash launcher syntax, `git diff --check`, the exact `spatial-se-400m` launcher dry-run, and an IOI/RAVEL frozen-file diff audit passed. No debug script or test file was created or run. ARC rank stability, prefix overlap, circuit size, and layer/route/operator composition are `not measured` in this entry.
- Execution/decision: publish this pre-effect record before launch, then deploy its exact clean POC descendant on the existing user-authorized `spatial-se-400m` v4-64 with mesh `data=16, model=2`, tmux `train`, and `~/train.log`. Run only preset `arc_discovery_localization`; preserve the result without changing the preregistered contract. Do not open ARC validation or test in this attempt.

#### #24 — Freeze the preregistered ARC discovery circuit

- Status/decision: `TPU measured; ready; circuit frozen`. The discovery-only run passed the fixed `0.80` split-rank-stability gate and the `k=4096` prefix passed both discovery gates. The exact 4,096-site circuit is eligible for the preregistered ARC validation protocol. Validation and test were not opened, and no result-dependent route, prefix, threshold, control, seed, or cohort change was made.
- Exact identity: canonical branch `codex/v4167-poc`, implementation commit `eae8b0fdb32d5f1fdfe4568604e8c843f634af3b`, exact clean deployed/analysis commit `7d94dc303162b727192b9721b354fc88744ca2f0`, existing user-authorized TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, target/config `v4172_400M` / `configs/train_config_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2.yaml`, checkpoint step `76293`, 393,800,708 parameters, checkpoint identity `a7ce8afcd0242bc4e9b567c9e5066c36ca223461eaa6ae6f251e6525d1f91c17`, checkpoint-config hash `08733ae4fefdfcda2bb8e61e51a6e6fce40c0b0e4d84cb80d715085da645039b`, and model-config hash `053a0b3d7cb8bdfffa2aa24441012b809dbd5f450728af6df0269645b266f52f`. The artifact protocol records branch, exact commit, and `git_dirty=false`. The no-install launcher reused the exact-resource environment recorded immediately before this run in `#22`; Python/JAX/Flax/Optax/NumPy were not freshly re-read before the TPU entered `DELETING`, so no new version measurement is claimed.
- Frozen discovery contract: source preregistration path/hash `configs/paper_arc_discovery_localization_v4172_400m.yaml` / `70ff8bcab6b1ad0448c29c582f0fc204e834a4b7c9800e925591d32f82cc6b7b`; immutable benchmark build/manifest hash `f056bb4f133cde4fc0ae02f9` / `f3fc05c12ac26c83e0cdf8089ef46bae7f26b517265545521605d767a8eafcb6`; runtime protocol hash `16440596df4d4a4f07e604659214b7d78166aae5e350cdbb121ed66e8062a33a`; seed `4172`; discovery row cap `128`; capture threshold `0.95`; initial QK/V/RST widths `512/2048/4096`; maxima `2048/8192/8192`; stability threshold `0.80`; prefixes `32/64/128/256/512/1024/2048/4096`; cumulative-importance/overlap thresholds `0.70/0.95`.
- Cohort/capture: the canonical 128 discovery rows reproduced base/source/pair accuracy `0.9765625/0.4140625/0.390625`, exactly 50 paired-correct independent `example_id` units, and eligible-ID hash `8f935043d48e9fbecc1aed1dd604f8325606084e416d650abca6af181714017b`. The preregistered balanced split was exactly `25/25`. One width retry produced final QK/V/RST widths `1024/4096/8192`; all `3,600/3,600` layer-route rows qualified. The ranking contained 341,062 sites with content hash `cddb10f0a2f0cbfe5a6d002d5b629a7a79b1120c7474350d2ed6cf8eea39e5e7`; raw capture digest was `5f5d15e0094e1f6d9ab2d61a76e15bf1d7977fc4c05066e6b32a8e08130b9349`.
- Discovery gates: split-rank stability was `0.893860449`, above `0.80`. At prefixes `32/64/128/256/512/1024/2048`, cumulative importance was `0.045605/0.082751/0.138671/0.201373/0.278917/0.383045/0.526492`, so none passed both gates even where overlap exceeded `0.95`. At `k=4096`, cumulative importance was `0.709301488` and split overlap was `0.957763672` (`3,923/4,096`, Jaccard `0.918950574`), so it was the smallest and only passing prefix.
- Frozen composition: selected Q/K/V/RST counts are `1,770/2,326/0/0`; zero V and RST presence is a discovery result. All 18 layers are represented. Selected-site totals peak at layer 14 with 409 sites, then layers 13/11/9 with `322/287/282`. The total ranking importance fractions are Q/K/V/RST `0.416102/0.568112/0.009021/0.006765`; first V and RST ranks are `4,230` and `8,041`, outside the selected prefix. The top site is layer-5 K operator 547. Selected-ranked-row, site-identity, and circuit hashes are `a5344782d588aeacc08fa8752d4a2471027bbb6deecd0aa0e5ad113254e48874`, `96bf2710cfdcb34894ff7154d8ccdd5d732f0fa4c52e4722e35ee2d9d2238fad`, and `ef8d58ecb44b05fc86b1cee81302eb58c69151713e02fa85e0af82547f43b414`.
- Frozen file: `configs/paper_arc_frozen_circuit_v4172_400m.yaml` has canonical content hash `49f3a7dc83d3212f786666d74f77d62c7ccb749fb3df6388ac524c9209728600`. It binds the exact result item hash, ranked-site hash, first-4,096 site resolution, all 18 layer-by-route cell counts, full-denominator selected-numerator suppression, exact same-example selected-numerator restoration, and the seed-24172 100-replicate random control sampled uniformly without replacement from the frozen complement while matching every layer-by-route count exactly.
- Split/storage audit: only `mib_arc.input_contract` and `mib_arc.discovery_operator_localization` executed. The artifact records validation/test accessor called `false`, evaluation count `0`, and used for selection `false`. Item JSON size/SHA-256 are `81,425,519` bytes / `32059614c227892a5231976aae3a529d42a949057150df1865dd1bc3773f5e1a`; the six-object root totals `81,465,996` bytes. The only long arrays are the aggregate 341,062-site ranking and exact 4,096 frozen identities. Exact-key scanning found zero durable per-example logits, margins, known-correct masks, operator vectors, raw capture rows, inputs, or parameters. The item-size warning was advisory and writing completed.
- Runtime boundary/evidence: started `2026-07-29T14:04:41Z` and completed `14:08:31Z`. Result root `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_arc_discovery_localization_20260729T140441Z-a987a54d`; item `items/mib_arc/discovery_operator_localization.json`. Launcher output showed all eight hosts fetched and reset to the pushed branch before starting the identical command; the completed artifact independently binds exact clean commit `7d94dc303162b727192b9721b354fc88744ca2f0`. A post-run all-host SSH audit could not execute because the TPU had already entered state `DELETING`; no TPU was created, resized, or replaced.
- Lesson/next: ARC discovery localization is stable enough to proceed, but it is not yet causal evidence. Implement the frozen validation path from the exact frozen file and then evaluate only intact, frozen suppression, the single 100-replicate layer-route-exact matched-random family, and exact restoration over the fixed 51 validation units. Do not open held-out test unless validation passes and its record is final.

#### #25 — Freeze the ARC validation implementation before effect access

- Status/split boundary: `implementation committed and locally validated; validation effect not yet measured`. IOI remains closed, the RAVEL negative result remains frozen, and ARC held-out test remains unopened. Dedicated item `mib_arc.frozen_circuit_validation` has complete dependency closure exactly `mib_arc.input_contract` plus that validation item. The runner permits only this closure and calls only the ARC validation accessor; discovery is read exclusively through the hash-bound aggregate localization artifact, and no test item or test runner path exists in this implementation.
- Exact identity: canonical branch `codex/v4167-poc`, exact implementation commit `4da980ea1d3faf52e46c1d8631cf919832fb528e`, target/config `v4172_400M` / `configs/train_config_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2.yaml`, checkpoint step `76293`, 393,800,708 parameters, checkpoint identity `a7ce8afcd0242bc4e9b567c9e5066c36ca223461eaa6ae6f251e6525d1f91c17`, checkpoint-config hash `08733ae4fefdfcda2bb8e61e51a6e6fce40c0b0e4d84cb80d715085da645039b`, model-config hash `053a0b3d7cb8bdfffa2aa24441012b809dbd5f450728af6df0269645b266f52f`, immutable benchmark build `f056bb4f133cde4fc0ae02f9`, and benchmark-manifest hash `f3fc05c12ac26c83e0cdf8089ef46bae7f26b517265545521605d767a8eafcb6`.
- Frozen inputs: `configs/paper_arc_frozen_circuit_v4172_400m.yaml` remains unchanged at canonical content hash `49f3a7dc83d3212f786666d74f77d62c7ccb749fb3df6388ac524c9209728600`. The loader also verifies discovery protocol hash `16440596df4d4a4f07e604659214b7d78166aae5e350cdbb121ed66e8062a33a`, clean discovery analysis commit `7d94dc303162b727192b9721b354fc88744ca2f0`, ranked-site content hash `cddb10f0a2f0cbfe5a6d002d5b629a7a79b1120c7474350d2ed6cf8eea39e5e7`, selected-ranked-row hash `a5344782d588aeacc08fa8752d4a2471027bbb6deecd0aa0e5ad113254e48874`, selected-site hash `96bf2710cfdcb34894ff7154d8ccdd5d732f0fa4c52e4722e35ee2d9d2238fad`, and frozen circuit hash `ef8d58ecb44b05fc86b1cee81302eb58c69151713e02fa85e0af82547f43b414`. It reconstructs exactly 4,096 sites with Q/K/V/RST counts `1,770/2,326/0/0` and verifies every one of the 18 layer-by-route cell counts; it never recomputes a ranking or prefix.
- Fixed intervention/statistics: validation requires the canonical first 128 validation rows and exactly 51 paired-correct independent `example_id` units. Conditions are intact, full-production-denominator suppression of the frozen selected numerator, one 100-replicate layer-route-exact matched-random family, and exact same-example selected-numerator restoration. Bootstrap and permutation counts remain `2,000/2,000`, alpha remains `0.05`, and the three gates remain suppression margin-drop CI low above zero, frozen-minus-control CI low above zero, and restoration-recovery CI low at least `0.80`. Failure or mixed results are persisted without route, operator, layer, prefix, threshold, seed, or cohort adjustment.
- Matched-control algorithm frozen before effect access: use base seed `24172` and NumPy `SeedSequence([base_seed, replicate_index])`; iterate layers ascending and routes in canonical Q/K/V/RST order; within each nonempty layer-route cell sample its exact frozen count uniformly without replacement from that cell's frozen-site complement; then canonicalize the set by layer, route order, and operator ID. Frozen sites and within-replicate duplicates are forbidden, and all 100 site-set hashes must be unique. The local exact-artifact audit produced 100 unique hashes; replicate zero has site-set hash `2ee639cd7c762811b9a467d2c5dfe682c1900657f1219bf53f9fcc534fe524de`.
- Unrelated-output audit frozen before effect access: for ARC, measure mean next-token log probability on the entire base prompt excluding only the first token. It does not include the answer suffix, has no formal threshold, and is never used in the three preregistered gates. Raw per-example answer logits, margins, operator vectors, and control site IDs are transient; durable output contains only aggregate statistics, distributions, frozen hashes, and the 100 control site-set hashes.
- Local evidence: repository-local Python/JAX/Flax were `3.12.7/0.6.2/0.10.7`. Module compilation and import, exact 81,425,519-byte discovery-artifact/spec loading, selected-prefix and every layer-route count audit, all-100 control generation, item dependency closure, Git Bash launcher syntax, exact `arc_frozen_validation` dry-run, and `git diff --check` passed. The launcher remains fixed to tmux `train` and `~/train.log`. No debug script or test file was created or run; ARC validation effect, restoration, control separation, and held-out test are `not measured` in this entry.
- Execution decision: a live read-only resource check found the existing user-authorized `spatial-se-400m` `READY` with accelerator type v4-64 and eight network endpoints. Publish this pre-effect notebook record, then launch only `arc_frozen_validation` from its exact clean pushed POC descendant using target `v4172_400M`, runtime `v4-64`, mesh `data=16, model=2`, `--no-install`, and `--replace`. Preserve the result even if negative. Do not implement or open ARC held-out test until the validation result is complete and its canonical record is final.

### 2026-07-30

#### #01 — Resolve pre-effect ARC validation TPU startup conflicts

- Status/claim boundary: `operational incident; no scientific effect estimate`. After the user reported that the existing authorized `spatial-se-400m` was back online, a live describe showed `READY`, `HEALTHY`, v4-64, and eight endpoints. The recreated endpoints reused external addresses with changed PuTTY host keys, so connectivity was restored only for those eight described workers. This did not access any benchmark split or model effect.
- Exact launch boundary: all workers fetched and reset to exact clean pushed commit `453cd03095f2cb0763a19892674ea1c0e7fd6cba` on `codex/v4167-poc` and attempted only preset `arc_frozen_validation`, target `v4172_400M`, runtime `v4-64`, mesh `data=16, model=2`, `--no-install`, tmux `train`, and `/home/madst0614/train.log`. No circuit, route, threshold, control, seed, cohort, restoration, or audit definition changed.
- First conflict: auto-survived v4174 training processes with exact command `python3 -u scripts/train_jax.py --config configs/train_config_v4174_400M_c4_20B_v4_64.yaml` still occupied TPU devices on the restarted workers. They were identified before any ARC effect line and stopped as stale existing work so the authorized validation could own the pod.
- Second conflict: on the next identical attempt, one task reported that TPU was already in use by PID `56014`; the other tasks then aborted together at the eight-host shutdown barrier at `2026-07-29T15:06:38Z`. The owner disappeared with the failed distributed process group, and an all-host `python3` audit was empty before the final restart. No completed result item, behavioral-eligibility estimate, suppression estimate, control estimate, restoration estimate, or test access came from either failed attempt.
- Decision: classify these as pre-effect runtime incidents, not evidence for or against ARC. Relaunch the exact same clean commit and frozen validation command once the all-host TPU process audit is empty. The valid completed result is recorded separately in `2026-07-30/#02`.

#### #02 — Close ARC auxiliary causal replication as a frozen validation negative

- Status/decision: `TPU measured; complete; mixed_inconclusive; frozen scientific negative`. The unchanged 4,096-site ARC circuit completed the preregistered 51-unit validation, but suppression and frozen-minus-control both failed in the opposite direction. Exact restoration passed. The artifact has no strongest supported claim, and ARC held-out test must not open under this protocol.
- Exact identity: canonical branch `codex/v4167-poc`, validation implementation commit `4da980ea1d3faf52e46c1d8631cf919832fb528e`, exact clean deployed/analysis commit `453cd03095f2cb0763a19892674ea1c0e7fd6cba`, existing user-authorized TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, target/config `v4172_400M` / `configs/train_config_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2.yaml`, checkpoint step `76293`, 393,800,708 parameters, checkpoint identity `a7ce8afcd0242bc4e9b567c9e5066c36ca223461eaa6ae6f251e6525d1f91c17`, checkpoint-config hash `08733ae4fefdfcda2bb8e61e51a6e6fce40c0b0e4d84cb80d715085da645039b`, model-config hash `053a0b3d7cb8bdfffa2aa24441012b809dbd5f450728af6df0269645b266f52f`, immutable benchmark build `f056bb4f133cde4fc0ae02f9`, and benchmark-manifest hash `f3fc05c12ac26c83e0cdf8089ef46bae7f26b517265545521605d767a8eafcb6`. The no-install launcher did not freshly re-read the complete remote package-version set, so no new software-version measurement is claimed.
- Frozen protocol: frozen file/hash `configs/paper_arc_frozen_circuit_v4172_400m.yaml` / `49f3a7dc83d3212f786666d74f77d62c7ccb749fb3df6388ac524c9209728600`; circuit hash `ef8d58ecb44b05fc86b1cee81302eb58c69151713e02fa85e0af82547f43b414`; exactly 4,096 sites with Q/K/V/RST counts `1,770/2,326/0/0`; full-production-denominator selected-numerator suppression; 100 layer-route-exact matched-random replicates with seed `24172`; exact same-example selected-numerator restoration; 2,000 bootstrap and 2,000 permutation samples at alpha `0.05`. All 100 controls excluded frozen sites, matched every layer-route count, had 4,096 unique within-replicate sites, and had unique site-set hashes; replicate zero reproduced frozen hash-bound control identity `2ee639cd7c762811b9a467d2c5dfe682c1900657f1219bf53f9fcc534fe524de`.
- Cohort/reference: the canonical first 128 validation rows reproduced base/source/pair accuracy `0.96875/0.4296875/0.3984375` and exactly 51 paired-correct independent `example_id` units with eligible-ID hash `e1b3e749f61c712081fc96e1cb3c5ac381e8670a7c10fd266df870baed62a8e1`. Intact analysis margin was `1.735533766`, exact accuracy `1.0`, and all 51 margin signs agreed with the production diagnostic reference; production-reference mean margin was `1.741223494`.
- Suppression result: suppressed mean margin was `2.819138826`, so intact-minus-suppressed margin drop was `-1.083605060`, bootstrap CI `[-1.472180468, -0.690348370]`. Exact accuracy was `0.980392157`, prediction flip fraction `0.019607843`, mean correct log-probability change was `+0.303203877` with CI `[-0.124069133, 0.751964806]`, and source-direction change was `-1.083605060` with CI `[-1.484733233, -0.661956765]`. The suppression margin-drop CI-low-above-zero gate failed.
- Matched-control result: the 100-replicate mean margin drop was `-0.034123426`, with replicate range `[-0.323253501, 0.311017892]`, mean exact accuracy `0.999411765`, and mean flip fraction `0.000588235`. Frozen-minus-control margin drop was `-1.049481634`, bootstrap CI `[-1.440537537, -0.643905105]`; the two-sided paired permutation result was `p=0.000499750`. Zero control replicates lay below the frozen effect, and frozen exceeded neither the control mean nor every replicate. The frozen-minus-control CI-low-above-zero gate failed.
- Restoration/selectivity: restored mean margin was `1.731109699`, exact accuracy `1.0`, and prediction flips `0`. Recovery fraction was `1.004082730`, bootstrap CI `[0.994586533, 1.013652275]`, so the preregistered `CI low >= 0.80` restoration gate passed. Mean unrelated base-prompt log-probability damage was `1.872223321`, CI `[1.777368841, 1.969814377]`; correct-answer log-probability damage was `-0.303203877`, yielding an absolute damage ratio `6.174800064`. This unrelated audit had no formal gate and did not alter the decision.
- Split/storage audit: only `mib_arc.input_contract` and `mib_arc.frozen_circuit_validation` executed. The artifact records `test_used=false`, `test_data_accessor_called=false`, `test_evaluated=false`, and `test_evaluation_count=0`; configuration changed after discovery and validation used to change specification are both false. Raw per-example logits, margins, operator vectors, and control site IDs were not persisted; aggregate statistics, distributions, and the 100 control hashes were preserved, and storage audit status is `passed`.
- Runtime/evidence: the valid run started at `2026-07-29T15:09:36Z`; the result item was written at `15:10:44Z`. Result root `gs://dawn-tpu-data-c4/checkpoints/dawn_srw_v4172_400M_c4_40B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260715_133004_3201/side_analysis/run_analysis_000000076293_arc_frozen_validation_20260729T150935Z-18d6efcb`; item `items/mib_arc/frozen_circuit_validation.json`; artifact protocol hash `5d124ef8b26c9afe3f7188aa48b05cfc2f653830438432c1443235d7322a6c2a`; item size/SHA-256 `49,143` bytes / `b4b33f0120154a7ab8ca078a4f5e7a522790fda28af6fdba3b75294847b97136`. The run summary completed with no artifact warnings.
- Lesson/next: ARC shows that stable discovery localization alone is insufficient for causal necessity. Preserve the negative result, do not retune or open the fixed 57-unit held-out test, and leave IOI untouched as the paper-central causal result. Proceed to the matched dense Transformer 400M checkpoint identity, 10M-token C4 validation, and identical six-task zero-shot evaluation.

#### #03 — Freeze the matched dense Transformer 400M evaluation boundary

- Status/split boundary: `implementation committed and locally validated; C4 and zero-shot not measured`. The registry-designated baseline-JAX 400M run is now represented by canonical target `baseline_jax_400m`, whose checkpoint field points directly to committed Orbax step `000000076293` rather than to a moving run root. No baseline model, checkpoint, training config, evaluation kernel, dataset, task, or metric definition changed.
- Exact implementation identity: canonical branch `codex/v4167-poc`, clean parent `8b01585e199a1e5a0554417d3cf1f8c29600618e`, exact target/gate implementation commit `043c4ea0a2afb16857ff2643e43c6e3829b31d64`. `configs/train_analysis_pool.yaml` adds only target `baseline_jax_400m` with model `baseline-JAX`, scale `400m`, config `configs/train_config_baseline_tpu_400M_c4_40B_v4_64_tp2.yaml`, model-axis mesh `2`, and the concrete checkpoint below. `scripts/analyze_train_analysis_pool.py` permits a non-operator model only when no operator-interpretability backend item is requested; baseline zero-shot is eligible, while every mechanistic preset still passes through the unchanged spatial-model validator and therefore cannot run on baseline-JAX.
- Frozen checkpoint: `gs://dawn-tpu-data-c4/checkpoints/baseline_400M_c4_40B_v4_64/run_vbaseline-JAX_20260713_194029_3201/checkpoints/000000076293`. Read-only GCS enumeration found committed steps `75,500`, `76,000`, and `76,293`; step `76,293` is the final committed epoch checkpoint and is selected once. Its metadata records global step `76,293`, epoch `1`, 76,293 steps per epoch, 78,124,032 consumed examples, 39,999,504,384 consumed tokens, model `baseline-JAX`, dimensions `18 x 1280`, FFN `5120`, 20 heads, sequence length `512`, training batch `1024`, mesh `data=16, model=2`, and original training commit `27b239b6d8ab445ab5f3e8ee593ffb54c50d3dee`. The checkpoint full-config hash is `c3ce5d7215388f18287658a7b6603fe00e0ea32c2ed84c6b625545c73a82fc25`; the 12,016-byte downloaded metadata object has SHA-256 `ab44bfad44ae048aabd71549a0af82de20429baa521bacbc5706d33c4fbeabb5`. The metadata val loss `2.935926878` is a training-time record and is not a paper C4 measurement.
- C4 protocol fixed before restore: run existing `scripts/c4_validation_eval_jax.py` protocol `packed_c4_validation_v1` directly from the concrete step with global evaluation batch `32`, sequence length `512`, `max_val_tokens=10,000,000`, `max_batches=0`, token-reduction chunk `32,768`, float32 evaluation, mesh `16x2`, no optimizer, and no checkpoint write. This is exactly the DAWN 400M `2026-07-29/#06` protocol: evaluate every complete global batch in the capped packed view and preserve `validation_summary.json`, `run_manifest.json`, and `summary.log`.
- Zero-shot protocol fixed before restore: after C4 completes, run only preset `zero_shot` through target `baseline_jax_400m`, with the unchanged six stock tasks LAMBADA, HellaSwag, PIQA, ARC-Easy, ARC-Challenge, and WinoGrande; `limit=null`; zero few-shot examples; global batch `32`; length buckets `64/128/256/512`; token chunk `32,768`; maximum generation `256`; 100,000 bootstrap iterations; fixed harness `lm-eval==0.4.2`; validation CE tolerance `5e-5`; seed `4172`; mesh `16x2`; compact aggregate artifacts. Do not run mechanistic items on the dense baseline.
- Local/live evidence: repository-local Python/JAX/Flax/Optax/NumPy were `3.12.7/0.6.2/0.10.7/0.2.8/2.5.1`. Module compilation, target/runtime resolution, checkpoint-metadata versus declared-model validation, launcher syntax, exact zero-shot dry-run, and `git diff --check` passed. No debug script or test file was created or run. A live read-only resource check found the existing user-authorized `spatial-se-400m` `READY` and `HEALTHY` as v4-64 with eight endpoints, and the all-host process audit found no existing training, analysis-pool, C4, or zero-shot Python owner.
- Execution decision: publish this pre-effect record on POC, then use its exact clean pushed descendant for both measurements. Run C4 first and finalize its artifact; then run zero-shot without changing checkpoint or protocol. Parameter count, restored parameter identity, C4 metrics, production/adapter cross-check, zero-shot scores, comparison deltas, and matched-retention conclusions remain `not measured` in this entry.

#### #04 — Stop dense C4 before data and select its canonical production kernel

- Status/claim boundary: `operational incident before effect; fixed and republished`. The first remote C4 command used exact clean pre-effect commit `e9b25e3d1d732b9a191d0956bcb9f44b52d8a665`, the frozen baseline step `76293`, mesh `16x2`, and output root ending `c4_validation_20260729T153143Z-9dc7c71e`. All eight hosts initialized JAX and restored the committed checkpoint, but execution stopped before validation-data construction or any C4 batch at `ValueError: kernel_profile='production_diagnostics' is v417x-only, got baseline-JAX`. No `validation_summary.json`, paper C4 metric, or zero-shot access was produced.
...
#### #06 — Stop dense zero-shot before task evaluation and install the pinned harness

- Status/claim boundary: `operational incident before effect; no scientific result`. The first dense zero-shot launch used exact clean commit `5f6abc6a42020bcb2f9729afc240c6541660b238`, target `baseline_jax_400m`, preset `zero_shot`, runtime v4-64, checkpoint step `76293`, and mesh `16x2`, but included launcher option `--no-install`. The remote environment did not contain `lm-eval`, so the backend failed its first harness gate at `required=0.4.2 detected=<not installed>`.
- Split/effect boundary: failure occurred at the version check before tokenizer setup, stock-task dataset loading, request construction, log-likelihood scoring, or metric aggregation. The outer manifest records `status=failed`, `completed_item_count=0`, no raw parameters, and no dense capture rows. No zero-shot task score, comparison delta, or paper result exists at this root.
- Evidence: result root `gs://dawn-tpu-data-c4/checkpoints/baseline_400M_c4_40B_v4_64/run_vbaseline-JAX_20260713_194029_3201/side_analysis/run_analysis_000000076293_zero_shot_20260729T154705Z-be365a8a`; its only objects are `run_manifest.json` and `summary.log`. Downloaded audit copies had SHA-256 `e18fc3cfe0c81906f40060057596b6787cf7020786560c42e9cdf9547c1c8157` and `61f57d39011f36c03d67e87c8eaf0cd82540b5ca226348b962e9dbe94cbbfdbd`.
- Correction/retry: use the launcher's existing dependency path, which pins `requirements_zero_shot_eval.txt` and reasserts JAX/Flax/Optax/NumPy `0.6.2/0.10.7/0.2.8/2.2.6` independently on all eight hosts before starting `tmux train`. Do not change the checkpoint, tasks, task limit, scoring metrics, harness version, tokenizer, batch, mesh, seed, or protocol. Start a new independent run rather than resuming the failed root.
- Live retry boundary: the exact independent retry root ends `run_analysis_000000076293_zero_shot_20260729T155031Z-89157be7` and still uses clean commit `5f6abc6a42020bcb2f9729afc240c6541660b238`. At the time this incident record was written, all host version assertions, exact checkpoint restore, tokenizer hash, and 16,352-target-token adapter CE cross-check had passed; six-task scoring was active and no effect result was yet available.

#### #07 — Complete the matched dense Transformer 400M six-task zero-shot row

- Status/decision: `TPU measured; complete; comparable paper result`. The exact independent retry completed all six frozen stock tasks with `limit=null`, zero few-shot examples, `smoke_test_only=false`, and `comparable=true`. Accept this as the matched dense Transformer 400M zero-shot paper row and close the 400M performance-retention track.
- Exact run identity: canonical branch `codex/v4167-poc`, exact clean deployed/evaluation commit `5f6abc6a42020bcb2f9729afc240c6541660b238`, existing user-authorized TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, target `baseline_jax_400m`, model `baseline-JAX`, config `configs/train_config_baseline_tpu_400M_c4_40B_v4_64_tp2.yaml`, exact checkpoint step `76293`, 393,896,960 parameters, 39,999,504,384 pretraining tokens, checkpoint-config hash `c3ce5d7215388f18287658a7b6603fe00e0ea32c2ed84c6b625545c73a82fc25`, and restored parameter-identity hash `1ca4f147733c55539baa19e03b4362afcd4ff41216b2ac2a0faa19debce3d053`.
- Frozen protocol/environment: `mamba_table3_zero_shot_v1`, stock `lm-eval==0.4.2`, global batch `32`, maximum length `512`, length buckets `64/128/256/512`, token-reduction chunk `32,768`, zero few-shot examples, no optimizer, and no checkpoint write. Software versions datasets/Flax/fsspec/gcsfs/huggingface-hub/JAX/jaxlib/lm-eval/Orbax/PyArrow/tokenizers/Transformers were `2.19.2/0.10.7/2024.3.1/2024.3.1/0.36.2/0.6.2/0.6.2/0.4.2/0.11.24/20.0.0/0.19.1/4.40.2`; `working_tree_clean=true`. The 16,352-target-token adapter cross-check differed from the canonical evaluator by `1.8859737771492746e-7`, below tolerance `5e-5`.
- Dense result: LAMBADA accuracy `0.3531923151562197 +/- 0.006658942751641765` and perplexity `24.004181895276243 +/- 0.8072977273791623`; HellaSwag normalized accuracy `0.4570802628958375 +/- 0.004971364031062592`; PIQA normalized accuracy `0.6980413492927094 +/- 0.010711732891588357`; ARC-Easy normalized accuracy `0.47558922558922556 +/- 0.010247548905242267`; ARC-Challenge normalized accuracy `0.24914675767918087 +/- 0.012639407111926435`; WinoGrande accuracy `0.5248618784530387 +/- 0.01403510288362775`. The six-task primary-metric mean is `0.45965196484436865`.
- Exact matched audit: the DAWN and dense backend manifests are identical for batch, checkpoint mesh, task limit, lm-eval version, maximum sequence length, effective mesh, zero-shot count, protocol name/version, every task-config hash, task list/order, token chunk, vocab size, complete tokenizer object, complete dataset fingerprint/revision object, and complete software-version object. Their only unequal top-level fields are the expected model/checkpoint/result identities, timestamps, known validation loss, and validation cross-check values.
- Matched comparison: DAWN six-task mean `0.46287245964071566` exceeds dense by `0.0032204947963470065` (`0.322049` percentage point). DAWN-minus-dense task differences are LAMBADA `-0.001164370269745807`, HellaSwag `+0.0012945628360884198`, PIQA `+0.014145810663764968`, ARC-Easy `+0.008417508417508435`, ARC-Challenge `+0.007679180887372017`, and WinoGrande `-0.011049723756906049`; the maximum absolute difference is `1.414581` percentage points. Dense LAMBADA perplexity is lower by `0.07320179152326745`. Jointly with `2026-07-30/#05`, these matched descriptive results support 400M performance retention, not formal equivalence or superiority.
- Evidence/storage: result root `gs://dawn-tpu-data-c4/checkpoints/baseline_400M_c4_40B_v4_64/run_vbaseline-JAX_20260713_194029_3201/side_analysis/run_analysis_000000076293_zero_shot_20260729T155031Z-89157be7`; backend host-result hash `e941f5115a99ed3dd53c3c883a4e896eaa4eeda60ebcba819788e2eb7a37b99c`; `results_summary.json` SHA-256 `31661e30f3c8780ae9e29f96949aa321e9c68271c789e121d6a132a479e37247`; backend `run_manifest.json` SHA-256 `b32eeab6e787889828e1181fa0594783de0ad71dd2f88b6613c6c4f294ff7507`; outer `summary.json` SHA-256 `e1ccfb998760ce37463a28cde470129fc6dba464e4ad4287eb60026b131ece15`. The root contains 13 aggregate objects totaling 65,359 bytes, all six items are `ready`, artifact warnings are empty, and manifests record `sample_records_persisted=false`, `harness_raw_persisted=false`, `raw_parameters_persisted=false`, and `dense_capture_rows_persisted=false`. The post-run all-host audit returned the same exact commit, zero tracked changes, zero remaining analysis owners, and zero matches for traceback/failure/fatal patterns on every host.
- Decision/next: freeze these matched 400M rows for the paper. Do not retune, rerun, or reinterpret IOI/RAVEL/ARC from the performance result. Proceed sequentially to exact numeric-step resolution and same-protocol C4/zero-shot performance evaluation of the registered DAWN-SRW v4172 1.3B run; baseline-JAX 1.3B remains excluded while training.

#### #08 — Freeze the DAWN-SRW v4172 1.3B performance boundary

- Status/split boundary: `implementation committed and pre-effect protocol frozen; restore, C4, and zero-shot not measured`. The registry-designated DAWN-SRW v4172 1.3B run is now represented by canonical target `v4172_1B`, whose checkpoint field points directly to committed Orbax step `000000087193` rather than to a moving run root. No model, checkpoint, training config, evaluation kernel, dataset, task, or metric definition changed.
- Exact implementation identity: canonical branch `codex/v4167-poc`, clean target-pin commit `18ae13a9c4d60b4a6de21bda0a8a617b8d239fa4`. The only target change replaces the registered run root with `gs://dawn-tpu-data-c4/checkpoints/train_config_v4172_1p3B_c4_20B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260717_173556_3201/checkpoints/000000087193`; target model, scale, config, runtime, and model-axis mesh remain `spatial-r1-v4.1.7.2`, `1b`, `configs/train_config_v4172_1p3B_c4_20B_v4_64_den_qk0p5_v1p0_rst1p2.yaml`, v4-64, and `2`.
- Frozen checkpoint identity: read-only GCS enumeration found committed steps `86,800`, `87,000`, and `87,193`; step `87,193` is the final committed epoch checkpoint and is selected once. Its metadata records global step `87,193`, epoch `1`, 87,193 steps per epoch, 39,062,464 consumed examples, 19,999,981,568 consumed tokens, model `spatial-r1-v4.1.7.2`, dimensions `24 x 4096`, 32 heads, sequence length `512`, QK/V/RST operator pools `8,534/26,904/54,352`, training batch `448`, mesh `data=16, model=2`, and original training commit `91cc948e3d6bc435a0c02fd1737ba0c85f2f7b7c`. The configured parameter count is `1,272,038,404`; an exact restored count remains pending.
- Config/evidence identity: checkpoint `raw_config` exactly equals the registered local YAML, and both materialize to selected/full-config hash `1e877263c7113fc9fc6a7a5f6d24b3b96b9f389bcb85cd8e3068c84f104a5222`. The downloaded 10,771-byte metadata object has SHA-256 `e6cb2514b52a55fda3cd5901666283e67894a4423154a543844328ee3389c3f6`; the 204-byte commit marker has SHA-256 `d5021b4bb10ad71033fbe23e8b942193a7c57c4587f747a95328bb1199205552`. Metadata validation loss `2.8363958039412007` is a training-time record and is not a paper C4 measurement.
- C4 protocol frozen before restore: run existing protocol `packed_c4_validation_v1` directly from the concrete step with global evaluation batch `32`, sequence length `512`, `max_val_tokens=10,000,000`, `max_batches=0`, token-reduction chunk `32,768`, float32 evaluation, mesh `16x2`, the unchanged v417x `production_diagnostics` kernel, no optimizer, and no checkpoint write. This is exactly the DAWN and dense 400M paper protocol; evaluate every complete global batch in the capped packed view and preserve `validation_summary.json`, `run_manifest.json`, and `summary.log`.
- Zero-shot protocol frozen before restore: only after the C4 artifact is complete and published, run preset `zero_shot` through target `v4172_1B`, with the unchanged stock tasks LAMBADA, HellaSwag, PIQA, ARC-Easy, ARC-Challenge, and WinoGrande; `limit=null`; zero few-shot examples; global batch `32`; length buckets `64/128/256/512`; token chunk `32,768`; maximum generation `256`; 100,000 bootstrap iterations; fixed harness `lm-eval==0.4.2`; validation CE tolerance `5e-5`; seed `4172`; mesh `16x2`; and compact aggregate-only artifacts.
...
#### #10 — Measure the DAWN-SRW v4172 1.3B packed C4 row

- Status/decision: `TPU measured; complete; comparable paper result`. The first clean restore of the frozen step and the unchanged `packed_c4_validation_v1` protocol completed over all 610 complete global batches admitted by the 10M-token cap. Accept this artifact as the DAWN-SRW v4172 1.3B C4 paper row and open only the already-frozen unlimited six-task zero-shot preset.
- Exact run identity: canonical branch `codex/v4167-poc`, exact clean deployed/evaluation commit `f93d7ed5612525c8a22a3f695814681ca58d9157`, existing user-authorized TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, model `spatial-r1-v4.1.7.2`, config `configs/train_config_v4172_1p3B_c4_20B_v4_64_ver1_den_qk0p5_v1p0_rst1p2.yaml`, and concrete checkpoint `gs://dawn-tpu-data-c4/checkpoints/train_config_v4172_1p3B_c4_20B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260717_173556_3201/checkpoints/000000087193`. The restored checkpoint contains exactly `1,272,038,404` parameters, records 19,999,981,568 pretraining tokens, full-config hash `1e877263c7113fc9fc6a7a5f6d24b3b96b9f389bcb85cd8e3068c84f104a5222`, and restored parameter-identity hash `5789893f21428224800d42b00005a5b8df67d136a7eec6c07bcef09bf0f0be81`.
- Frozen protocol/environment: no-update float32 scoring, global batch `32`, sequence length `512`, `max_val_tokens=10,000,000`, `max_batches=0`, token-reduction chunk `32,768`, unchanged v417x `production_diagnostics` kernel, and no checkpoint write. Software JAX/jaxlib/Flax/NumPy/Orbax/fsspec/gcsfs were `0.6.2/0.6.2/0.10.7/2.2.6/0.11.24/2024.3.1/2024.3.1`; `working_tree_clean=true`.
- Cohort/result: the capped view contained 19,531 sequences and 9,999,872 source tokens. Exactly 19,520 sequences formed 610 complete global batches; 11 incomplete-tail sequences were dropped, leaving 9,974,720 valid next-token targets. Cross-entropy was `2.8358793028776748`, perplexity `17.04538176604594`, and token accuracy `0.4370986854768856` (`43.7098685477%`). The artifact records `comparable=true`.
- Descriptive scale comparison: the DAWN 400M row used the identical evaluation cohort and produced CE `2.94021285810529`, perplexity `18.919873132016434`, and token accuracy `0.4249118772256264`. The 1.3B-minus-400M differences are CE `-0.104333555227615`, perplexity `-1.8744913659705`, and accuracy `+0.0121868082512592` absolute (`+1.218681` percentage points); 1.3B CE is `3.5485%` lower. This is not a controlled scaling curve or dense comparison because pretraining tokens differ by about twofold and the dense 1.3B run remains excluded.
- Data/evidence: validation metadata `gs://dawn-tpu-data-c4/c4_val_meta.json` has SHA-256 `4c2fc359ddd7542052442e5deab2eecd37f5cdf7f3dca19a0943ea47eaf4aaf7`, identical to both 400M paper rows. Result root `gs://dawn-tpu-data-c4/checkpoints/train_config_v4172_1p3B_c4_20B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260717_173556_3201/side_analysis/run_analysis_000000087193_c4_validation_20260729T162030Z-1a691644`; result hash `f5549fffe65c48822f7eff3a241bdf398d9660f54d46809c7fb501103043da46`. The root contains only `run_manifest.json`, `validation_summary.json`, and `summary.log`, totaling 4,291 bytes; downloaded SHA-256 values were `5f69c2efa119c31720ef27baef371425e411f3d0eabd79bddfc3eee8cf964a7d`, `be82950c947f92dda878e11e28f333300af21a0ba95c3c9d830a90cc3e9b57a6`, and `1077f1c4b9dae85991aa036f2820bb262701abe9364eef9dd08f0bc8422339d2`.
- Completion audit/next: all eight hosts reported exact commit `f93d7ed5`, zero tracked changes, zero C4 Python owners, no `train` tmux session, and zero matches for traceback/resource-exhaustion/process-loss/fatal patterns. Publish this result on POC, then run only target `v4172_1B` with the unchanged `zero_shot` preset, `limit=null`, and mesh `16x2`; do not begin selected 1.3B IOI replication until zero-shot is complete.

#### #11 — Complete the DAWN-SRW v4172 1.3B six-task zero-shot row

- Status/decision: `TPU measured; complete; comparable paper result`. The exact unlimited stock suite completed all six frozen tasks with `limit=null`, zero few-shot examples, `smoke_test_only=false`, and `comparable=true`. Accept this artifact as the DAWN-SRW v4172 1.3B zero-shot paper row, close the 1.3B performance track, and open only the selected 1.3B IOI replication.
- Exact run identity: canonical branch `codex/v4167-poc`, exact clean deployed/evaluation commit `f028b36bfa1fd4fae8956b6655916d7a00012a1b`, existing user-authorized TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, target `v4172_1B`, model `spatial-r1-v4.1.7.2`, exact checkpoint step `87193`, `1,272,038,404` parameters, 19,999,981,568 pretraining tokens, checkpoint-config hash `1e877263c7113fc9fc6a7a5f6d24b3b96b9f389bcb85cd8e3068c84f104a5222`, and restored parameter-identity hash `5789893f21428224800d42b00005a5b8df67d136a7eec6c07bcef09bf0f0be81`.
- Frozen protocol/environment: `mamba_table3_zero_shot_v1`, stock `lm-eval==0.4.2`, global batch `32`, maximum length `512`, length buckets `64/128/256/512`, token-reduction chunk `32,768`, zero few-shot examples, no optimizer, and no checkpoint write. Software datasets/Flax/fsspec/gcsfs/huggingface-hub/JAX/jaxlib/lm-eval/Orbax/PyArrow/tokenizers/Transformers were `2.19.2/0.10.7/2024.3.1/2024.3.1/0.36.2/0.6.2/0.6.2/0.4.2/0.11.24/20.0.0/0.19.1/4.40.2`; `working_tree_clean=true`. The 16,352-target-token validation-CE cross-check differed from the canonical evaluator by `3.281608953464854e-7`, below tolerance `5e-5`.
- Result: LAMBADA accuracy `0.3958858917135649 +/- 0.006813285290879463` and perplexity `17.635757173498437 +/- 0.5389753532583852`; HellaSwag normalized accuracy `0.510157339175463 +/- 0.004988751698341144`; PIQA normalized accuracy `0.7230685527747551 +/- 0.010440499969334526`; ARC-Easy normalized accuracy `0.5096801346801347 +/- 0.010257860554461125`; ARC-Challenge normalized accuracy `0.2687713310580205 +/- 0.012955065963710688`; WinoGrande accuracy `0.5074980268350434 +/- 0.014050905521228577`. The six-task primary-metric mean is `0.485843546039497`.
- Protocol audit: every frozen protocol field matches the DAWN 400M backend manifest exactly, including batch, checkpoint mesh, dataset fingerprints and revisions, dtype, evaluation mode, length buckets, limit, lm-eval task versions, maximum sequence length, effective mesh, zero-shot count, protocol name/version, complete software versions, all task-config hashes, task list/order, token chunk, complete tokenizer object, and vocabulary size. Only expected model/checkpoint/result identities, timestamps, known validation loss, and validation cross-check values differ.
- Descriptive comparison: the 1.3B-minus-DAWN-400M six-task mean difference is `+0.0229710863987813` (`+2.297109` percentage points). Task differences are LAMBADA `+0.043857946827091`, HellaSwag `+0.051782513443537`, PIQA `+0.0108813928182807`, ARC-Easy `+0.0256734006734007`, ARC-Challenge `+0.0119453924914676`, and WinoGrande `-0.0063141278610892`; LAMBADA perplexity is lower by `6.44162651330107`. Relative to dense 400M, the mean difference is `+0.0261915811951283` (`+2.619158` percentage points). These are not controlled scaling-curve or dense-1.3B claims because the pretraining-token budgets differ and no completed matched dense 1.3B evaluation is in scope.
- Evidence/storage: result root `gs://dawn-tpu-data-c4/checkpoints/train_config_v4172_1p3B_c4_20B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260717_173556_3201/side_analysis/run_analysis_000000087193_zero_shot_20260729T163122Z-73a8b2d7`; backend host-result hash `84b211080997fdd0ddb8e842ac958e987e54634209af70c7bcb038c48995a9f6`; `results_summary.json` SHA-256 `df14faa266b8114344524c9c5d4f88eff447283bc1a4c2da82d7e08d5ec13e41`; backend `run_manifest.json` SHA-256 `90a96b4822e582553928d8b33ded118bae3640cccef23aeaf66b731d20613cf0`; outer `summary.json` SHA-256 `53d28b804eea9dc3b7df4fafff2966ca8cdadc37bf5d3302ab9ca6f67f74e065`; outer `run_manifest.json` SHA-256 `0809e90e3442a0ddb2ebf435a75572bb9a5a78f2383dbcab86483bad2b140325`; `summary.log` SHA-256 `496304d4a858a741103616a36d719b9cb283c3ecda6362fcec41e4578321bffb`. The root contains 13 aggregate objects totaling 67,157 bytes; all six items are `ready`, artifact warnings are empty, and manifests record `sample_records_persisted=false`, `harness_raw_persisted=false`, `raw_parameters_persisted=false`, and `dense_capture_rows_persisted=false`.
- Completion audit/next: all eight hosts reported exact commit `f028b36b`, zero tracked changes, zero remaining analysis owners, no `train` tmux session, and zero matches for traceback/failure/fatal patterns. Freeze both 1.3B performance rows. Next preregister and execute only the selected 1.3B IOI replication; do not modify the completed 400M IOI result and do not evaluate baseline-JAX 1.3B.

#### #12 — Bind the selected 1.3B IOI replication to an immutable input contract

- Status/decision: `TPU measured; complete; contract only; no benchmark split opened`. Restore the exact 1.3B paper checkpoint once under the canonical operator-interpretability runtime and accept the resulting IOI input contract as the identity boundary for the selected scale replication. This run performed no behavioral eligibility, operator capture, circuit selection, intervention, validation, or held-out test.
- Exact run identity: canonical branch `codex/v4167-poc`, exact clean deployed/evaluation commit `c1b366d051ce1d689ddc5fd63d231062801cebe3`, existing user-authorized TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, target `v4172_1B`, model `spatial-r1-v4.1.7.2`, exact checkpoint step `87193`, `1,272,038,404` restored parameters, and checkpoint-config hash `1e877263c7113fc9fc6a7a5f6d24b3b96b9f389bcb85cd8e3068c84f104a5222`.
- Frozen identities: checkpoint identity `2e38b41fc298d60254ca9cb12ad0a06032b12f5e7a8c26f7e83f62afd74bf5f9`; parameter-schema hash `dda425f34bbab46bb3913d4abff5af3e1558a27a70114bf2a6645acdbc0104d5`; model-config hash `8097a118097cf174e0da66839112e1ac767004699d8ad1c654321f61ec8adb61`; benchmark build `f056bb4f133cde4fc0ae02f9`; benchmark-manifest hash `f3fc05c12ac26c83e0cdf8089ef46bae7f26b517265545521605d767a8eafcb6`; tokenizer revision `86b5e0934494bd15c9632b12f734a8a67f723594`; tokenizer-vocabulary hash `de054a2243ead40e8e1bcc06edf510f87bdff3f2d57fd0788f2715a7b2e0bd38`.
- Split boundary: requested and executed item lists contain only `mib_ioi.input_contract`. No discovery, validation, or test data accessor was invoked; no paired-correct count, ranking, site, route composition, effect, threshold decision, or result-dependent circuit choice was observed. Parameter content was not persisted, and the checkpoint identity is the resolved path/step/run metadata plus parameter schema rather than a parameter-content hash.
- Evidence/storage: result root `gs://dawn-tpu-data-c4/checkpoints/train_config_v4172_1p3B_c4_20B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260717_173556_3201/side_analysis/run_analysis_000000087193_items-24ea44b957ad_20260729T165308Z-675c18db`; item protocol hash `98edd7f02e5c359e39e93e7e6d6ae5fb76b9bcd66ee677da157a88cdb54c2968`. Downloaded `input_contract.json`, `run_manifest.json`, outer `summary.json`, and `summary.log` have SHA-256 `6bd5ca30c1870420d1732695e1220e2e41dbc72d23de9bf50595aa5d6326cb14`, `590294db55f84c9243de39c8917b2a6892224c5bad1b442166faf0756870f3b7`, `916329be7fa219cb5330fe7ed5e44d136b15e92168cd90b7559cff3c20bab671`, and `be48ed4031a7fad17468392445377aa9574bf84f452cb48460ef9159499b82b1`.
- Completion audit/next: every host reported exact commit `c1b366d0`, zero tracked changes, no surviving `train` tmux session or analysis process, and zero traceback/resource-exhaustion/process-loss/fatal matches. Publish this contract before implementing the discovery item. Then freeze one discovery-only protocol using all 128 discovery rows, the same four routes and site identity, a fixed capture-width schedule, rank stability minimum `0.80`, fixed audited prefixes through `4096`, cumulative absolute importance minimum `0.70`, and split overlap minimum `0.95`; validation and test remain inaccessible until a discovery circuit is frozen.

#### #13 — Preregister the separate DAWN-SRW v4172 1.3B IOI discovery localization

- Status/decision: `implementation committed and pre-effect protocol frozen; discovery, validation, and test not measured`. Add one dedicated `mib_ioi.scale_discovery_operator_localization` item and `ioi_scale_discovery_localization` preset whose dependency closure contains only the immutable IOI input contract and this discovery item. The runner rejects any mixed invocation, loads only the discovery split, and cannot access validation or test.
- Exact implementation identity: canonical branch `codex/v4167-poc`, exact clean implementation commit `8bd0a717a002595e25060121d35b31f09410961d`, target `v4172_1B`, checkpoint step `87193`, checkpoint identity `2e38b41fc298d60254ca9cb12ad0a06032b12f5e7a8c26f7e83f62afd74bf5f9`, checkpoint-config hash `1e877263c7113fc9fc6a7a5f6d24b3b96b9f389bcb85cd8e3068c84f104a5222`, model-config hash `8097a118097cf174e0da66839112e1ac767004699d8ad1c654321f61ec8adb61`, benchmark build `f056bb4f133cde4fc0ae02f9`, and benchmark-manifest hash `f3fc05c12ac26c83e0cdf8089ef46bae7f26b517265545521605d767a8eafcb6`.
- Frozen specification: `configs/paper_ioi_scale_discovery_localization_v4172_1p3b.yaml`, canonical content hash `0fa2462b68fb1c08c6666f48bb63e80bfeec3be2ad3155bce268f4d7a6805348`, status `preregistered_before_discovery_localization`. The run uses all 128 discovery rows and every base-and-source-correct independent `example_id`, with minimum eligible count `32`; the exact eligible count is intentionally unknown before execution. Literal 400M operator IDs are forbidden, and Q/K/V/RST route presence or absence is a discovery result that cannot be changed afterward.
- Capture/ranking freeze: production-precision contribution at the official last prompt token; captured-mass threshold `0.95`; initial QK/V/RST widths `1024/4096/8192`; maximum widths `4096/16384/16384`. These widths are the nearest power-of-two schedule preserving the 400M captured-pool fractions against fixed 1.3B pool sizes `8,534/26,904/54,352`; they are fixed before any 1.3B behavior or ranking is observed. Site identity is `(layer, route, operator_id)`, routes are exactly `q/k/v/rst`, ranking is absolute discovery mean contribution importance, tie-breaking is `layer/route/operator_id`, and the split rule remains the 400M-compatible seeded hash of benchmark plus example ID with at least 16 independent units per half.
- Circuit gate: require rank stability at least `0.80`; audit only prefixes `32/64/128/256/512/1024/2048/4096`; freeze the smallest prefix jointly reaching cumulative absolute importance at least `0.70` and split top-k overlap at least `0.95`. Insufficient behavior, unstable ranking, or no passing prefix freezes no circuit and ends the 1.3B causal replication without opening validation or test.
- Confirmatory protocol if and only if frozen: validation first over all fixed base-and-source-correct validation rows with minimum `32`; intact, full-denominator frozen suppression, equal-count/same-layer/activation/route-frequency controls each `x100`, and exact same-example selected-numerator restoration. Control seeds remain `14172/14173/14174/14175`; bootstrap/permutation counts remain `2000/2000`, alpha `0.05`; validation requires a suppression-drop CI lower bound above zero, the discovered drop to exceed every control family, and restoration-recovery CI lower bound at least `0.80`. Held-out test remains inaccessible until the validation record is final and may not change sites, suppression, controls, restoration, or thresholds.
- Local validation: repository-local Python/JAX/Flax `3.12.7/0.6.2/0.10.7`; production modules compiled; both the existing ARC frozen-spec loader and the new IOI scale-spec loader returned their exact embedded hashes; dependency closure resolved exactly to `mib_ioi.input_contract` then `mib_ioi.scale_discovery_operator_localization`; item/target catalogs loaded; Git Bash syntax validation and the exact launcher dry run with the six frozen capture widths passed; `git diff --check` passed. No debug script or test file was created.
- Decision/next: publish this preregistration before effect measurement, then deploy one exact clean pushed descendant on the existing user-authorized `spatial-se-400m` and run discovery only. If a circuit is frozen, generate a separate hash-bound frozen circuit file from the exact aggregate discovery artifact before validation; otherwise record the fail-closed negative result and stop the scale-causal track.

#### #14 — Close the selected DAWN-SRW v4172 1.3B IOI replication at the discovery gate

- Status/decision: `TPU measured; complete; valid fail-closed negative result`. The discovery-only run passed behavioral eligibility and rank stability but returned `no_preregistered_prefix`. No 1.3B circuit is frozen, `confirmatory_eligible=false`, and the preregistered validation, matched controls, restoration, and held-out test must not be opened.
- Exact run identity: canonical branch `codex/v4167-poc`, exact clean deployed/evaluation commit `51fe9e30a1781996407183fd8f83fa78c951d649`, existing user-authorized TPU `spatial-se-400m`, TPU v4-64 with 32 devices/eight hosts, mesh `data=16, model=2`, target `v4172_1B`, model `spatial-r1-v4.1.7.2`, exact checkpoint step `87193`, checkpoint identity `2e38b41fc298d60254ca9cb12ad0a06032b12f5e7a8c26f7e83f62afd74bf5f9`, checkpoint-config hash `1e877263c7113fc9fc6a7a5f6d24b3b96b9f389bcb85cd8e3068c84f104a5222`, model-config hash `8097a118097cf174e0da66839112e1ac767004699d8ad1c654321f61ec8adb61`, benchmark build `f056bb4f133cde4fc0ae02f9`, benchmark-manifest hash `f3fc05c12ac26c83e0cdf8089ef46bae7f26b517265545521605d767a8eafcb6`, preregistered specification hash `0fa2462b68fb1c08c6666f48bb63e80bfeec3be2ad3155bce268f4d7a6805348`, and runtime protocol hash `a3367de6511c25b556496fc4292acacf3343cfbcc72878e8a037fcd4d1a8c9f1`.
- Behavioral eligibility: all 128 discovery rows were evaluated without opening validation or test. Base accuracy was `1.0`, source and pair accuracy were `0.9765625`, mean base margin was `3.2305120266973972`, and 125 independent base-and-source-correct `example_id` units passed the fixed minimum of 32. Their identity hash is `c1e160cdcd5db6a50e78661dff67ec9044616b28ec4be856cd930423333b8e0b`.
- Capture/ranking: the exact adaptive schedule ran `1024/4096/8192 -> 2048/8192/16384 -> 4096/16384/16384`; only the final preregistered maximum qualified every route at captured mass `>=0.95`. All `12,000/12,000` route-layer-example rows qualified, producing `1,168,379` aggregate ranked sites with content hash `2700df70727f3c32cd3b2a14e65532a084e77d35d3d925d95625ac1c6f868115` and raw-capture digest `2ffac5fb34fd11ba282935e92a93244f0640d1dd046e11e1ceb56e38b9083b28`. Aggregate absolute importance was Q/K/V/RST `41.2711%/57.0154%/0.9827%/0.7309%`; V and RST first appeared at ranks `6323` and `4366`, respectively. Raw capture rows and per-example operator vectors were not persisted.
- Frozen discovery gate: the seeded discovery halves contained `61/64` independent units and rank stability was `0.9071877488194419`, above the fixed `0.80` threshold. Prefix `32/64/128/256/512/1024/2048/4096` cumulative-importance fractions were `0.047066/0.082953/0.124632/0.177221/0.245260/0.340202/0.471907/0.635144`; their split-overlap fractions were `1.0/1.0/0.984375/0.984375/0.972656/0.966797/0.967285/0.972412`. Every audited prefix met the `0.95` overlap gate, but none reached the joint `0.70` cumulative-importance gate. The largest allowed prefix still missed by `0.064856`, so `selected_k=0` is the protocol-mandated result rather than a tunable choice.
- Evidence/storage: result root `gs://dawn-tpu-data-c4/checkpoints/train_config_v4172_1p3B_c4_20B_v4_64_ver1_den_qk0p5_v1p0_rst1p2/run_vspatial-r1-v4.1.7.2_20260717_173556_3201/side_analysis/run_analysis_000000087193_ioi_scale_discovery_localization_20260729T170612Z-25fd8097`. It contains six objects totaling `276,538,847` bytes. The `276,497,751`-byte item JSON is large because it preserves the complete aggregate ranking, not raw per-example vectors; its advisory-size warning records that writing continued and nothing was discarded. SHA-256 values are item JSON `aac4f6ddc4189499e0e675403f5bc1e5a51fa2cbd93a66053bb85d5cacb9ace9`, input contract `35d7ffe749cf49dac351eee03092aa670c7c3484cb31755202c8a59cdaa77416`, backend summary `357d75d22408a0189592b6c37f283eee557970e96c6653836ee74366bf5617d2`, outer summary `ac14b919d8b49315c64b0f62d3ef73caa1ef34741db9a0070cde494d53cf7dd8`, outer run manifest `7ed94f9ea30d35c7dd68a1891270827632c35bb115cc1474b888d4fe3ed070ee`, and summary log `a325b83bddd55118cbf2beaa4bda03d0d23aeaefbadb72bfe41e12f00385bcd9`.
- Split/completion audit and next: the artifact explicitly records `validation_data_accessor_called=false`, `validation_evaluated=false`, `test_data_accessor_called=false`, and `test_evaluated=false`. All eight hosts reported exact commit `51fe9e30`, zero tracked changes, no remaining analysis owner, no `train` tmux session, and zero traceback/resource-exhaustion/process-loss/fatal matches. Close selected 1.3B causal replication as a scientific negative result, publish this record on canonical POC, and move to final paper tables, figures, and text without changing the 400M IOI result or reopening 1.3B confirmation.
