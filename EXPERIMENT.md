...
- Current blocker: the exact top-2 forward candidate is locally validated but not TPU measured.
- Next experiment: screen `codex/v4174-exact-top2-forward` against stored generation `g3` with the two-layer gate. Continue to the matched five-step full benchmark only if combined QKV+RST F+B improves by at least 10%, then require retained benefit under step-100-skew routing before promotion.
