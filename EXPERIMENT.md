...
- Next experiment: run only the preregistered ARC discovery localization on the v4172 400M checkpoint. Freeze a circuit only if rank stability and one audited prefix pass their fixed gates; otherwise record `no frozen circuit` and do not open ARC validation or test. Preserve both the successful IOI record and the negative RAVEL discovery record unchanged. The matched dense 400M performance track follows ARC on the single available TPU.
