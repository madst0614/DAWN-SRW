...
- Active run: the old-sign generation `g2` workload was archived and replaced as authorized. Generation `g3`, the all-24-space exact candidate, and the group4 exact refinement have completed their requested from-scratch protocols; the TPU benchmark tmux wrappers are idle after completion.
- Current blocker: both exact-forward candidates now have matched full-step evidence, but neither has a user disposition. Step-100-skew execution is not measured: generation `g3` was a benchmark-only control with no checkpoint, and the older recorded step-100 run has aggregate routing counts but no saved checkpoint or exact per-token routing replay artifact.
- Next experiment: report the full-step, two-layer diagnostic, compile, HBM, validity, and skew-availability evidence without promoting, merging, or deleting either candidate. Await the user's keep, hold, discard, or additional-skew-measurement decision.
