...
- Publication state: accepted-source integration policy commit `4e2ebedf2a88f63c274265efa1930dc26f5d85c9` is published to `origin/codex/v4167-poc`. Its accepted code parent `ec744a2a095227b4d928cdc058816976d10e98f0` contains the bundle4/exact-top-2 execution line, explicit operation-space structure, measured dense RW analytic pullback, and independent-basis/fallback/metric-contract change. Experiment #34 records final-report centralization on the current POC notebook line.
...
- Current blocker: candidate commit `7123dffa` missed the mandatory combined gate by 0.024139 s despite a material isolated QKV gain. RST extension and the full-step protocol were intentionally not run and remain `not measured`.
- Next experiment: preserve the exact-forward and QKV P/RW/U analytic branches as rejected source evidence without merging their code to POC. A further QKV-only design would need to reduce QKV F+B from 0.325456 s to at most 0.301276 s to make the unchanged-RST combined 10% gate; changing the gate or extending RST despite the miss requires a new user decision.
