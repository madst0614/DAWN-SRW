...
- Baseline state: generation `g3` remains the active registry entry until the already measured candidate is promoted through the normal promotion path. The candidate satisfies the two-layer, full-step, compile, HBM, and failure gates without changing the comparison fingerprint.
...
- Current blocker: none for promotion. Step-100 skew remains `not measured` and closed for the rejected exact-forward line; it is not required for this backward-only analytic-boundary extension.
- Next experiment: finish the canonical POC integration, promote the candidate's already measured gate/full-step evidence directly to generation `g4`, update the registry and notebook on `codex/v4167-poc`, and push the central branch.
