...
- Current blocker: no local implementation blocker. The user explicitly authorized the existing TPU `spatial-se-400m` for the IOI validation task; only that exact resource may be contacted. The frozen validation runtime has not yet been launched.
- Next experiment: publish the frozen intervention implementation and this pre-validation record on `codex/v4167-poc`, then run only `ioi_frozen_validation` on `spatial-se-400m`: intact, frozen suppression, four controls times 100, and exact restoration over the 123 paired-correct validation units. Finalize and publish that record before any held-out test accessor is called.
